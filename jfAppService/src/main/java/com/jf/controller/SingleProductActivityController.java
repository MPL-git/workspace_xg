...
//	/**
//	 * 
//	 * 方法描述 ：单品秒杀tab type = 3（废弃：2018-07-17，需求676，限时抢购改版）
//	 * @author  chenwf: 
...
//	/**
//	 * 
//	 * 方法描述 ：单品秒杀 type = 3 (废弃:因为做品牌团的，要兼容旧版本，把这个接口废弃重做)
//	 * @author  chenwf: 
//	 * @date 创建时间：2017年10月9日 下午4:54:16 
//	 * @version
//	 * @param request
//	 * @return
//	 */
//	@RequestMapping(value = "/api/n/getSeckillTimeList")
//	@ResponseBody
//	public ResponseMsg getSeckillTimeList(HttpServletRequest request){
//		try {
//			JSONObject reqPRM = (JSONObject) request.getAttribute("reqPRM");
//			JSONObject reqDataJson = reqPRM.getJSONObject("reqData");// 获取请求参数
//			
//			Object[] obj = {"pageSize","currentPage","beginTime"};
//			this.requiredStr(obj,request);
//			
//			long beginTime = reqDataJson.getLong("beginTime")/1000;
...
//			List<Map<String,Object>> list = singleProductActivityService.getSeckillTimeList(params);
...
//	}
	...
	@RequestMapping(value = "/api/n/getNewSeckillTimeTab")
	@ResponseBody
	public ResponseMsg getNewSeckillTimeTab(HttpServletRequest request){
		try {
			JSONObject reqPRM = (JSONObject) request.getAttribute("reqPRM");
			...
			if((system.equals(Const.ANDROID) && version >= 45) || (system.equals(Const.IOS) && version >= 48)){
				list = singleProductActivityService.selectSeckillTimeTab();
			}else {
				list = singleProductActivityService.getNewSeckillTimeTab();
			}
			//头部图
			QueryObject queryObject = new QueryObject();
	        queryObject.addQuery("status", Const.AD_STATUS_UP);
			...
			if(CollectionUtils.isNotEmpty(adInfoList)){
				for (AdInfo adInfo : adInfoList) {
					if(topPicList.size() <= 5){
						Map<String,Object> adMap = adInfoService.getAdLocation(adInfo,reqPRM);
						topPicList.add(adMap);
					}else{
						break;
					}
	...
	public ResponseMsg getNewUserSeckillTimeList(HttpServletRequest request){
		try {
			...
			String reqType = "";
			if(reqDataJson.containsKey("reqType")){
				reqType = reqDataJson.getString("reqType");
			}
			...
			return new ResponseMsg(ResponseMsg.SUCCESS,ResponseMsg.SUCCESS_MSG,Const.RETURN_SIZE_10,list);
		} catch (ArgException args){
			return new ResponseMsg(ResponseMsg.ERROR,args.getMessage());
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @return
	 */
	@RequestMapping(value = "/api/n/getSingleNewExplosionActivityList")
	@ResponseBody
	public ResponseMsg getSingleNewExplosionActivityList(HttpServletRequest request){
		try {
			JSONObject reqPRM = (JSONObject) request.getAttribute("reqPRM");
			JSONObject reqDataJson = reqPRM.getJSONObject("reqData");// 获取请求参数
			
			Object[] obj = {"currentPage","type","productTypeIdOne"};
			this.requiredStr(obj,request);
			String type = reqDataJson.getString("type");
			//商品一级品类id
			Integer productTypeIdOne = reqDataJson.getInt("productTypeIdOne");
			Integer pageSize = Const.RETURN_SIZE_10;//页数
			Integer currentPage = Integer.valueOf(reqDataJson.getString("currentPage"));//页码
			if(currentPage == null){
				currentPage = 0;
			...
			params.put("pageSize", pageSize);
			params.put("currentPage", currentPage*pageSize);
			List<Map<String,Object>> list = singleProductActivityService.getSingleNewExplosionActivityList(params);
			return new ResponseMsg(ResponseMsg.SUCCESS,ResponseMsg.SUCCESS_MSG,pageSize,list);
		} catch (ArgException args){
			return new ResponseMsg(ResponseMsg.ERROR,args.getMessage());
		} catch (Exception e) {
...
//			//商品一级品类id
//			String productTypeIdOne = "";
//			if(reqDataJson.containsKey("productTypeIdOne")){
//				productTypeIdOne = reqDataJson.getString("productTypeIdOne");
//			}
//			//最低价格
...
//				if((version == 18 && system.equals(Const.ANDROID))){
//					pageSize = 100;
//				}
//			} 
...
//			params.put("pageSize", pageSize);
//			params.put("currentPage", currentPage*pageSize);
//			List<Map<String,Object>> list = singleProductActivityService.getSingleExplosionActivityList(params,version,system);
//			return new ResponseMsg(ResponseMsg.SUCCESS,ResponseMsg.SUCCESS_MSG,pageSize,list);
//		} catch (ArgException args){
//			return new ResponseMsg(ResponseMsg.ERROR,args.getMessage());
//		} catch (Exception e) {
...
//	}
	...
	public ResponseMsg getSingleNewEnjoyActivityList(HttpServletRequest request){
		try {
			...
			List<Integer> topAdIdList = new ArrayList<Integer>();
			Map<String,Object> dataMap = new HashMap<>();
			...
			if(CollectionUtils.isNotEmpty(adInfoList)){
				...
				topAdIdList.add(adInfo.getId());
			}
			dataMap.put("topPic", topPicList);
			dataMap.put("topAdId", topAdIdList);
			dataMap.put("data", list);
...
//			System.out.println(dataMap);
	...
	public ResponseMsg getSingleBrokenCodeClearingActivityList(HttpServletRequest request){
		try {
			...
			//version <= 28 propValId是尺码id
			//version > 28 propValId是尺码别名
			...
			if(system.equals(Const.ANDROID) && version <= 28){
				paramsMap.put("propValId", propValId);
			}else{
				//如果Android是新版本的，根据同义词别名找出对应尺码id集合

...
public class SubDepositOrderController extends BaseController {
	...
	public ResponseMsg updateSubDepositOrder(HttpServletRequest request) {
		try {
			...
			Object[] obj = {"memberId", "combineDepositOrderId"};
			this.requiredStr(obj,request);
			
			Date date = new Date();
			Integer memberId = Integer.parseInt(reqDataJson.getString("memberId"));//会员标识id
			...
			if(lockedAmount > 0) {
				...
				productItemService.cancelReleaseProductLockedAmount(productItemId, lockedAmount, null);
			}

...
public class ButtonService {
	 ...
	 */
	public Map<String, Object> getEvaluateButtonButton(String subOrderId, int orderDtlSize, String isComment, String subOrderStatus) {
		boolean button = true;
		String msg = "";
		if(!StringUtil.isBlank(subOrderId)){
			if(!"0".equals(isComment) || subOrderStatus.equals(Const.ORDER_STATUS_CLOSE)){
				button = false;
				msg = "未确认收货不能评价";
			}
		}else{
	 ...
	 * @param isComment 
	 */
	public Map<String, Object> getAfterSalesApplyButton(String subOrderStatus, String customerServiceOrderStatus,String isGive, String activityType, String productStatus) {
		boolean button = false;
		String msg = "";
		customerServiceOrderStatus = customerServiceOrderStatus == null ? "" :customerServiceOrderStatus;
		if(!activityType.equals(Const.PRODUCT_ACTIVITY_TYPE_BARGAIN) && !activityType.equals(Const.PRODUCT_ACTIVITY_TYPE_INVITE_FREE_CHARGE)){
	 ...
	 * @param isComment 
	 */
	public Map<String, Object> getAfterSalesApplyAgainButton(String subOrderStatus, String customerServiceOrderStatus, String isGive, String activityType, List<InterventionOrder> interventionOrders, String serviceType, String productStatus) {
		boolean button = false;
		String msg = "";
		customerServiceOrderStatus = customerServiceOrderStatus == null ? "" :customerServiceOrderStatus;
		if(!activityType.equals(Const.PRODUCT_ACTIVITY_TYPE_BARGAIN) && !activityType.equals(Const.PRODUCT_ACTIVITY_TYPE_INVITE_FREE_CHARGE)){
			if("0".equals(isGive)){
				if(!"2".equals(productStatus)){
					if(subOrderStatus.equals(Const.ORDER_STATUS_SHIPPED) || subOrderStatus.equals(Const.ORDER_STATUS_RECEIVED_GOODS)){
						if(!StringUtil.isBlank(customerServiceOrderStatus)){
							if(customerServiceOrderStatus.equals(Const.CUSTOMER_ORDER_STATUS_REJECT) || customerServiceOrderStatus.equals(Const.CUSTOMER_ORDER_STATUS_CANCEL)){
								if(CollectionUtil.isEmpty(interventionOrders)){
									button = true;
								}else{
									msg = "介入正在处理中。。。";
								}
							}else if(customerServiceOrderStatus.equals(Const.CUSTOMER_ORDER_STATUS_SUCCESS) && Const.CUSTOMER_ORDER_TYPE_C.equals(serviceType)){
								button = true;
							}else{
								msg = "售后正在处理中。。。";
							}

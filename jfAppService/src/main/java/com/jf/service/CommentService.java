...
public class CommentService extends BaseService<Comment, CommentExample> {
	...
	public Map<String,Object> addMemberComment(JSONObject reqDataJson) {
		...
		if(type.equals("1")){
			...
			subOrderExample.createCriteria().andIdEqualTo(subOrderId).andIsCommentEqualTo("0").andDelFlagEqualTo("0");
			SubOrder subOrderUpdate = new SubOrder();
			subOrderUpdate.setIsComment("1");
			int count = subOrderService.updateByExampleSelective(subOrderUpdate, subOrderExample);
			if(count <= 0){
				throw new ArgException("您已评价过,请勿重复评价");
			}
			isAppendComment = "0";
		...
		if(type.equals("1")){
			...
			map = commentDrawSettingService.getDrawIntegral(memberId,subOrder);
		}else{
	...
	public Map<String, Object> getUserProductAllComment(Integer productId, String type,Integer currentPage,Integer pageSize,List<Integer> commentIdList) {
		...
		if(CollectionUtils.isNotEmpty(commentCustoms)){
			for (CommentCustom commentCustom : commentCustoms) {
				...
				boolean isRealSVip = memberInfoService.isRealSVip(null, commentCustom.getMemberId());
				...
				dataMap.put("productScore", productScore);
				dataMap.put("mchtReply", mchtReply);
                dataMap.put("isSvip", isRealSVip ? StateConst.TRUE : StateConst.FALSE);
				dataList.add(dataMap);
			}
		}
		Map<String, Object> map = new HashMap<String, Object>();
	...
	public void updateOrderComment(JSONObject reqDataJson) {
		JSONArray dataList = reqDataJson.getJSONArray("dataList");
		Integer memberId = reqDataJson.getInt("memberId");
		Integer subOrderId = reqDataJson.getInt("subOrderId");
		Integer mchtScore = reqDataJson.getInt("mchtScore");
	...
	public List<Map<String, Object>> getTotalCountByMchtIdList(Map<String, Object> paramMap) {
		return commentCustomMapper.getTotalCountByMchtIdList(paramMap);
	}
}

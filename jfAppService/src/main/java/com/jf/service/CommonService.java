...
public class CommonService {
	...
	public String getCloumnLinkUrl(String linkValue, String type) {
		...
		}else if("10".equals(type)){//装修页面
			linkValue = Config.getProperty("mUrl")+"/xgbuy/views/activity/nest/decorate/brand_decorate.html?infoId="+linkValue+"&version=1";
		}else if("11".equals(type)){//精选活动详情页面
			linkValue = Config.getProperty("mUrl")+"/xgbuy/views/activity/nest/decorate/brand_decorate_activesel.html?id="+linkValue;
		}else if("12".equals(type)){//主会场
			linkValue = Config.getProperty("mUrl")+"/xgbuy/views/index.html?redirect_url=main_venue/index.html";
		} else if ("13".equals(type)) { //积分转盘
			linkValue = Config.getProperty("mUrl")+"/xgbuy/views/index.html?redirect_url=activity/pointsLottery/index.html";
		}
		return linkValue;
	}
	...
	public String buildH5Page(String type) {
		...
		if("1".equals(type)){//新星计划分润余额规则
			sbf.append("<HTML>");
			sbf.append("<BODY style='font-size: 13px;'>");
			sbf.append("<style>");
			sbf.append("</style>");
	...
	public Map<String, Object> buildAdMap(AdDTO adDTO) {
		...
		if("7".equals(linkType)){
			...
			}else if("24".equals(linkValue)){
				linkType = "4";
				linkValue = getCloumnLinkUrl(linkValue, "12");
			}
		}

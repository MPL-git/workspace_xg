...
public class MchtShopDynamicService extends BaseService<MchtShopDynamic, MchtShopDynamicExample> {
		...
		if(CollectionUtils.isNotEmpty(mchtShopDynamicList) ) {
			...
			if(!"0".equals(mchtShopDynamic.getCreateBy()) && !"定时任务创建".equals(mchtShopDynamic.getRemarks())) {
				...
				for(MchtShopDynamic mShopDynamic : mShopDynamicList) {
					...
					mShopDynamicMap.put("msdId", mShopDynamic.getId().toString());
					mShopDynamicMap.put("msdContent", mShopDynamic.getContent());
					mShopDynamicMap.put("msdReadCount", mShopDynamic.getReadCount()==null?"0":mShopDynamic.getReadCount().toString());
	...
	public Map<String, Object> getMchtShopDynamicList(JSONObject reqPRM, JSONObject reqDataJson) {
		...
		for(Map<String, Object> m : resultList) {
			...
			//STORY #1886
			InputStream stream = MchtShopDynamicService.class.getResourceAsStream("/config.properties");
	        try {
	        	Properties properties = new Properties();
	        	properties.load(stream);
	        	String originalId = properties.getProperty("originalId");
	        	String xcxShareType = properties.getProperty("xcxShareType");
	        	objectMap.put("originalId", originalId);//小程序原始ID
	        	objectMap.put("xcxShareType", xcxShareType);//分享小程序的版本（0-正式，1-开发，2-体验）
			} catch (Exception e) {
				e.printStackTrace();
			}
	        String redirectType = "";
	        if(source.equals("1")){//1.商家3.自己
	        	redirectType = "0";
	        }else if(source.equals("2")){//2.好友
	        	redirectType = "1";
	        }
	        objectMap.put("wxPath", "page/activity/novaPlan/pages/newsFeed/index?type="+redirectType+"&id=" + m.get("mcht_shop_dynamic_id"));//分享小程序页面的具体路径
			objectMap.put("shareUrl", Config.getProperty("mUrl")+"/xgbuy/views/index.html?redirect_url=activity/novaplan/pages/newsfeed/index.html?type="+redirectType+"&id="+m.get("mcht_shop_dynamic_id"));
	...
	public String formatNumber(int param) {
		String str = "";
	...
	public Map<String, Object> getMemberDynamicDtl(JSONObject reqPRM, JSONObject reqDataJson) {
		...
		map.put("isSvip", memberInfoService.isRealSVip(memberInfo, friendMemberId) ? StateConst.TRUE : StateConst.FALSE);

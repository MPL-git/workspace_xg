...
public class OrderPreferentialInfoService extends BaseService<OrderPreferentialInfo, OrderPreferentialInfoExample> {
    ...
    /**
     * 计算（定金抵扣）单品优惠券优惠、活动优惠或商家优惠
     *
     * @param activityTypeMap          各activityType对应的购物车列表
     * @param activityTotalAmountMap   各activityType对应的活动总金额（已扣除定金抵扣金额）
     * @param memberId
     * @param activityMap              activityType为0和101（即品牌款）的商品ID对应的活动（ActivityCustom）
     * @param combineOrderId
     * @param reqPRM
     * @param reqDataJson
     * @param skuDepositTotalAmountMap 该用户预付了定金的规格ID对应的可抵扣金额（已乘以份数）
     * @param skuDepositTotalMap       该用户预付了定金的规格ID对应的份数和可抵扣金额（已乘以份数）
     * @param tType
     * @return: java.util.Map<java.lang.String, java.lang.Object>
     * activityTotalAmountMap:各activityType对应的活动总金额（已扣除定金抵扣金额、单品优惠金额、会场或商家优惠），可能有多扣了一次定金
     * salePriceMapList:规格的优惠信息列表
     * totalProductPreferentialAmount：购物车的优惠总额(定金抵扣 + 单品优惠 + 会场或商家优惠)
     */
    ...
    public Map<String, Object> getPreferentialInfo(Map<String, List<ShoppingCartCustom>> activityTypeMap,
                                                   ...
                                                   Map<Integer, BigDecimal> skuDepositTotalAmountMap, Map<Integer, Map<String, Object>> skuDepositTotalMap, String tType, Map<Integer, MchtInfo> mchtInfoMap, PreContext preContext) {
            ...
            if (key.equals(Const.PRODUCT_ACTIVITY_TYPE_AREA)) {
                ...
                for (Integer areaId : shopCartAreaMap.keySet()) {
                    activityPreferentialMap = getActivityOrMchtPreferential(shopCartAreaMap.get(areaId),
                            salePriceMapList, areaId, null, memberId, combineOrderId, system, version,
                ...
                for (Integer mId : shopCartMchtMap.keySet()) {
                    activityPreferentialMap = getActivityOrMchtPreferential(shopCartMchtMap.get(mId), salePriceMapList,
                            null, mId, memberId, combineOrderId, system, version, reqDataJson,
                ...
                activityPreferentialMap = getActivityOrMchtPreferential(carts, salePriceMapList, null,
    ...
    /**
     * @param shoppingCartAreas        单个商家或活动对应的购物车列表
     * @param salePriceMapList         一个空的map
     ...
     * @param version
     * @param reqDataJson
     * @param activityTotalAmount      单个商家或活动对应的购物车总金额
     * @param skuDepositTotalAmountMap 该用户预付了定金的规格ID对应的可抵扣金额（已乘以份数）
     * @param skuDepositTotalMap       该用户预付了定金的规格ID对应的份数和可抵扣金额（已乘以份数）
     * @param activityTotalAmountMap   各activityType对应的活动总金额（已扣除定金抵扣金额）
     * @param tType
     * @return: java.util.Map<java.lang.String, java.lang.Object>
     * activityTotalAmountMap:各activityType对应的活动总金额（已扣除定金抵扣金额）（已扣除单品优惠金额）（已扣除会场或商家优惠），可能有多扣了一次定金
     * salePriceMapList:规格的优惠信息列表
     * totalProductPreferentialAmount：单个商家或活动对应的购物车的优惠总额(定金抵扣 + 单品优惠 + 会场或商家优惠)
     */
    public Map<String, Object> getActivityOrMchtPreferential(List<ShoppingCartCustom> shoppingCartAreas, List<Map<String, Object>> salePriceMapList,
                                                             Integer activityAreaId, Integer mchtId, Integer memberId,
        ...
        BigDecimal MPiece = zero;
        Integer productQuantitySum = 0;//会场购买总件数
        BigDecimal totalProductPreferentialAmount = zero;//所有商品的优惠总金额
        Coupon coupon = null;
        ...
        for (ShoppingCartCustom shoppingCart : shoppingCartAreas) {
            boolean isSelfSpopMcht = isSelftSpopMcht(mchtInfoMap.get(shoppingCart.getMchtId()));
            ...
            Map<String, Object> productCouponMap = memberCouponService.getProductCouponAmount(productId, productItemId, reqDataJson, saleOrMallPrice, orderDtlId, tType, combineOrderId, isSelfSpopMcht);//商品券
            ...
            shoppingCart.setSkuDepositTotalAmount(skuDepositTotalAmount);    //定金抵扣
            shoppingCart.setProductCouponAmount(productCouponAmount);        //商品券优惠金额
            shoppingCart.setProductCouponBelong(productCouponBelong);        //商品券提供方
            activityTotalAmount = activityTotalAmount.subtract(productCouponAmount);    //单个商家或活动对应的购物车总金额 - 商品优惠金额（定金抵扣已经在上面减过）
        }
        ...
        if (StringUtil.isBlank(preferentialType)) {
            ...
            for (ShoppingCartCustom shoppingCart : shoppingCartAreas) {
                ...
                BigDecimal productAeductAmount = zero;
                Integer depositQuantity = 0;
                boolean b = true;//商品券有且只能使用一次
        ...
        } else if ("5".equals(preferentialType) && !appContext.olderThan(67, 69)) { //购物津贴
        ...
        for (ShoppingCartCustom shoppingCart : shoppingCartAreas) {
            ...
            if (preferentialType.equals("1")) {
                ...
                if (coupon != null) {
                    ...
                    if (
                            ("1".equals(couponPreferentialType) && activityTotalAmount.compareTo(minimum) >= 0)
                                    ||
                                    ("2".equals(couponPreferentialType) &&
                                            (
                                                    "1".equals(conditionType)
                                                            || ("2".equals(conditionType) && activityTotalAmount.compareTo(minimum) >= 0)
                                                            || ("3".equals(conditionType) && totalQuantity >= minicount)
                                            )
                                    )
                    ) {
                    ...
                    // 保存用过的优惠信息
                    if ("1".equals(tType)) {
                        orderService.saveOrderPreferentialInfo(preferentialType, Integer.valueOf(couponId), orderDtlId, rang, name, productPreAmountDtl, belong, memberId);
                    }
                }
                //获取优惠券的优惠信息
                ...
                belong = isSelfSpopMcht ? "1" : fullCut.getBelong();
                ...
                if ("1".equals(tType)) {
                    orderService.saveOrderPreferentialInfo(preferentialType, Integer.valueOf(preferentialId), orderDtlId, rang, name, productPreAmountDtl, belong, memberId);
                }
                ...
                if (!StringUtil.isBlank(preferentialId)) {
                    ...
                    // 赠送商品ID
                    Integer giftProductId = fullGive.getProductId();
                    // 赠送商品数量
                    ...
                    belong = isSelfSpopMcht ? "1" : fullGive.getBelong();
                    ...
                    if ("1".equals(productFlag)) {
                        activityDiscount += "商品ID--" + giftProductId + ",";
                    }
                    ...
                    if ("1".equals(tType)) {
                        // 每个会场只可以优惠一次
                        if (giveType.equals("1")) {// 满额赠
                            if (sumFlag.equals("0")) {// 非累加
                                // 单价*数量 > 最低消费
                                if (activityTotalAmount.compareTo(minimum) >= 0) {
                                    // 保存满额赠飞累加数据
                                    orderService.saveNonCumulativeInfo(combineOrderId, shoppingCart, giftProductId, memberId, date,
                                            productNum, productFlag, couponFlag, giveCouponId, integralFlag,
                                            giveIntegeral, belong, rang, preferentialType,
                                ...
                                if (activityTotalAmount.compareTo(minimum) >= 0) {
                                    // 保存满额赠累加数据
                                    orderService.saveCumulativeInfo(combineOrderId, shoppingCart, giftProductId, memberId, date,
                                            productNum, productFlag, couponFlag, giveCouponId, integralFlag,
                                            giveIntegeral, activityTotalAmount, minimum, belong, rang, preferentialType,
                                            Integer.valueOf(preferentialId));
                                }
                            }
                        } else if (giveType.equals("2")) {// 买即赠
                            // 买即赠送商品，插入订单明细中
                            orderService.saveNonCumulativeInfo(combineOrderId, shoppingCart, giftProductId, memberId, date,
                                    productNum, productFlag, couponFlag, giveCouponId, integralFlag, giveIntegeral,
                ...
                String name = rule;
                belong = isSelfSpopMcht ? "1" : fullDiscount.getBelong();
                ...
                if (type.equals("1")) {
                    ...
                    String[] rules = rule.split(",");
                    int max = Integer.valueOf(rules[0]);// 满M件
                    double min = Double.valueOf(rules[1]);// N元
                    name = max + "件" + min + "元";
                    activityDiscount = max + "件" + min + "元,";
                    if (productQuantitySum >= max) {// 会场购买件数符合条件满几件
                        // 平摊到每只商品
                        if (shoppingCartAreas.size() == i) {
                            // 最后一次计算
                    ...
                    if (fullDiscountLadder != null) {
                        ...
                        if ((system.equals(Const.ANDROID) && version >= 58) || (system.equals(Const.IOS) && version >= 59)) {
                            ...
                            productPreAmountDtl = saleOrMallPrice.multiply(new BigDecimal("1").subtract(min.divide(new BigDecimal("10")))).setScale(2, BigDecimal.ROUND_HALF_UP);
                        }
                    }
                    ...
                    if ((version > 34 && system.equals(Const.ANDROID)
                        ...
                        name = "第" + min + "件" + discount + "折,";
                        activityDiscount = "第" + min + "件" + discount + "折,";
                    }
                }
                if ("1".equals(tType)) {
                    orderService.saveOrderPreferentialInfo(preferentialType, Integer.valueOf(preferentialId), orderDtlId, rang, name, productPreAmountDtl, belong, memberId);
                }
            } else if (preferentialType.equals("5") && !appContext.olderThan(67, 69)) { //购物津贴
                String rule = allowanceInfo.getRule();
                belong = isSelfSpopMcht ? "1" : allowanceInfo.getBelong();
                ...
                if (allowanceInfo.getUsageBeginTime().before(date) && allowanceInfo.getUsageEndTime().after(date)
                    ...
                    if (shoppingCartAreas.size() == i) {
                        ...
                        productPreAmountDtl = saleOrMallPrice.multiply(totalDiscount).divide(activityTotalAmount, 0, BigDecimal.ROUND_DOWN);
                        totalProductPreAmountDtl = totalProductPreAmountDtl.add(productPreAmountDtl);
                    }
                    ...
                    activityDiscount = sumFlagStr + "满" + rules[0] + "元减 " + rules[1] + "元,";
                    if ("1".equals(tType)) {
                        orderService.saveOrderPreferentialInfo("11", Integer.valueOf(preferentialId), orderDtlId, rang, name, productPreAmountDtl, belong, memberId);
                    }
                }
            }
            BigDecimal platformPreAmount = zero;
            BigDecimal mchtPreAmount = zero;
            if (isSelfSpopMcht) { //spop商家 所有优惠费用归属平台（包括定金抵扣）

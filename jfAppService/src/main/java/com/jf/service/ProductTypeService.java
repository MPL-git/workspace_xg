...
/**
  * @author  chenwf: 
  * @date 创建时间：2017年4月25日 下午6:13:16 
  * @version 1.0 
  * @parameter  
  * @return  
*/
...
public class ProductTypeService extends BaseService<ProductType, ProductTypeExample> {
	...
	public Map<String, Object> getCategoryTypeList20180830(Map<String, Object> paramsMap, String type) {
		List<Map<String,Object>> categoryList = new ArrayList<Map<String,Object>>();
		List<ProductCustom> productCustoms = productService.getShopMallProductTypeList(paramsMap);
		if(CollectionUtil.isNotEmpty(productCustoms)){
			for (ProductCustom productCustom : productCustoms) {
				Map<String, Object> dataMap = new HashMap<String, Object>();
				String id = productCustom.getProductTypeId().toString();
				String name = productCustom.getProductTypeName();
				dataMap.put("productTypeId", id);
				dataMap.put("productTypeName", name);
				categoryList.add(dataMap);
			}

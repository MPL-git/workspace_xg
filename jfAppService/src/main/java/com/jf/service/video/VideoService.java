...
public class VideoService extends AppBaseService<Video, VideoExample> {
    ...
    public VideoCustom getDetailById(Integer videoId) {
    ...
    public List<VideoCustom> findByRelativeProductType(Integer videoId, String videoIds, PageRequest pageRequest) {
        ...
        params.put("offset", 0); //排除已查询过的videoID所以每次查前面不进行翻页
    ...
    @Transactional
    public void getVideoXcxCode(VideoCustom videoCustom) {
        String page = "page/video/pages/full/index";
        String scene = StringUtil.buildMsg("videoid={}", videoCustom.getId());
        String newXcxCode = weixinService.createAndGetAppletUnLimitQrCode(page, scene);
        if (!StringUtils.hasText(newXcxCode)) {
            return;
        }
        videoCustom.setVideoXcxCode(newXcxCode);
        Video updateVideo = new Video();
        updateVideo.setId(videoCustom.getId());
        updateVideo.setVideoXcxCode(videoCustom.getVideoXcxCode());
        updateByPrimaryKeySelective(updateVideo);
    }

...
public class RefundOrderService extends BaseService<RefundOrder, RefundOrderExample> {
	private static Logger logger = LoggerFactory.getLogger(RefundOrderService.class);
	@Resource
	...
	public List<RefundOrder> findList(String orderType, Integer serviceOrderId) {
		...
		criteria.andStatusEqualTo("0").andRefundMethodEqualTo("1").andTryTimesEqualTo(0).andDelFlagEqualTo("0");
		...
		refundOrderExample.setOrderByClause("id");
		return refundOrderMapper.selectByExample(refundOrderExample);
	}
	...
	public void getZfbRefundQuery(RefundOrder refundOrder, CombineOrder combineOrder) {
		try {
			...
			if(response.isSuccess()){
						...
						if(code.equals("10000")){
							if(reqDataJson.containsKey("out_request_no") && !StringUtil.isBlank(reqDataJson.getString("out_request_no"))){
								if(reqDataJson.getString("out_request_no").equals(refundOrder.getRefundReqNo())){
									updateRefundInfo(refundOrder,null);
									logger.info("退款流水--》》："+refundOrder.getRefundReqNo()+"退款成功");
								}
							}
						}
					}
				...
				logger.info("退款流水--》》："+refundOrder.getRefundReqNo()+"退款失败");
			}
	...
	public void getWxRefundQuery(RefundOrder refundOrder, CombineOrder combineOrder) {
		...
		String mchId = "";
		String key = "";
		Integer paymentId = combineOrder.getPaymentId();
		...
		String sign = WXSignUtils.createSign("UTF-8", params,key);
		String xmlInfo = "<xml>"
						+"<appid>"+appId+"</appid>"
						+"<mch_id>"+mchId+"</mch_id>"
			...
			if(result.getReturn_code().equals("SUCCESS")){
				if(result.getResult_code().equals("SUCCESS")){
					if(appId.equals(result.getAppid()) && mchId.equals(result.getMch_id())){
						updateRefundInfo(refundOrder,result);
					}
				}
			}
		}
	...
	private void updateRefundInfo(RefundOrder refundOrder, UnifiedorderResult result) {
		Date date = new Date();
		Date successDate = null;
		if(result != null && !StringUtil.isBlank(result.getRefund_success_time())){
			successDate = DateUtil.getDate(result.getRefund_success_time());
		}else{
			successDate = refundOrder.getCreateDate();
		}
		refundOrder.setStatus("1");
		refundOrder.setSuccessDate(successDate);
		...
		if (list != null && list.size() > 0) {
			...
			for (Element element : list) {
				...
				if(element.getName().length() >=20){
					if ("refund_success_time".equals(element.getName().substring(0, 19))) {
						result.setRefund_success_time(element.getText());
					}
				}

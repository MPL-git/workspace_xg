...
<head>
 ...
 <script type="text/javascript">
	 ...
	 //专员审核报名
	 function activityOperate(statusPage, activityAreaId) {
		$.ligerDialog.open({
			height: $(window).height() - 100,
			width: $(window).width() - 200,
			title: "专员审核报名",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/activityAreaNew/activityManager.shtml?statusPage="+statusPage+"&activityAreaId="+activityAreaId,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
	...
	//专员审核商品
	function showActivityProduct(statusPage, activityAreaId, statusFlag) {
		$.ligerDialog.open({
			height: $(window).height() - 100,
			width: $(window).width() - 200,
			title: "专员审核商品",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/activityProductNew/activityProductManager.shtml?statusPage="+statusPage+"&activityAreaId="+activityAreaId+"&statusFlag="+statusFlag,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
 	 ...
 	 var listConfig={
	      ...
	      listGrid:{ columns: [
	                        ...
	                        {display:'查看/操作',name:'', align:'center', isSort:false, width:260, render:function(rowdata, rowindex) {
								...
								if(rowdata.type == '1') {
									html.push("<a href=\"javascript:activityOperate(1, "+rowdata.id+");\">【审核报名】</a>");
								}
								if(rowdata.type == '2') {
									html.push("<a href=\"javascript:showActivityProduct(1, "+rowdata.id+", 1);\">【审核单品】</a>");
								}
							    return html.join("");
	                        }},
	                        ...
	                        {display:'待审报名数',name:'', align:'center', isSort:false, width:80, render:function(rowdata, rowindex) {
							    return rowdata.trialOperateNum+"/"+rowdata.passOperateNum;
	                        }},
	                        {display:'待审商品数',name:'', align:'center', isSort:false, width:80, render:function(rowdata, rowindex) {
	                        	return rowdata.trialOperateAuditNum+"/"+rowdata.passOperateAuditNum;
	                        }}
			         ], 

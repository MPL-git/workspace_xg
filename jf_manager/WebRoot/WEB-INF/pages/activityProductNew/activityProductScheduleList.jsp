...
<head>
	...
	<script type="text/javascript">
		var viewerPictures;
		$(function() {
			if( $("#cooAuditStatus option:selected").val() !="1"){
				$(".AllPassBtn").hide();
			}
			//监听审核状态(当值不为待审核时，隐藏全部通过按钮)
			$("#cooAuditStatus").change(function(){
		...
		function updateProductItem(id) {
			$.ligerDialog.open({
				...
				title: "商品审核",
				name: "INSERT_WINDOW",
				url: "${pageContext.request.contextPath}/activityProductNew/lawAuditProduct.shtml?id="+id,
				showMax: true,
				showToggle: false,
				showMin: false,
				isResize: false,
		...
		var listConfig={
			url:"/activityProductNew/getActivityProductList.shtml?statusAudit=1&activityId=${activityId }&activityAreaId=${activityAreaId}",
			btnItems:[
					<c:if test="${mchtType=='1'}">

					{text: '批量调价', icon: 'add', click: function() {
			...
			listGrid:{ columns: [
					...
					{display:'吊牌价',name:'', align:'center', isSort:false, width:120, render: function(rowdata, rowindex) {
							var html = [];
					...
					{display:'活动价',name:'', align:'center', isSort:false, width:120, render: function(rowdata, rowindex) {
							...
							if (rowdata.mchtType=='1' && ${statusPage==2}) {
								html.push("<a href=\"javascript:updateProductItem(" + rowdata.productId + ");\">修改</a>");
							}
							return html.join("");
						}},
					...
					{display:'毛利',name:'', align:'center', isSort:false, width:80, render:function(rowdata, rowindex) {
							var html = [];
							html.push(rowdata.grossProfitMin);
							if(rowdata.grossProfitMin != rowdata.grossProfitMax){
								html.push("-");
								html.push(rowdata.grossProfitMax);
							};
							return html.join("");
						}},
							...
							if(rowdata.isGift == 1){
								...
								if(rowdata.operateAuditStatus == '2' ) {
									...
									if(rowdata.productAuditStatus == '3') {
										html.push("<a id='"+ rowdata.id +"-a' href=\"javascript:auditActivityProduct("+ rowdata.id +", 2, 3);\">驳回</a>");
									}else {
										html.push("<a id='"+ rowdata.id +"-a' href=\"javascript:auditActivityProduct("+ rowdata.id +", 2, 2);\">通过</a>");
										html.push("<span style=\"margin: 0px 5px;\">or</span>");
										html.push("<a href=\"javascript:auditActivityProduct("+ rowdata.id +", 2, 3);\">驳回</a>");
									}
									return html.join("");
								}
								return "";
			...
			$.ligerDialog.confirm('是否全部通过？', function (yes) {
				if(yes){
					$.ajax({
						type:"POST",
						dataType:"json",
						url:"${pageContext.request.contextPath}/activityProductNew/batchAuditOperation.shtml?activityId=${activityId }&activityAreaId=${activityAreaId}&statusAudit=1",
...
<div id="toptoolbar"></div>	<form id="dataForm" runat="server">
		...
		<div class="search-tr"  >
			...
			<c:if test="${statusFlag != '1' }">
				<div class="search-td-search-btn" style="display: inline-flex;">
					<div id="searchbtn" onClick="searchList();" style="width: 30px;">搜索</div>
					<div style="padding-left: 10px;" class="AllPassBtn">

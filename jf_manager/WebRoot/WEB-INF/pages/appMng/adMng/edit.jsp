...
var v;
$(function ()  {
	var linkType = '${adInfoCustom.linkType }';
	var linkId = '${adInfoCustom.linkId }';
	var linkUrl = '${adInfoCustom.linkUrl }';
	...
	if(linkType != '') {
		if(linkType == '3' || linkType == '4' || linkType == '5'|| linkType == '10' || linkType == '13' 
				|| linkType == '15' || linkType == '17' || linkType == '19' || linkType == '21' 
				|| linkType == '23' || linkType == '25' || linkType == '27') {
	...
	<c:if test="${mngPos == 1}">
		var showChannelArr = '${adInfoCustom.showChannel }'.split(",");
		console.log(showChannelArr);
		for (i = 0; i < showChannelArr.length; i++) {
			$("#checkShowChannel_"+showChannelArr[i]).attr("checked", "true");
		}
	</c:if>
...
function changeTwoType(titleName){
	...
	html.push('</select>');
	html.push('<select name="linkUrl" id="linkUrl"  multiple="multiple" size="5" style="margin-top:15px;margin-left:10px;width:190px;">');
...
function radioItem(linkType, linkId) {
	switch (linkType) {
		case '1':
			...
			$("#show1-2").html('<input type="text" id="linkId" name="linkId" value="' +linkId+ '" placeholder="请输入会场ID" validate="{digits:true,maxlength:9}"/>');
			break;
		case '2':
			$("#show1-1").html('活动ID<span class="required">*</span>');
			$("#show1-2").html('<input type="text" id="linkId" name="linkId" value="' +linkId+ '" placeholder="请输入活动ID" validate="{digits:true,maxlength:9}"/>');
			break;
		case '3':
			$("#show1-1").html('商品ID<span class="required">*</span>');
			$("#show1-2").html('<input type="text" id="linkUrl" name="linkUrl" value="' +linkId+ '" placeholder="请输入商品ID" validate="{digits:true,maxlength:12}"/>');
			break;
		case '4':
			$("#show1-1").html('URL<span class="required">*</span>');
			$("#show1-2").html('<input type="text" id="linkUrl" name="linkUrl" value="' +linkId+ '" placeholder="请输入URL连接" validate="{maxlength:120}"/>');
			break;
		case '5':
			$("#show1-1").html('URL');
			$("#show1-2").html('<input type="text" id="linkUrl" name="linkUrl" placeholder="无连接"  value="' +linkId+ '" />');
			break;
		case '6':
			if(linkId == '')
				linkId = 1;
		...
		case '7':
			...
			$("#show1-2").html('<input type="text" id="linkId" name="linkId" value="' +linkId+ '" placeholder="请输入自建页面ID"  validate="{digits:true,maxlength:9}"/>');
			break;
		case '8':
			$("#show1-1").html('淘宝优选关键字<span class="required">*</span>');
			$("#show1-2").html('<input type="text" id="linkId" name="linkId"  value="' +linkId+ '"   placeholder="请输入淘宝优惠关键字" validate="{digits:true,maxlength:9}"/>');
		  	break;
		case '9':
			$("#show1-1").html('<span>新品牌团<span class="required">*</span></span>');
			linkIStr = '<select name="linkId" id="linkId" style="width: 160px;">';
		...
		case '10':
			...
			$("#show1-2").html('<input type="text" id="linkUrl" name="linkUrl"  value="' +linkId+ '" placeholder="请输入商家序号" validate="{maxlength:120}" />');
		  	break;
		case '11':
		...
		case '15':
			...
			$("#show1-2").html('<input type="text" id="linkUrl" name="linkUrl" value="' +linkId+ '"  placeholder="请输入有好货品牌ID" validate="{maxlength:120}"/>');
		  	break;
		case '16':
		...
		case '17':
			...
			$("#show1-2").html('<input type="text" id="linkUrl" name="linkUrl" value="' +linkId+ '"  placeholder="潮鞋上新品牌ID" validate="{maxlength:120}"/>');
		  	break;
		case '18':
		...
		case '19':
			...
			$("#show1-2").html('<input type="text" id="linkUrl" name="linkUrl" value="' +linkId+ '"  placeholder="潮流男装品牌ID"  validate="{maxlength:120}"/>');
		  	break;
		case '20':
		...
		case '21':
			...
			$("#show1-2").html('<input type="text" id="linkUrl" name="linkUrl" value="' +linkId+ '" placeholder="断码特惠品牌ID" validate="{maxlength:120}"/>');
		  	break;
		case '22':
		...
		case '23':
			...
			$("#show1-2").html('<input type="text" id="linkUrl" name="linkUrl" value="' +linkId+ '"  placeholder="运动鞋服品牌ID" validate="{maxlength:120}"/>');
		  	break;
		case '24':
		...
		case '25':
			...
			$("#show1-2").html('<input type="text" id="linkUrl" name="linkUrl" value="' +linkId+ '"  placeholder="潮鞋美妆品牌ID" validate="{maxlength:120}"/>');
		  	break;
		case '26':
		...
		case '27':
			...
			$("#show1-2").html('<input type="text" id="linkUrl" name="linkUrl" value="' +linkId+ '"  placeholder="食品超市品牌ID" validate="{maxlength:120}"/>');
		  	break;
		case '28':
			$("#show1-1").html('<span>商城一级分类<span class="required">*</span></span>');
			linkIStr = '<select name="linkId" id="linkId" style="width: 160px;"><option value="">请选择</option>';
		...
		case '29':
			...
			$("#show1-2").html('<input type="text" id="linkId" name="linkId" value="' +linkId+ '"  placeholder="请输入优惠券ID" validate="{maxlength:120}"/>');
		  	break;
	}
}
...
<body>
	<form method="post" id="form1" name="form1" action="${pageContext.request.contextPath}/appMng/adMng/saveSubmit.shtml">
		...
		<table class="gridtable">
		     <c:if test="${mngPos == 1002 || mngPos == 1004 || mngPos == 1007 || mngPos == 1032 }">
		       <tr>
					...
					<td colspan="5" align="left" class="l-table-edit-td">
						<select style="width: 160px;" id="sourceProductTypeId" name="sourceProductTypeId" validate="{required:true}"  >
							...
							<c:forEach var="sourceProductTypes" items="${sourceProductTypes}">
							   <option value="${sourceProductTypes.id}" <c:if test="${sourceProductTypes.id==adInfoCustom.sourceProductTypeId}">selected="selected"</c:if>>${sourceProductTypes.sourceProductTypeName}</option>
							</c:forEach>
						</select>
					</td>
				</tr>
	...
	<%-- 		<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
				<select name="linkType" id="linkType" style="width: 160px;">
					<c:forEach var="typeItem" items="${linkTypes }">
						<c:if test="${mngPos == 2 and typeItem.statusValue != 6 }">
						<option  value="${typeItem.statusValue }" <c:if test="${typeItem.statusValue==adInfoCustom.linkType}">selected</c:if> >${typeItem.statusDesc}</option>
							<label><span class="radioClass"><input class="radioItem" onClick="radioItem(this.value, '')" type="radio" value="${typeItem.statusValue }" name="linkType" <c:if test="${typeItem.statusValue==adInfoCustom.linkType}">checked="checked"</c:if>>${typeItem.statusDesc}</span></label>
						</c:if>
					...
					<td colspan="5" align="left" class="l-table-edit-td">
						<select style="width: 160px;" id="catalog" name="catalog">
							<c:forEach var="catalogItem" items="${catalogs}">
								<c:if test="${catalogItem.statusValue!=1}">
									<option value="${catalogItem.statusValue}" <c:if test="${catalogItem.statusValue==adInfoCustom.catalog}">selected="selected"</c:if>>${catalogItem.statusDesc}</option>
								</c:if>
							</c:forEach>
						</select>
					</td>
					...
					<td colspan="5" align="left" class="l-table-edit-td">
						...
						<c:forEach var="positionItem" items="${positions}">
							<c:if test="${positionItem.statusValue==1 || positionItem.statusValue==2 || positionItem.statusValue==3}">
								<option value="${positionItem.statusValue}" <c:if test="${positionItem.statusValue==adInfoCustom.position}">selected="selected"</c:if>>${positionItem.statusDesc}</option>
							</c:if>
						</c:forEach>
						</select>
					</td>
			...
			<c:if test="${mngPos == 10}">
               ...
               <td align="left" class="l-table-edit-td" >
               		<div><img id="iosPicImg" style="width: 300px;height: 150px" alt="" src="${pageContext.request.contextPath}/file_servelt${adInfoCustom.iosPic}"></div>
	    			<div style="float: left;height: 30px;">
	    			...
	    			<input id="iosPic" name="iosPic" type="hidden" value="${adInfoCustom.iosPic}">
               </td>
            </tr>
		   	<tr>
               <td  class="title" width="20%">安卓广告图片：</td>
               <td align="left" class="l-table-edit-td" >
               		<div><img id="androidPicImg" style="width: 300px;height: 150px" alt="" src="${pageContext.request.contextPath}/file_servelt${adInfoCustom.androidPic}"></div>
	    			<div style="float: left;height: 30px;">
	    			...
	    			<input id="androidPic" name="androidPic" type="hidden" value="${adInfoCustom.androidPic}">
               </td>
            </tr>
		   </c:if>
			<c:if test="${mngPos == 1}">
				<tr>
					...
					<td colspan="5" align="left" class="l-table-edit-td">
						<label><input type="checkbox" id="checkShowChannel_1" name="checkShowChannel" value="1"/>APP端</label>&nbsp;&nbsp;
						<label><input type="checkbox" id="checkShowChannel_2" name="checkShowChannel" value="2"/>小程序</label>&nbsp;&nbsp;
						<label><input type="checkbox" id="checkShowChannel_3" name="checkShowChannel" value="3"/>H5&其他</label>
			...
			<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					<input type="text" id="autoUpDate" name="autoUpDate" value="<fmt:formatDate value='${adInfoCustom.autoUpDate}' pattern='yyyy-MM-dd HH:mm'/>"/>
				</td>
			</tr>
			...
			<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					<input type="text" id="autoDownDate" name="autoDownDate" value="<fmt:formatDate value='${adInfoCustom.autoDownDate}' pattern='yyyy-MM-dd HH:mm'/>"/>
				</td>
			</tr>

...
<script type="text/javascript">
		...
		$.ajax({
			...
			success : function(data) {
				if ("0000" == data.returnCode) {
					$("#serviceStatus-"+id).attr("style", "color:green;");
					$("#serviceStatus-"+id).html("处理中");
	...
	function editUpdateBy(id){
		  $.ligerDialog.open({
			...
			title: "批量变更",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/appealOrder/editUpdateByManager.shtml?ids="+id,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
	...
	var listConfig = {
		...
		listGrid : {
			columns : [{ display: '投诉编号', width: 160, render: function(rowdata, rowindex) {
						var html = [];
					...
					},
					{
						...
						render : function(rowdata, rowindex) {
							if (rowdata.activityId) {
								return rowdata.activityName + "<br>" + "["
										+ rowdata.activityId + "]";
							}
						}
					},
					...
					},
					{
						...
						render : function(rowdata, rowindex) {
							if (rowdata.staffName) {
								...
								return "<input type='button' style='cursor: pointer;width: 60px;height: 35px;-moz-border-radius: 2px;-webkit-border-radius: 2px;border-radius:6px;border:1px solid #ddd;background-color:#269abc;border-color: #269abc;' onclick='savePlatformProcessor("
										+ rowdata.id
										+ ",this)' value='领取'></input>";
							}
						}
					},
					{
						display : '操作',
						render : function(rowdata, rowindex) {
							return "<a href='javascript:;' onclick='viewAppealOrder("
									+ rowdata.id + ");'>查看</a>";
						}
					} ],
		...
		<div class="search-pannel">
				...
				<div class="search-td">
					...
					<div class="l-panel-search-item">
						<select id="appealType" name="appealType" style="width: 150px;">
							...
							<c:forEach var="appealOrderAppealType"
								items="${appealOrderAppealTypeList}">
				...
				<div class="search-td">
					...
					<div class="l-panel-search-item">
						<select id="status" name="status" style="width: 150px;">
							<option value="">请选择</option>
							<c:forEach var="appealOrderStatus"
								items="${appealOrderStatusList}">
				...
				<div class="search-td">
					...
					<div class="l-panel-search-item">
						<select id="serviceStatus" name="serviceStatus"
							style="width: 150px;">
							<option value="">请选择</option>
							<c:forEach var="appealOrderServiceStatus"
								items="${appealOrderServiceStatusList}">
								<option value="${appealOrderServiceStatus.statusValue}">${appealOrderServiceStatus.statusDesc}</option>
							</c:forEach>
						</select>
					</div>
				</div>
			...
			<div class="search-tr">
				<div class="search-td">
					...
					<div class="l-panel-search-item">
						<select id="platformProcessor" name="platformProcessor"
							style="width: 150px;">
							<option value="">请选择</option>
							<option value="unclaimed">未领取</option>
							<c:forEach var="platformProcessor"
								items="${platformProcessorList}">
								<option value="${platformProcessor.platform_processor}">${platformProcessor.platform_processor_name}</option>
							</c:forEach>
						</select>
					</div>
				</div>
			...
			<div class="search-tr">
				...
				<div class="search-td">
					<div class="search-td-label" style="color: red;">商品ID：</div>

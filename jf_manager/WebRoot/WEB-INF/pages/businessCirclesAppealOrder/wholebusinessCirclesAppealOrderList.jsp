 ...
 <script type="text/javascript">
     ...
     function changeStaff(id) {
         ...
         $.ligerDialog.open({
             ...
             title: "变更处理人",
             name: "INSERT_WINDOW",
             url: "${pageContext.request.contextPath}/businessCirclesAppealOrder/changeStaffDialog.shtml?id=" + id,
             showMax: true,
             showToggle: false,
             showMin: false,
             isResize: true,
 	 ...
 	 var listConfig={ 
	      url:"/businessCirclesAppealOrder/wholebusinessCirclesAppealOrderdata.shtml",
	      listGrid:{ columns: [
						  ...
						  {display:'会员ID',name:'memberIds', align:'center', isSort:false, width:100,render:function(rowdata, rowindex){
							  var html = [];
						  ...
						  {display:'相关类型单号',name:'', align:'center', isSort:false, width:200,render:function(rowdata, rowindex){
							   ...
							   if (rowdata.customerServiceOrderCount>0 || rowdata.interventionOrderCount>0 || rowdata.appealOrderCount>0 || rowdata.subOrderCustomscuont>0) {
								  html.push("<a href=\"javascript:subOrderCustomscuont(" + rowdata.id + ");\">子订单(<span>未完成"+rowdata.subOrderCustomscuont+"</span>)</a><br>"); 
					      ...
					      {display:'客服处理人',name:'customerservicehandlerName', align:'center', isSort:false, width:200,render:function(rowdata, rowindex){
					    	        var html = [];
					    	        if (rowdata.customerservicehandlerName!=null && rowdata.customerservicehandlerName!='') {						
						        	    html.push(rowdata.customerservicehandlerName);
						        	    html.push("<a href=\"javascript:changeStaff(" + rowdata.id + ");\">【变更】</a>");
									}
						        	return html.join("");
					      }},
...
<body style="padding: 0px; overflow: hidden;">
	...
	<form id="dataForm" runat="server" >
		<div class="search-pannel">
			...
			<div class="search-tr"  >
				<div class="search-td">
					<div class="search-td-label" >登记编号：</div>
					<div class="search-td-combobox-condition" >
			  ...
			  <div class="search-td">
				...
				<select id="createby" name="createby" class="querysel">
					...
					<c:forEach var="createbyList" items="${createbyList}">
					   <option value="${createbyList.create_by}">${createbyList.staff_name}</option>
				   </c:forEach>					
				</select>
				</div>
		 	  </div>	  			

...
<style type="text/css">
 ...
 <script type="text/javascript">
	 ...
	 function updateIsShow(commentId, isShow) {
 	 ...
 	 var listConfig={ 
	      url:"/comment/getMchtCommentInfoList.shtml?mchtId=${mchtId }",
	      btnItems:[],
	      listGrid:{ columns: [
							...
							{display:'隐藏状态',name:'isShowDesc', align:'center', isSort:false, width:80}, 
							{display:'操作',name:'', align:'center', isSort:false, width:120, render: function(rowdata, rowindex) {
								var html = [];
								html.push("<a href='javascript:;' onclick='viewDetail(" + rowdata.subOrderId + ");'>【查看订单】</a></br>");
								if(rowdata.isShow == '1') {
									html.push("<a href='javascript:;' onclick='updateIsShow("+rowdata.id+", 0)'>【隐藏】</a>");
								}else {
									html.push("<a href='javascript:;' onclick='updateIsShow("+rowdata.id+", 1)'>【显示】</a>");
								}
							    return html.join("");
							}}
			         ], 
...
<body style="padding: 0px; overflow: hidden;">
	...
	<div class="search-tr"  >
		<div class="search-td">
			<h2>${mchtInfo.shopName }的评价信息</h2>
		</div>
	</div>
	<div class="search-tr"  >
		<div class="search-td">
			<div class="search-td-label" >商家描述：</div>
			<div class="search-td-combobox-condition" >
				<c:if test="${avgProductScore == 'null' }">--</c:if>
				<c:if test="${avgProductScore != 'null' }">
					<span style='color: red;'>${avgProductScore }</span>分
				</c:if>
			</div>
		</div>
		<div class="search-td">
			<div class="search-td-label" >卖家服务：</div>
			<div class="search-td-combobox-condition" >
				<c:if test="${avgMchtScore == 'null' }">--</c:if>
				<c:if test="${avgMchtScore != 'null' }">
					<span style='color: red;'>${avgMchtScore }</span>分
				</c:if>
			</div>
		</div>
		<div class="search-td">
			<div class="search-td-label" >物流服务：</div>
			<div class="search-td-combobox-condition" >
				<c:if test="${avgWuliuScore == 'null' }">--</c:if>
				<c:if test="${avgWuliuScore != 'null' }">
					<span style='color: red;'>${avgWuliuScore }</span>分
				</c:if>
			</div>
		</div>
	</div>
	<form id="dataForm" runat="server" >
		<div class="search-pannel">
			...
			<div class="search-tr"  >
				...
				<div class="search-td" style="width: 27%;margin-left:42px;">
			 	...
			 	<div class="search-td-search-btn" style="width:7%;">

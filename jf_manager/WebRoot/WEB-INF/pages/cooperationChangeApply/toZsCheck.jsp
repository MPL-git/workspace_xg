...
<script type="text/javascript">
    ...
    KindEditor.ready(function(K) {
        editor1 = K.create('textarea[name="zsAuditRemarks"]', {
    ...
    $(function(){
        ...
        // $("input[name='zsAuditStatus']").bind('click',function(){
        // 	var zsAuditStatus = $(this).val();
        // 	if(zsAuditStatus == 1){
        // 		$("#zsRejectReasonTr").show();
        // 	}else if(zsAuditStatus == 2){
        // 		$("#zsRejectReasonTr").show();
        // 	}
        // });
        ...
        $("#confirm").bind('click',function(){
            ...
            editor1.sync('zsAuditRemarks');//将内容输入到文本框
            var zsAuditRemarks = $("#zsAuditRemarks").val().trim();
            if(zsAuditRemarks=='' && zsAuditStatus == 2){
            ...
            if(!shopName){
                commUtil.alertError("请输入变更后的店铺名");
                return;
            }
            </c:if>
            ...
            if(!productTypeId){
                commUtil.alertError("请输入变更后的主营类目");
                return;
            }
            </c:if>
            ...
            if(!deposit){
                commUtil.alertError("请输入变更后的保证金");
                return;
            }
            </c:if>
            var rateCheck = true;
            ...
            if(!rateCheck){
                commUtil.alertError(title);
                return;
            }
            </c:if>
            var mchtBrandRateChangeJsonStr = JSON.stringify(mchtBrandRateChangeArray);
            $.ajax({
                url : "${pageContext.request.contextPath}/cooperationChangeApply/zsCheck.shtml",
                type : 'POST',
                dataType : 'json',
                cache : false,
                async : false,
                data : {"id":id,"shopType":shopType,"businessFirms":businessFirms,"brand":brand,"productType":productType,"shopName":shopName,
                    "productTypeId":productTypeId,"deposit":deposit,"mchtBrandRateChangeJsonStr":mchtBrandRateChangeJsonStr,"zsAuditStatus":zsAuditStatus,
                    "zsAuditRemarks":zsAuditRemarks
                },
                timeout : 30000,
...
<div class="title-top">
	...
	<c:if test="${not empty shopNameAuth}">
		...
		<table class="gridtable">
			...
			<tr>
				...
				<td>
					<span id="newShopName">${cooperationChangeApply.shopName}</span>
					<a href="javascript:;" onclick="editShopInfo(1);">【查看】</a>
					<c:if test="${isView == 0}">
						<a href="javascript:;" onclick="editShopInfo(0);">【修改】</a>
					</c:if>
				</td>
			</tr>
		</table>
		...
		<table class="gridtable">
			...
			<tr>
				...
				<td>
					...
					<c:if test="${isView == 0}">
						<a href="javascript:;" onclick="editProductType();">【修改】</a>
					</c:if>
				</td>
			</tr>
		</table>
		...
		<table class="gridtable">
			...
			<tr>
				...
				<td>
					...
					<c:if test="${isView == 0}">
						<a href="javascript:;" onclick="editDeposit();">【修改】</a>
					</c:if>
				</td>
			</tr>
		</table>
		...
		<table class="gridtable">
			...
			<c:forEach items="${mchtProductBrands}" var="mchtProductBrand">
				<tr>
					...
					<%--<td><fmt:formatNumber type="number" pattern="0.00" maxFractionDigits="2" value="${mchtProductBrand.popCommissionRate}"></fmt:formatNumber></td>--%>
	...
	<br>
	<div>
		<span class="table-title">招商审核结果</span>
	</div>
	<table class="gridtable">
		<tr>
			<td colspan="1">招商确认结果</td>
			<td>
				<input type="radio" name="zsAuditStatus" value="1" <c:if test="${cooperationChangeApply.zsAuditStatus eq 1}">checked="checked"</c:if>>通过
				<input type="radio" name="zsAuditStatus" value="2" <c:if test="${cooperationChangeApply.zsAuditStatus eq 2}">checked="checked"</c:if>>驳回
			</td>
		</tr>
		<tr id="zsRejectReasonTr" >
			<td colspan="1">备注/驳回理由<span style="color: red;">*</span></td>
			<td>
				<textarea rows="10" cols="80" id="zsAuditRemarks" name="zsAuditRemarks" maxlength="256"></textarea>
			</td>
		</tr>
		<tr>
			<td colspan="1">操作</td>
		...
		<form action="" id="shopInfoForm">
			<table class="gridtable">
				...
				<tr id="opBtnTr">
					...
					<td>
						<input id="confirmShopInfo" type="button" style="float:left;" class="l-button l-button-test" value="确定"/>
					</td>
				</tr>
			</table>
		</form>
...
<div class="video_box" style="position:fixed; width:700px; height:250px; top:20%; left:35%; display: none;" id="productTypeDiv">
	<a href="javascript:;" class="video_close"><img src="${pageContext.request.contextPath}/images/close.png"></a>
	<div class="panel panel-default" style="margin-bottom:0px;">
		<div class="modal-header">
			<h3 class="modal-title" id="productTypeTitle">
				主营类目修改
			</h3>
		</div>
		<table class="gridtable">
			...
			<tr>
				<td class="title">操作</td>
				<td>
					<input id="confirmProductType" type="button" style="float:left;" class="l-button l-button-test" value="确定"/>
				</td>
			</tr>
		</table>
	</div>
...
<div class="video_box" style="position:fixed; width:700px; height:250px; top:20%; left:35%; display: none;" id="depositDiv">
	<a href="javascript:;" class="video_close"><img src="${pageContext.request.contextPath}/images/close.png"></a>
	<div class="panel panel-default" style="margin-bottom:0px;">
		<div class="modal-header">
			<h3 class="modal-title" id="depositTitle">
				保证金修改
			</h3>
		</div>
		<table class="gridtable">
			<tr>
				...
				<td colspan="2" >
					<input type="text" id="changeDeposit" value="${cooperationChangeApply.deposit}">
				</td>
			</tr>
			<tr>
				<td class="title">操作</td>
				<td>
					<input id="confirmDeposit" type="button" style="float:left;" class="l-button l-button-test" value="确定"/>
				</td>
			</tr>
		</table>
	</div>

...
<head>
 ...
 <script type="text/javascript">
	...
	//砍价方案模版
	function cutPriceCnfTplManager() {
		$.ligerDialog.open({
			height: $(window).height() - 100,
			width: $(window).width() - 400,
			title: "砍价方案模版",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/cutPriceProduct/cutPriceCnfTplManager.shtml",
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
	...
	function cutPriceCnfManager(singleProductActivityId) {
		$.ligerDialog.open({
			...
			title: "砍价设置",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/cutPriceProduct/addOrUpdateCutPriceCnfManager.shtml?singleProductActivityId="+singleProductActivityId,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
	...
	function updateAuditStatusManager(singleProductActivityId) {
		$.ligerDialog.open({
			...
			title: "审核",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/cutPriceProduct/updateAuditStatusManager.shtml?singleProductActivityId="+singleProductActivityId,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
	...
	function updateStatus(singleProductActivityId, status) {
 	...
 	var listConfig={
	      url:"/cutPriceProduct/getCutPriceProductList.shtml",
	      btnItems:[
	  		      { text: '砍价方案模版', icon:'modify', id:'modify', dtype:'win', click:cutPriceCnfTplManager }
	  	      ],
	      listGrid:{ columns: [
							{display:'砍价ID',name:'id', align:'center', isSort:false, width:80},
	                        ...
	                        {display:'实际领取人数',name:'cjCutPriceOrderCount', align:'center', isSort:false, width:80},
	                        ...
	                        {display:'操作',name:'', align:'center', isSort:false, width:200, render: function(rowdata, rowindex) {
								var html = [];
								html.push("<a href=\"javascript:cutPriceCnfManager(" + rowdata.id + ");\">【砍价设置】</a>");

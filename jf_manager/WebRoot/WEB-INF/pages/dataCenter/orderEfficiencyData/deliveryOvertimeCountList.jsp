...
<head>
 ...
 function overtimeNotDeliveryList(mchtCode,payDateBegin,payDateEnd) {
	 $.ligerDialog.open({
			height: $(window).height()*0.95,
			width: $(window).width()*0.95,
			title: "超时未发货订单",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/orderEfficiencyData/overtimeNotDelivery/list.shtml?mchtCode="+mchtCode+"&payDateBegin="+payDateBegin+"&payDateEnd="+payDateEnd,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
 ...
 function deliveryOvertimeList(mchtCode,payDateBegin,payDateEnd) {
	 $.ligerDialog.open({
			height: $(window).height()*0.95,
			width: $(window).width()*0.95,
			title: "发货超时订单",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/orderEfficiencyData/deliveryOvertime/list.shtml?mchtCode="+mchtCode+"&payDateBegin="+payDateBegin+"&payDateEnd="+payDateEnd,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
 ...
 var listConfig={
     url:"/orderEfficiencyData/deliveryOvertimeCount/data.shtml",
     listGrid:{ columns: [
			            ...
			            { display: '超时总数', width: 190,render: function (rowdata, rowindex) {
							return '<span>'+rowdata.overtime_order_count+'</span>';	                		
		                }},
		                { display: '超时比例', width: 190,render: function (rowdata, rowindex) {
							return rowdata.rate;	                		
		                }},
		                { display: '操作', width: 190,render: function (rowdata, rowindex) {
		                	var payDateBegin = $("#payDateBegin").val();
		                	var payDateEnd = $("#payDateEnd").val();
							return '<a href="javascript:;" onclick="overtimeNotDeliveryList('+"'"+rowdata.mcht_code+"'"+','+"'"+payDateBegin+"'"+','+"'"+payDateEnd+"'"+');">查看未发货</a>&nbsp;&nbsp;<a href="javascript:;" onclick="deliveryOvertimeList('+"'"+rowdata.mcht_code+"'"+','+"'"+payDateBegin+"'"+','+"'"+payDateEnd+"'"+');">查看发货超时</a>';	                		
		                }}
		                ],

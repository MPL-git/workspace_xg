...
<head>
 ...
 function falseOrderList(mchtCode,payDateBegin,payDateEnd) {
	 $.ligerDialog.open({
			height: $(window).height()*0.95,
			width: $(window).width()*0.95,
			title: "虚假发货订单",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/orderEfficiencyData/falseOrder/list.shtml?mchtCode="+mchtCode+"&payDateBegin="+payDateBegin+"&payDateEnd="+payDateEnd,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
 ...
 var listConfig={
     url:"/orderEfficiencyData/falseOrderCount/data.shtml",
     listGrid:{ columns: [
			            ...
			            { display: '发货订单总数', width: 190,render: function (rowdata, rowindex) {
							return '<span>'+rowdata.delivery_order_count+'</span>';	                		
		                }},
			            { display: '虚发订单总数', width: 190,render: function (rowdata, rowindex) {
							return '<span>'+rowdata.false_order_count+'</span>';	                		
		                }},
		                { display: '虚发比例', width: 190,render: function (rowdata, rowindex) {
							return rowdata.rate;	                		
		                }},
		                { display: '操作', width: 190,render: function (rowdata, rowindex) {
		                	var payDateBegin = $("#payDateBegin").val();
		                	var payDateEnd = $("#payDateEnd").val();
							return '<a href="javascript:;" onclick="falseOrderList('+"'"+rowdata.mcht_code+"'"+','+"'"+payDateBegin+"'"+','+"'"+payDateEnd+"'"+');">查看</a>';	                		
		                }}
		                ],

...
<head>
 ...
 var listConfig={
     url:"/orderEfficiencyData/falseOrder/data.shtml",
     listGrid:{ columns: [
						{ display: '发货时间',width: 200, render: function (rowdata, rowindex) {
							if(rowdata.deliveryDate){
								return new Date(rowdata.deliveryDate).format("yyyy-MM-dd hh:mm:ss");	                		
							}
		                }},
						{ display: '承诺发货时间',width: 200, render: function (rowdata, rowindex) {
							if(rowdata.deliveryLastDate){
								return new Date(rowdata.deliveryLastDate).format("yyyy-MM-dd hh:mm:ss");	                		
							}
		                }},
						{ display: '子订单号',width: 200, render: function (rowdata, rowindex) {
							return '<a href="javascript:;" onclick="viewDetail('+rowdata.id+')">'+rowdata.subOrderCode+'</a>';	                		
		                }},
			            ...
			            { display: '虚发跟单状态', width: 80,render: function (rowdata, rowindex) {
			            	if(rowdata.shamFollowStatusDesc){
								return rowdata.shamFollowStatusDesc;	                		
			            	}else{
								return "未处理";	                		
			            	}
		                }},

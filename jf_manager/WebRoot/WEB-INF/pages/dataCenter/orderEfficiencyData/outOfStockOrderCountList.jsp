...
<head>
 ...
 function outOfStockOrderList(mchtCode,payDateBegin,payDateEnd) {
	 $.ligerDialog.open({
			height: $(window).height()*0.95,
			width: $(window).width()*0.95,
			title: "缺货订单",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/orderEfficiencyData/outOfStockOrder/list.shtml?mchtCode="+mchtCode+"&payDateBegin="+payDateBegin+"&payDateEnd="+payDateEnd,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
 ...
 var listConfig={
     url:"/orderEfficiencyData/outOfStockOrderCount/data.shtml",
     listGrid:{ columns: [
			            ...
			            { display: '缺货订单总数', width: 190,render: function (rowdata, rowindex) {
							return '<span>'+rowdata.out_of_stock_order_count+'</span>';	                		
		                }},
		                { display: '缺货比例', width: 190,render: function (rowdata, rowindex) {
							return rowdata.rate;	                		
		                }},
		                { display: '操作', width: 190,render: function (rowdata, rowindex) {
		                	var payDateBegin = $("#payDateBegin").val();
		                	var payDateEnd = $("#payDateEnd").val();
							return '<a href="javascript:;" onclick="outOfStockOrderList('+"'"+rowdata.mcht_code+"'"+','+"'"+payDateBegin+"'"+','+"'"+payDateEnd+"'"+');">查看</a>';	                		
		                }}
		                ],

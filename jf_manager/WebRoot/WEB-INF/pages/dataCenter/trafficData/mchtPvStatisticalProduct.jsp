...
<script type="text/javascript" src="${pageContext.request.contextPath}/common/js/utils/util.js"></script>
 ...
 <script type="text/javascript">
 	...
 	$(function() {
 	 ...
 	 var listConfig={ 
		      url: "/mchtPvStatistical/mchtPvStatisticalProductList.shtml",
		      btnItems: [],
		      listGrid: { columns: [
...
<body style="padding: 0px; overflow: hidden;" >
	<p class="p-title">
		<span class="span-title1" >历史商品概况</span>
		<span class="span-title2" >（${mchtInfo.shopName }-${mchtInfo.mchtCode }）</span>
	</p>
	<form id="dataForm1" action="${pageContext.request.contextPath}/mchtPvStatistical/mchtPvStatisticalProduct.shtml" method="post" >
		<div class="search-pannel" >
			<input type="hidden" name="mchtId" value="${mchtInfo.id }" />
			<div class="search-tr"  > 
				<div class="search-td">
	...
	<div id="dataShow" >
		<div class="div-tab">
			<table class="tab-2">
				...
				<tr>
					<td>${flowMchtProductPvMap.total_visitor_count_a }</td>
					<td>${flowMchtProductPvMap.total_visitor_count_tourist_a }</td>
					<td>${flowMchtProductPvMap.total_pv_count_a }</td>
					<td>${flowMchtProductPvMap.total_pv_count_tourist_a }</td>
					<td>${flowMchtProductPvMap.total_product_count_a }</td>
				</tr>
				<tr>
					<td>
						较对比日
						<c:if test="${flowMchtProductPvMap.total_visitor_count_rate < 0 }">
							<span style="margin-left: 10px; color: #66cc66;">
						</c:if>
						<c:if test="${flowMchtProductPvMap.total_visitor_count_rate > 0 }">
							<span style="margin-left: 10px; color: #ff6666;">
						</c:if>
						<c:if test="${empty flowMchtProductPvMap.total_visitor_count_rate or flowMchtProductPvMap.total_visitor_count_rate == '0.00' }">
           					<span style="margin-left: 10px;">
           				</c:if>
						${flowMchtProductPvMap.total_visitor_count_rate }%</span>
					</td>
					<td>
						较对比日
						<c:if test="${flowMchtProductPvMap.total_visitor_count_rate_tourist < 0 }">
							<span style="margin-left: 10px; color: #66cc66;">
						</c:if>
						<c:if test="${flowMchtProductPvMap.total_visitor_count_rate_tourist > 0 }">
							<span style="margin-left: 10px; color: #ff6666;">
						</c:if>
						<c:if test="${empty flowMchtProductPvMap.total_visitor_count_rate_tourist or flowMchtProductPvMap.total_visitor_count_rate_tourist == '0.00' }">
           					<span style="margin-left: 10px;">
           				</c:if>
						${flowMchtProductPvMap.total_visitor_count_rate_tourist }%</span>
					</td>
					<td>
						较对比日
						<c:if test="${flowMchtProductPvMap.total_pv_count_rate < 0 }">
							<span style="margin-left: 10px; color: #66cc66;">
						</c:if>
						<c:if test="${flowMchtProductPvMap.total_pv_count_rate > 0 }">
							<span style="margin-left: 10px; color: #ff6666;">
						</c:if>
						<c:if test="${empty flowMchtProductPvMap.total_pv_count_rate or flowMchtProductPvMap.total_pv_count_rate == '0.00' }">
           					<span style="margin-left: 10px;">
           				</c:if>
						${flowMchtProductPvMap.total_pv_count_rate }%</span>
					</td>
					<td>
						较对比日
						<c:if test="${flowMchtProductPvMap.total_pv_count_rate_tourist < 0 }">
							<span style="margin-left: 10px; color: #66cc66;">
						</c:if>
						<c:if test="${flowMchtProductPvMap.total_pv_count_rate_tourist > 0 }">
							<span style="margin-left: 10px; color: #ff6666;">
						</c:if>
						<c:if test="${empty flowMchtProductPvMap.total_pv_count_rate_tourist or flowMchtProductPvMap.total_pv_count_rate_tourist == '0.00' }">
           					<span style="margin-left: 10px;">
           				</c:if>
						${flowMchtProductPvMap.total_pv_count_rate_tourist }%</span>
					</td>
					<td>
						较对比日
						<c:if test="${flowMchtProductPvMap.total_product_count_rate < 0 }">
							<span style="margin-left: 10px; color: #66cc66;">
						</c:if>
						<c:if test="${flowMchtProductPvMap.total_product_count_rate > 0 }">
							<span style="margin-left: 10px; color: #ff6666;">
						</c:if>
						<c:if test="${empty flowMchtProductPvMap.total_product_count_rate or flowMchtProductPvMap.total_product_count_rate == '0.00' }">
           					<span style="margin-left: 10px;">
           				</c:if>
						${flowMchtProductPvMap.total_product_count_rate }%</span>
					</td>
				</tr>
			</table>
		</div>
		<div class="div-tab">
			<table class="tab-2">
				...
				<tr>
					<td>${mchtProductPvMap.member_remind_count_a }</td>
					<td>${mchtProductPvMap.shopping_cart_quantity_sum_a }</td>
					<td>${mchtProductPvMap.order_dtl_quantity_sum_a }</td>
					<td>${mchtProductPvMap.pay_product_sum_a }</td>
					<td></td>
				</tr>
				<tr>
					<td>
						较对比日
						<c:if test="${mchtProductPvMap.member_remind_count_rate < 0 }">
							<span style="margin-left: 10px; color: #66cc66;">
						</c:if>
						<c:if test="${mchtProductPvMap.member_remind_count_rate > 0 }">
							<span style="margin-left: 10px; color: #ff6666;">
						</c:if>
						<c:if test="${empty mchtProductPvMap.member_remind_count_rate or mchtProductPvMap.member_remind_count_rate == '0.00' }">
           					<span style="margin-left: 10px;">
           				</c:if>
						${mchtProductPvMap.member_remind_count_rate }%</span>
					</td>
					<td>
						较对比日
						<c:if test="${mchtProductPvMap.shopping_cart_quantity_sum_rate < 0 }">
							<span style="margin-left: 10px; color: #66cc66;">
						</c:if>
						<c:if test="${mchtProductPvMap.shopping_cart_quantity_sum_rate > 0 }">
							<span style="margin-left: 10px; color: #ff6666;">
						</c:if>
						<c:if test="${empty mchtProductPvMap.shopping_cart_quantity_sum_rate or mchtProductPvMap.shopping_cart_quantity_sum_rate == '0.00' }">
           					<span style="margin-left: 10px;">
           				</c:if>
						${mchtProductPvMap.shopping_cart_quantity_sum_rate }%</span>
					</td>
					<td>
						较对比日
						<c:if test="${mchtProductPvMap.order_dtl_quantity_sum_rate < 0 }">
							<span style="margin-left: 10px; color: #66cc66;">
						</c:if>
						<c:if test="${mchtProductPvMap.order_dtl_quantity_sum_rate > 0 }">
							<span style="margin-left: 10px; color: #ff6666;">
						</c:if>
						<c:if test="${empty mchtProductPvMap.order_dtl_quantity_sum_rate or mchtProductPvMap.order_dtl_quantity_sum_rate == '0.00' }">
           					<span style="margin-left: 10px;">
           				</c:if>
						${mchtProductPvMap.order_dtl_quantity_sum_rate }%</span>
					</td>
					<td>
						较对比日
						<c:if test="${mchtProductPvMap.pay_product_sum_rate < 0 }">
							<span style="margin-left: 10px; color: #66cc66;">
						</c:if>
						<c:if test="${mchtProductPvMap.pay_product_sum_rate > 0 }">
							<span style="margin-left: 10px; color: #ff6666;">
						</c:if>
						<c:if test="${empty mchtProductPvMap.pay_product_sum_rate or mchtProductPvMap.pay_product_sum_rate == '0.00' }">
           					<span style="margin-left: 10px;">
           				</c:if>
						${mchtProductPvMap.pay_product_sum_rate }%</span>
					</td>
					<td></td>
				</tr>
			</table>

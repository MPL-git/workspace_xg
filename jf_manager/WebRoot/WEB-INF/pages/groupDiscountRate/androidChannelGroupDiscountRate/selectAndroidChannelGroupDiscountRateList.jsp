 ...
 <script type="text/javascript">
	 ...
	 function updateDiscountRate(id) {
		...
		if(discountRate != '' && flag != null) {
			...
			$.ajax({
				 type : 'POST',
				 url : "${pageContext.request.contextPath}/androidChannelGroupDiscountRate/updateDiscountRate.shtml",
	 ...
	 function updateDiscountRateBatch() {
		 $.ligerDialog.open({
			...
			title: "批量修改优惠率",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/androidChannelGroupDiscountRate/updateDiscountRateBatchManager.shtml",
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
	 ...
	 function addAndroidChannelGroupDiscountRate() {
		 $.ligerDialog.open({
			 height: 400,
			 width: 600,
			 title: "新增渠道优惠率",
			 name: "INSERT_WINDOW",
			 url: "${pageContext.request.contextPath}/androidChannelGroupDiscountRate/addAndroidChannelGroupDiscountRateManager.shtml",
			 showMax: true,
			 showToggle: false,
			 showMin: false,
			 isResize: true,
 	 ...
 	 var listConfig={
	      url:"/androidChannelGroupDiscountRate/selectAndroidChannelGroupDiscountRateList.shtml",
		  btnItems:[{text:'添加渠道优惠率', icon:'add', click: function() {
		  		addAndroidChannelGroupDiscountRate();
		   }}
		  ],
		  pageSize: 10,
	      listGrid:{ columns: [
						...
						{display:'渠道集合ID',name:'groupId', align:'center', isSort:false, width:180},
						{display:'渠道集合名称',name:'groupName', align:'center', isSort:false, width:180},
						{display:'渠道集合优惠率',name:'discountRate', align:'center', isSort:false, width:180, render:function(rowdata, rowindex) {
							var html = [];
						...
						{display:'渠道包前缀',name:'groupPrefix', align:'center', isSort:false, width:180},
			...
			<div class="search-tr"  >
				...
				<div class="search-td">
					...
					<div class="search-td-combobox-condition" >
						<input type="text" id="androidChannelGroupId" name="androidChannelGroupId" >
					</div>
				</div>
				<div class="search-td">
					...
					<div class="search-td-combobox-condition" >
						<select id="groupNameId" name="groupNameId" style="width: 135px;" >
							...
							<c:forEach var="androidChannelGroup" items="${androidChannelGroupList }">
								<option value="${androidChannelGroup.id }">${androidChannelGroup.groupName }</option>
							</c:forEach>
						</select>
				 	 </div>
				</div>

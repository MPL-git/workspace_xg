...
<script type="text/javascript">
	...
	//查看评价
	function showIsComment(interventionOrderId) {
		$.ligerDialog.open({
			height : 800,
			width : 800,
			title : "评价详情",
			name : "INSERT_WINDOW",
			url : "${pageContext.request.contextPath}/interventionOrder/showIsComment.shtml?interventionOrderId="
					+ interventionOrderId,
			showMax : true,
			showToggle : false,
			showMin : false,
	...
	//子框口调用
	function updateIsInitiateViolateFun(interventionOrderId, updateDate,
			isInitiateViolate) {
		$("#updateDate-" + interventionOrderId).html(updateDate);
		if (isInitiateViolate == '0') {
			$("#isInitiateViolate-" + interventionOrderId).html("未发起");
			$("#isInitiateViolateShow-" + interventionOrderId).html(
					"<a href=\"javascript:showInterventionOrder("
							+ interventionOrderId + ", 6);\">查看</a>");
		} else {
			$("#isInitiateViolate-" + interventionOrderId).html("已发起");
			$("#isInitiateViolateShow-" + interventionOrderId).html(
					"<a href=\"javascript:showInterventionOrder("
							+ interventionOrderId + ", 1);\">查看</a>");
		}
	}
	function updateIsCommentFun(interventionOrderId) {
		$("#isComment-" + interventionOrderId).html("");
	}
	...
	var listConfig = {
		url : "/interventionOrder/getInterventionOrderList.shtml?statusFlag=8",
		btnItems : [],
		listGrid : {
			columns : [
					{display:'申请介入编号', name:'interventionCode', align:'center', isSort:false, width:180, render: function (rowdata, rowindex) {
						var html = [];
		            ...
		            }},
					{
						...
						render : function(rowdata, rowindex) {
							return "<a href=\"javascript:viewAfterServiceDetail("
									+ rowdata.serviceOrderId
									+ ",'"
									+ rowdata.customerServiceTypeDesc
									+ "');\">"
									+ rowdata.customerServiceOrderCode + "</a>";
						}
					},
					{
						display : '申请人',
					...
					{
						display : '判定内容 / 驳回理由',
						name : 'sumOperateAudit',
						align : 'center',
						isSort : false,
						width : 200,
						render : function(rowdata, rowindex) {
							var html = [];
							if (rowdata.mchtResultReasonDesc != null
									&& rowdata.mchtResultReasonDesc != '') {
								html.push("商家：" + rowdata.mchtResultReasonDesc
										+ "<br/>");
							}
							if (rowdata.clientResultReasonDesc != null
									&& rowdata.clientResultReasonDesc != '') {
								html.push("买家："
										+ rowdata.clientResultReasonDesc);
							}
							return html.join("");
					...
					},
					{
						...
						render : function(rowdata, rowindex) {
							if (rowdata.updateDate != null
								...
								return "<span id='updateDate-"+rowdata.id+"'>"
										+ updateDate
												.format("yyyy-MM-dd hh:mm:ss")
										+ "</span>";
							} else {
					...
					},
					{
						...
						width : 100,
						render : function(rowdata, rowindex) {
							if (rowdata.isInitiateViolate == '1') {
								return "<span id='isInitiateViolate-"+rowdata.id+"'>已发起</span>";
							} else {
								return "<span id='isInitiateViolate-"+rowdata.id+"'>未发起</span>";
							}
						}
					},
					{
						display : '处理人',
						name : 'platformProcessorName',
						align : 'center',
						isSort : false,
						width : 100
					},
					{
						...
						render : function(rowdata, rowindex) {
							...
							if (rowdata.isInitiateViolate == '1') {
								html
										.push("<span id='isInitiateViolateShow-"+rowdata.id+"' style='margin:5px 5px;'><a href=\"javascript:showInterventionOrder("
												+ rowdata.id
												+ ", 1);\">查看</a></span>");
							} else {
								html
							...
							if (rowdata.isComment != '1') {
								html
										.push("<span id='isComment-"+rowdata.id+"' style='margin:5px 5px;'><a href=\"javascript:showIsComment("
												+ rowdata.id
												+ ");\">评价</a></span>");
							} else {
								html
										.push("<span id='isComment-"+rowdata.id+"' style='margin:5px 5px;'></span>");
							}
							return html.join("");
						}
					} ],
...
<body style="padding: 0px; overflow: hidden;">
	...
	<form id="dataForm" runat="server">
		<div class="search-pannel">
			<div class="search-tr">
				...
				<div class="search-td">
					...
					<div class="search-td-combobox-condition">
						<select id="platformProcessorId" name="platformProcessorId" style="width: 135px;">
							<option value="">请选择...</option>
							<c:forEach var="platformProcessor" items="${platformProcessorList }">
								<option value="${platformProcessor.platform_processor }">
									${platformProcessor.platform_processor_name }</option>
							</c:forEach>
						</select>
					</div>
				</div>
		...
		<div class="search-pannel">
			<div class="search-tr">
				<div class="search-td">
					<div class="search-td-label">发起违规状态：</div>
					<div class="search-td-combobox-condition">
						<select id="isInitiateViolate" name="isInitiateViolate" style="width: 135px;">
							<option value="">请选择...</option>
		...
		<div class="search-pannel">
			<div class="search-tr">
				<div class="search-td">
					<div class="search-td-label">评价状态：</div>
					<div class="search-td-combobox-condition">
						<select id="isComment" name="isComment" style="width: 135px;">
							<option value="">请选择...</option>

...
<script type="text/javascript">
	...
	function submitFun(interventionOrderId) {
		var winType = $("[name='winType']:checked").val();
		var clientResultReason = $("#clientResultReason").val();
		var mchtResultReason = $("#mchtResultReason").val();
		var platformRemarks = $("#platformRemarks").val();
		$.ajax({
			type: 'post',
			url: '${pageContext.request.contextPath}/interventionOrder/updateInterventionOrder.shtml?statusFlag=4',
			data: {interventionOrderId:interventionOrderId, winType:winType, clientResultReason:clientResultReason, mchtResultReason:mchtResultReason, platformRemarks:platformRemarks},
			dataType: 'json',
			success: function(data) {
				if(data == null || data.code != 200) {
					commUtil.alertError(data.msg);
...
<body>
	...
	<div class="title-top">
		...
		<div class="marT10">
			<span class="marL20"><input type="radio" name="winType" value="2" >商家胜诉</span>
			<span class="marL20"><input type="radio" name="winType" value="1" checked="checked" >买家胜诉</span>
		</div>
		<table class="gridtable marT10 status-table">
           	<tr>
               <td class="title title-width">
               		<span>买家<span class="required">*</span></span>
               </td>
               <td>
               		<select id="clientResultReason" name="clientResultReason" style="width: 50%;">
						<c:forEach var="clientResultReason" items="${clientResultReasonList }">
							<option id="clientResultReason-${clientResultReason.statusValue }" value="${clientResultReason.statusValue }" <c:if test="${interventionOrderCustom.clientResultReason == clientResultReason.statusValue}">selected</c:if> > 
           	...
           	<tr>
               <td class="title title-width">
               		<span>商家<span class="required">*</span></span>
               </td>
               <td>
               		<select id="mchtResultReason" name="mchtResultReason" style="width: 50%;">
						<c:forEach var="mchtResultReason" items="${mchtResultReasonList }">
							<option id="mchtResultReason-${mchtResultReason.statusValue }" value="${mchtResultReason.statusValue }" <c:if test="${interventionOrderCustom.mchtResultReason == mchtResultReason.statusValue}">selected</c:if> > 
               ...
               <td>
               		<textarea rows="5" cols="100" id="platformRemarks" name="platformRemarks" >${interventionOrderCustom.platformRemarks }</textarea>
               </td>
			</tr>
        </table>
	</div>
	...
	<div style="margin: 20 auto; text-align: center">
		<button style="cursor: pointer;width: 150px;height: 35px;background-color: rgba(22, 155, 213, 1);border: none;color: white;" onclick="submitFun(${interventionOrderCustom.id });" >
			提交
		</button>
	</div>	

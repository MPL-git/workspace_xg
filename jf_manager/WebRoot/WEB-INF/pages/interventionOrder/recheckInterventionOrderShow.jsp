...
<script type="text/javascript">
	...
	$(function(){
		...
		$("[name='directorStatus']").bind("click",function() {
			 var directorStatus = $("[name='directorStatus']:checked").val();
			 if(directorStatus == '1') {
				 $(".directorStatus-table").show();
			 }else {
				 $(".directorStatus-table").hide();
			 }
		});
	...
	function submitFun(interventionOrderId) {
		var directorStatus = $("[name='directorStatus']:checked").val();
		var directorReason = $("#directorReason").val();
		var directorRemarks = $("#directorRemarks").val();
		if(directorStatus == '1') {
			directorReason = $("#directorReason").val();
			directorRemarks = $("#directorRemarks").val();
		}
		if(directorStatus == '1' && directorReason == '') {
			commUtil.alertError('理由不能为空！');
		}else {
			$.ajax({
				type: 'post',
				url: '${pageContext.request.contextPath}/interventionOrder/updateInterventionOrder.shtml?statusFlag=6',
				data: {interventionOrderId:interventionOrderId, directorStatus:directorStatus, directorReason:directorReason, directorRemarks:directorRemarks},
				dataType: 'json',
				success: function(data) {
					if(data == null || data.code != 200) {
						commUtil.alertError(data.msg);
...
<body>
	...
	<div class="title-top" >
		...
		<div class="marT10">
			<span class="marL20"><input type="radio" name="directorStatus" value="3" checked="checked" >同意驳回</span>
			<span class="marL20"><input type="radio" name="directorStatus" value="1" >不同意驳回</span>
		</div>
		<table class="gridtable marT10 directorStatus-table" style="display: none;">
           	<tr>
               <td class="title title-width">
               		<span>理由<span class="required">*</span></span>
               </td>
               <td>
               		<textarea rows="5" cols="100" id="directorReason" name="directorReason" >${interventionOrderCustom.directorReason }</textarea>
               </td>
			</tr>
           	<tr>
               <td class="title title-width">
               ...
               <td>
               		<textarea rows="5" cols="100" id="directorRemarks" name="directorRemarks" >${interventionOrderCustom.directorRemarks }</textarea>
               </td>
			</tr>
        </table>
	</div>

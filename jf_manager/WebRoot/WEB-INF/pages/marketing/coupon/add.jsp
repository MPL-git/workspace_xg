...
$(function ()  {
	...
	var v = $("#form1").validate({
		...
		{   
			...
			var isSupportCouponRain=$("input[name='isSupportCouponRain']:checked").val();
...
/* 			var Vf=/^[0-9]*[1-9][0-9]*$/;
            ...
            if (preferentialType=='2' &&  $.trim(maxDiscountMoney.value)=="") {
                commUtil.alertError("折扣最多优惠多少元不可为空!");
                isValidateOk=false;
            }else if (preferentialType=='2' &&  $.trim(maxDiscountMoney.value)!="") {
				if (!str.test(maxDiscountMoney.value)) {
                    commUtil.alertError("折扣最多优惠多少元必须是 整数。");
                    isValidateOk=false;
				}
			}
		 ...
		 if (!ver.test(minimum1)) {
            ...
            if (days=='1') {
             ...
             if (beginDate1.getTime()<recBeginDate1.getTime() || endDate1.getTime()>=recEndDate1.getTime()) {
            	 commUtil.alertError("追加日期范围必须大于或等于在领取开始日期，领取结束日期-1之间，不可超出此日期范围！");
             	 isValidateOk=false;
			  }
...
function types(value){
	if (value=='1') {
		...
		$("#discount").val("");
		$("#conditionType").val("");
		minimum1 = $("input[name='minimum1']").val("");
		/* $("#minimum1").val(""); */
		$("#minicount").val("");
		$("#maxDiscountMoney").val("");
	}else if (value=='2') {
...
<html>
<body>
	<form method="post" id="form1" name="form1" action="${pageContext.request.contextPath}/marketing/coupon/saveSubmit.shtml">
		...
		<table class="gridtable">
			<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					<input id="name" validate="{required:true,maxlength:128}" name="name" type="text" style="width:200px;" value=""/>
			...
			<tr>
				...
				<td id="mmu" colspan="5" align="left" class="l-table-edit-td">
					面额&nbsp;<input id="money" validate="{maxlength:8}" name="money" type="text" style="width:60px;" maxlength="6" value=""/>元
					&nbsp;&nbsp;
					满&nbsp;<input id="minimum" validate="{maxlength:8}" name="minimum" type="text" style="width:60px;" value=""/>元
				</td>
				<td id="dcm" colspan="5" align="left" class="l-table-edit-td">
					...
					&nbsp;&nbsp;
					最多优惠<input id="maxDiscountMoney" name="maxDiscountMoney"  maxlength="8"  type="text" style="width:60px;" value=""/>元
			...
			<%-- <tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					...
					<div id="canSuperposeTr">是否叠加使用品类券<input type="radio" name="canSuperpose" value="1">是<input type="radio" name="canSuperpose" value="0" checked="checked">否</div>
				</td>
			...
			<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					<c:forEach var="typeItem" items="${expiryTypes}">
						<span class="radioClass"><input class="radioItem" onchange="clearanceTime()" type="radio" value="${typeItem.statusValue }" name="expiryType" <c:if test="${typeItem.statusValue==1}">checked="checked"</c:if> >${typeItem.statusDesc}</span>
					</c:forEach>
				</td>
			</tr>
			...
			<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					<input id="grantQuantity" validate="{required:true,digits:true}" name="grantQuantity" type="text" style="width:60px;" maxlength="6" value=""/>张
			...
			<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					<c:forEach var="typeItem" items="${recTypes }">
					   <c:if test="${typeItem.statusValue ne '4'}">
						<span class="radioClass"><input class="radioItem" type="radio" value="${typeItem.statusValue }" name="recType" <c:if test="${typeItem.statusValue==1}">checked="checked"</c:if>>${typeItem.statusDesc}</span>
					   </c:if>
					</c:forEach>
					<span id="integral"><input id="needIntegral" name="needIntegral" type="text" style="width:60px;" value="" validate="{digits:true}"/>金币换1张优惠券</span>
				</td>
			</tr>
			...
			<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					<span class="radioClass"><input class="radioItem" type="radio" value="3" name="recLimitType" checked="checked">不限</span>
					<span class="radioClass"><input class="radioItem" type="radio" value="1" name="recLimitType">每人每天限领1张</span>

...
$(function ()  {
	...
	if (${couponCustom.status}!=0){
		...
		$("input[name='preferentialType']").attr("disabled","true");
		$("input[name='couponType']").attr("disabled","true");
		$("input[name='productTypeId']").attr("disabled","true");
		$("input[name='money']").attr("disabled","true");
		$("input[name='minimum']").attr("disabled","true");
		$("input[name='minimum1']").attr("disabled","true");
		$("input[name='minicount']").attr("disabled","true");
		$("input[name='discount']").attr("disabled","true");
		$("select[name='conditionType']").attr("disabled","true");
		$("input[name='maxDiscountMoney']").attr("disabled","true");
	}
	...
	var v = $("#form1").validate({
		...
		{   
			...
			if (preferentialType=='2' &&  $.trim(maxDiscountMoney.value)!="") {
				...
				$("#maxDiscountMoney").ligerTip({ content: "请重新输入折扣最多优惠多少元。"});
				...
				if ((linkType=='1' || linkType=='2' || linkType=='3' || linkType=='4' || linkType=='7') && linkValue=='') {
					...
					isValidateOk=false;
			            ...
			            if (recBeginDate1.getTime() > expiryBeginDate1.getTime()) {
			            	 commUtil.alertError("有效日期不能小于领取日期！");
			             	 isValidateOk=false;
						} */
    ...
    if(!isEmpty(${couponCustom.maxDiscountMoney})){
...
<html>
<body>
	<form method="post" id="form1" name="form1" action="${pageContext.request.contextPath}/marketing/coupon/editSubmit.shtml">
		...
		<table class="gridtable">
			<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					<input id="name" validate="{required:true,maxlength:128}" name="name" type="text" style="width:200px;" value="${couponCustom.name }"/>
			...
			<%-- <tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					<input id="money" validate="{required:true,number:true,maxlength:8}" name="money" type="text" style="width:60px;" maxlength="6" value="${couponCustom.money }"/>元
				</td>
			</tr>
			...
			<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					满<input id="minimum" validate="{required:true,number:true,maxlength:8}" name="minimum" type="text" style="width:60px;" value="${couponCustom.minimum }"/>元使用
				</td>
			</tr> --%>
			<tr>
			...
			<tr>
			 <td colspan="1" class="title">折扣<span class="required">*</span></td>
				<td id="mmu" colspan="5" align="left" class="l-table-edit-td">
					面额&nbsp;<input id="money"  name="money" type="text" style="width:60px;" maxlength="11" value="${couponCustom.money}"/>元
					&nbsp;&nbsp;
					满&nbsp;<input id="minimum" validate="{maxlength:11}" name="minimum" type="text" style="width:60px;"  value="${couponCustom.minimum}"/>元
				</td>
			...
			<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					<c:forEach var="typeItem" items="${expiryTypes }">
						<span class="radioClass"><input class="radioItem1" type="radio" value="${typeItem.statusValue }" name="expiryType" <c:if test="${typeItem.statusValue==couponCustom.expiryType}">checked="checked"</c:if> >${typeItem.statusDesc}</span>
					</c:forEach>
				</td>
			</tr>
			...
			<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					<input id="grantQuantity" validate="{grantQuantityRequired:true,digits:true}" name="grantQuantity" type="text" style="width:60px;" maxlength="6" value="${couponCustom.grantQuantity }"/>张

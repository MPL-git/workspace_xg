...
<script type="text/javascript">
	...
	function preview(decorateInfoId) {
		$.ligerDialog.open({
			...
			title: "预览",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/customAdPage/marketingPage/preview.shtml?decorateInfoId="+decorateInfoId,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
	...
	function design(decorateInfoId,pageType) {
		$.ligerDialog.open({
			height: $(window).height()*0.9,
			width: $(window).width()*0.9,
			title: "装修",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/customAdPage/marketingPage/design.shtml?decorateInfoId=" + decorateInfoId +"&pageType=" + pageType,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
	...
	var listConfig = {
		      ...
		      listGrid:{ columns: [
		        				...
		        				{ display:'主题信息操作', align:'center', isSort:false, width:180, render:function(rowdata, rowindex) {
		        				    return "<a href=\"javascript:design(" + rowdata.decorateInfoId +","+ rowdata.pageType +  ");\">装修</a>";
		        				}}
				         ], 
			...
			<div class="search-pannel">
				<div class="search-tr"  >
					<div class="search-td" style="position: relative;" >
						<div>
							<c:if test="${pageType =='4'}">现金签到</c:if>
							<c:if test="${pageType =='5'}">砍价免费拿</c:if>
							<c:if test="${pageType =='6'}">购物车</c:if>
							<c:if test="${pageType =='7'}">消息</c:if>
							<c:if test="${pageType =='8'}">商品未上架</c:if>
							<c:if test="${pageType =='9'}">邀请享免单</c:if>
							<c:if test="${pageType =='21'}">醒购店长权益介绍</c:if>

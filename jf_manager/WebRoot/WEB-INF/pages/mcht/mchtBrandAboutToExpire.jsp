...
<script src="${pageContext.request.contextPath}/js/viewer.min.js"
    ...
    $(function(){
     ...
     function viewerMchtPic(mchtProductBrandId,picType){
     	...
     	var date = new Date();
     	var afterThirtyDays = new Date(date);
     	afterThirtyDays.setDate(date.getDate() + 30);
	    var listConfig={
	    		  
	    	      url:"/mcht/mchtBrandAboutToExpireList.shtml",
	    	      ...
	    	      listGrid:{ columns: [
		    			          { display: '品牌开通日期', name: 'createDate', width:180,render:function(rowdata,rowindex){
		    			           ...
		    			           { display: '店铺名称', name: 'shopName',width:180},
	    			                { display: '公司/经营信息', name: 'productBrandName',width:100 , render: function(rowdata, rowindex){
					  					var html = [];
					  					 html.push("<a href=\"javascript:editMchtBaseInfo(" + rowdata.mchtId + ");\">查看</a>"); 
					  					return html.join("");
				  					}},
	    			                { display: '品牌资质归档情况', name: 'archiveStatus',width:180 , render: function(rowdata, rowindex){
					  					var html = ["["+ rowdata.productBrandName +"]"];
					  					 html.push("<a href=\"javascript:viewMchtProductBrandPics(" + rowdata.id + ");\">查看</a>"); 
					  					return html.join("");
				  					}},
	    			                { display: '品牌授权到期日期',  name: 'platformAuthExpDate', width: 120,render:function(rowdata,rowindex){
			    			        	  if (rowdata.platformAuthExpDate!=null) {
			    			                	if(date.getTime()<=Number(rowdata.platformAuthExpDate) && Number(rowdata.platformAuthExpDate)<=afterThirtyDays.getTime()){
	    			                ...
	    			                { display: '其它许可证有效期', name: 'otherExpDate', width: 120 ,render:function(rowdata,rowindex){
	    			                	if (rowdata.otherExpDate!=null) {
		    			                	if(date.getTime()<=Number(rowdata.otherExpDate) && Number(rowdata.otherExpDate)<=afterThirtyDays.getTime()){
	    			                ...
	    			                { display: '品牌运营状态',  name: 'statusDesc', width: 100}, 

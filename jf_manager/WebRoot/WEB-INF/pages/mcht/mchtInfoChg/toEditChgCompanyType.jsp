...
$(function(){
	$("#confirm").bind('click',function(){
		...
		submitting = true;
		$.ajax({
			url : "${pageContext.request.contextPath}/mcht/mchtInfoChg/saveCompanyType.shtml",
			type : 'POST',
			dataType : 'json',
			cache : false,
			async : false,
			data : {"id":id,"companyType":companyType},
			timeout : 30000,
...
<body>
	...
	<input type="hidden" id="mchtChgId" value="${id}">
		<table class="gridtable">
          	<tr>
          		<td class="title"><c:if test="${settledType eq 1}">公司类型</c:if><c:if test="${settledType eq 2}">组织形式</c:if></td>
          		<td>
          			<input type="text" id="companyType" name="companyType" value="${companyType}">
          		</td>
          	</tr>
	        <tr>

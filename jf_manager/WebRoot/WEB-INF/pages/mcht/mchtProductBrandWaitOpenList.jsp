...
<script src="${pageContext.request.contextPath}/js/viewer.min.js"
    ...
    $(function(){
     ...
     function open(mchtProductBrandId) {
    		$.ligerDialog.open({
    			height: $(window).height() - 100,
    			width: $(window).width() - 200,
	    		title: "开通",
	    		name: "INSERT_WINDOW",
	    		url: "${pageContext.request.contextPath}/mcht/toOpenMchtProductBrand.shtml?mchtProductBrandId=" + mchtProductBrandId,
	    		showMax: true,
	    		showToggle: false,
	    		showMin: false,
	    		isResize: true,
     ...
     function viewerMchtPic(mchtProductBrandId,picType){
	    ...
	    var listConfig={
	    	      ...
	    	      url:"/mcht/mchtProductBrandWaitOpenData.shtml",
	    	      
	    	      listGrid:{ columns: [
		    			          { display: '创建日期', name: 'zsCommitAuditDate', width:180,render:function(rowdata,rowindex){
					              ...
					              { display: '公司名称',  name: 'companyName', width: 150 , render: function(rowdata, rowindex){
	    			                ...
	    			                { display: '品牌信息',  name: 'viewBrand', width: 150 , render: function(rowdata, rowindex){
					  					var html = [];
	    			                ...
	    			                { display: '授权有效期', name: 'platformAuthExpDate', width: 180 ,render:function(rowdata,rowindex){
    				              	...
    				              	{ display: "操作", name: "op",width:100 , render: function(rowdata, rowindex) {
    				              	  if (rowdata.yyStaffId==${sessionScope.USER_SESSION.staffID} || rowdata.assistantContact=='2') {
    		 		                	  return "<a href=\"javascript:open(" + rowdata.id + ");\">【开通】</a>";										
									    }
    				              	}}
	    			                ],   

...
<head>
 ...
 function toOperate(id){
	 $.ajax({
			url : "${pageContext.request.contextPath}/mcht/checkZsMchtPlatformContact.shtml",
			type : 'POST',
			dataType : 'json',
			cache : false,
			async : false,
			...
			success : function(data) {
				if ("0000" == data.returnCode) {
					$.ligerDialog.open({
						...
						title: "开通",
						name: "INSERT_WINDOW",
						url: "${pageContext.request.contextPath}/mcht/toOperate.shtml?mchtId=" + id,
						showMax: true,
						showToggle: false,
						showMin: false,
						isResize: true,
...
// 		              	}},
		                ...
		                { display: '保证金状态',  name: 'id', width: 150, render: function(rowdata, rowindex) {
 		                ...
 		                { display: "商管运营", name: "yy_contact_name",width:100 , render: function(rowdata, rowindex) {
 		                	if((rowdata.yy_contact_name==null||rowdata.yy_contact_name=="")&&${isCanGet}){
 		                		return "<a href=\"javascript:getAudit(" + rowdata.id + ");\">领取</a>";
 		                	}else{
 		                		return rowdata.yy_contact_name;
 		                	}
		              	}},
		              	{ display: '开通状态', name: 'operateStatus',width:100 , render: function(rowdata, rowindex) {
 		                	if(!rowdata.operateStatus){
 		                		return "待开通";
 		                	}else if(rowdata.operateStatus == 0){
 		                		return "关闭";
 		                	}else{
 		                		return "开通";
 		                	}
		              	}}, 
		              	{ display: '开通备注', name: 'operate_remarks',width:100 },
		              	{ display: "操作", name: "op",width:100 , render: function(rowdata, rowindex) {
		              		if(${isCanGet}){
	 		                	return "<a href=\"javascript:toOperate(" + rowdata.id + ");\">开通</a>";
		              		}
		              	}}
		                ],   
     ...
     container:{
      ...
      function getAudit(id){
    	  	...
    	  	$.ligerDialog.confirm('确认领取？', function (yes) {
    			if(yes){
    				$.ajax({
    					url : "${pageContext.request.contextPath}/mcht/getAuditYY.shtml",
    					type : 'POST',
    					dataType : 'json',
    					cache : false,
    					async : false,

...
$(function(){
	...
	$("#confirm").bind('click',function(){
		...
		if(!status){
			commUtil.alertError("请选择开通结果");
			return;
		}
		var remarks = $("#remarks").val().trim();
		submitting = true;
		$.ajax({
			url : "${pageContext.request.contextPath}/mcht/openMchtProductBrand.shtml",
			type : 'POST',
			dataType : 'json',
			cache : false,
			async : false,
			data : {"mchtProductBrandId":mchtProductBrandId,"status":status,"remarks":remarks},
			timeout : 30000,
...
<body>
	...
	<input type="hidden" id="mchtProductBrandId" value="${mchtProductBrandId}">
		<table class="gridtable">
          	<tr>
          		<td class="title">开通结果<span style="color: red;">*</span></td>
          		<td>
          			<input type="radio" name="status" value="1">正常<input type="radio" name="status" value="4">驳回申请
          		</td>
          	</tr>
          	<tr>
          		<td class="title">我的备注/驳回理由</td>
          		<td>

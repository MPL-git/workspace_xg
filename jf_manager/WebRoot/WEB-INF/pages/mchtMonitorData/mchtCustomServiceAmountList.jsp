...
function customServiceOrderAmountCount(mchtCode,payDateBegin,payDateEnd,productBrandId) {
	 $.ligerDialog.open({
			height: $(window).height()*0.95,
			width: $(window).width()*0.95,
			title: "每日售后金额统计",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/mchtMonitorData/customServiceOrderAmountCount.shtml?mchtCode="+mchtCode+"&payDateBegin="+payDateBegin+"&payDateEnd="+payDateEnd+"&productBrandId="+productBrandId,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
 ...
 var listConfig={
     url:"/mchtMonitorData/mchtCustomServiceAmountData.shtml",
     listGrid:{ columns: [
			            ...
			            { display: '支付商品金额',width:80,render: function (rowdata, rowindex) {
			           		return '<span>'+rowdata.pay_product_amount+'</span>';
		                }},
		                { display: '未发货商品金额',width:100, render: function (rowdata, rowindex) {
		               		return '<span>'+rowdata.delivery_product_amount+'</span>';
		                }},
		                { display: '退款商品金额',width:80, render: function (rowdata, rowindex) {
		                	return '<span>'+rowdata.refund_amount+'</span>';
		                }},
		                { display: '退款率',width:70, render: function (rowdata, rowindex) {
							return rowdata.refund_rate;
		                }},
		                { display: '退货商品金额',width:80, render: function (rowdata, rowindex) {
		                	return '<span>'+rowdata.return_goods_amount+'</span>';
		                }},
		                { display: '退货率',width:70, render: function (rowdata, rowindex) {
							return rowdata.refund_goods_rate;
		                }},
		                { display: '换货商品金额',width:80, render: function (rowdata, rowindex) {
		                	return '<span>'+rowdata.exchange_goods_amount+'</span>';
		                }},
		                ...
		                { display: '操作',width:60, render: function (rowdata, rowindex) {
							...
							return '<a href="javascript:;" onclick="customServiceOrderAmountCount('+"'"+rowdata.mcht_code+"'"+','+"'"+payDateBegin+"'"+','+"'"+payDateEnd+"'"+','+productBrandId+');">查看</a>';
		                }}
		                ],

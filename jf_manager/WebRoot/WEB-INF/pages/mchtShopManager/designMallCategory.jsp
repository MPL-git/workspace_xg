...
$(function(){
	$("input[name='editDecorateModule']").live('click',function(){
		...
		$.ligerDialog.open({
			...
			title: title,
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/mchtShopManager/mallCategory/editDecorateModule.shtml?decorateAreaId=" + decorateAreaId+"&moduleType="+moduleType+"&seqNo="+seqNo+"&decorateModuleId="+decorateModuleId+"&decorateInfoId="+decorateInfoId,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
		...
		<div class="search-pannel" style="overflow-y: auto;">
			<div class="search-tr"  > 
				<div style="display: inline-flex;">
					...
					<div style="padding-left: 10px;">
						<input type="button" style="width: 50px;height: 30px;cursor: pointer;" value="预览" onclick="preview(${decorateInfoId});">
					</div>
				</div>
			</div>

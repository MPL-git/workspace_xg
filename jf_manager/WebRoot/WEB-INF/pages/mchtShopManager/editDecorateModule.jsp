...
<script type="text/javascript">
	...
	function openMapInfo(_this) {
		...
		var linkValueHtml;
		if(!linkType){
			linkTypeHtml = '<select name="linkType" id="linkType" onchange="setLinkValueHtml(this)"><option value="">请选择</option><option value="7">商城一级页面</option><option value="8">商城二级列表ID</option><option value="1">会场ID</option><option value="2">特卖ID</option><option value="3">商品ID</option><option value="4">自建页面ID</option></select>';
			linkValueHtml = '<input id="linkValue" name="linkValue" type="text" style="height:35px;" value="">';
		}else if(linkType == "7"){
			linkTypeHtml = '<select name="linkType" id="linkType" onchange="setLinkValueHtml(this)"><option value="">请选择</option><option value="7" selected>商城一级页面</option><option value="8">商城二级列表ID</option><option value="1">会场ID</option><option value="2">特卖ID</option><option value="3">商品ID</option><option value="4">自建页面ID</option></select>';
			...
			<c:forEach items="${productTypes}" var="productType">
				...
				linkValueHtml+='<option value="${productType.id}" '+_selected${productType.id}+'>${productType.name}馆</option>';
			</c:forEach>
			linkValueHtml+='</select>';
		}else{
			var _selected1="",_selected2="",_selected3="",_selected4="",_selected8="";
			if(linkType == "1"){
				_selected1 = "selected";
			}else if(linkType == "2"){
				_selected2 = "selected";
			...
			linkTypeHtml = '<select name="linkType" id="linkType" onchange="setLinkValueHtml(this)"><option value="">请选择</option><option value="7">商城一级页面</option><option value="8" '+_selected8+'>商城二级列表ID</option><option value="1" '+_selected1+'>会场ID</option><option value="2" '+_selected2+'>特卖ID</option><option value="3" '+_selected3+'>商品ID</option><option value="4" '+_selected4+'>自建页面ID</option></select>';
	...
	function saveMapInfo(_this) {
		...
		if(!testNumber(linkValue.value)){
			commUtil.alertError("只能输入正整数！");
			return false;
		}
		if(linkType.value != 7){//排除7.商城一级页面
			$.ajax({
...
//			box_dbframe.title = pop_title.value;
	...
	function setLinkValueHtml(_this){
		...
		if(linkType == 7){
			...
			<c:forEach items="${productTypes}" var="productType">
				html+='<option value="${productType.id}">${productType.name}馆</option>';
			</c:forEach>
			html+='</select>';
		}else{

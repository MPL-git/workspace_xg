...
$(function(){
	$("#export").bind('click',function(){
		...
		var year = $("#year").val();
		var month = $("#month").val();
		location.href="${pageContext.request.contextPath}/popDealOrderDtl/mchtMonthlyCollections/exportTotal.shtml?mchtCode="+mchtCode+"&name="+name+"&year="+year+"&month="+month;
	});
	...
	$("#count").bind('click',function(){
		...
		$.ligerDialog.open({
			...
			title: "合计",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/popDealOrderDtl/mchtMonthlyCollections/count.shtml?mchtCode="+mchtCode+"&name="+name+"&year="+year+"&month="+month,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
...
function popDealOrderDtlList(mchtCode,dateBegin,dateEnd,accept,refund){
 ...
 var listConfig={
     url:"/popDealOrderDtl/mchtMonthlyCollections/data.shtml",
     listGrid:{ columns: [
		                ...
		                { display: '本月代收',render: function (rowdata, rowindex) {
		                ...
		                { display: '本月代收应结',render: function (rowdata, rowindex) {
		                	if(rowdata.settleAmount){
		                		...
		                		var accept = "1";
		                		var refund = "0";
								return '<a href="javascript:;" onclick="popDealOrderDtlList('+"'"+rowdata.mchtCode+"'"+','+"'"+date_begin+"'"+','+"'"+date_end+"'"+','+accept+','+refund+');">'+formatMoney(rowdata.settleAmount,2)+'</a>';	                		
		                	}else{
		                		return "0.00";
		                	}
		                }},
		                { display: '本月代退', render: function (rowdata, rowindex) {
		                	if(rowdata.returnAmount){
								return formatMoney(rowdata.returnAmount,2);	                		
		                	}else{
		                		return "0.00";
		                	}
		                }},
		                { display: '本月代退应扣',render: function (rowdata, rowindex) {
		                	if(rowdata.returnOrderAmount){
		                		...
		                		var accept = "0";
		                		var refund = "1";
								return '<a href="javascript:;" onclick="popDealOrderDtlList('+"'"+rowdata.mchtCode+"'"+','+"'"+date_begin+"'"+','+"'"+date_end+"'"+','+accept+','+refund+');">'+formatMoney(rowdata.returnOrderAmount,2)+'</a>';	                		
		                	}else{
		                		return "0.00";
		                	}
		                }},
		...
		<div class="search-tr"  >
			...
			<div class="l-panel-search-item" >
			 ...
			 <div class="search-td-search-btn" style="display: inline-flex;">
				...
				<div style="padding-left: 10px;">
					<input type="button" style="width: 65px;height: 30px;cursor: pointer;" value="合计" id="count">
				</div>
			</div>
		 </div>

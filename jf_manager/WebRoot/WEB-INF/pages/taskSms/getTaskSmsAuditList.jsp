	...
	function viewTaskSms(taskSmsId) {
	 ...
	 function auditOrApproveTask(taskSmsId) {
 		$.ligerDialog.open({
 			...
 			title: "审核",
 			name: "INSERT_WINDOW",
 			url: "${pageContext.request.contextPath}/taskSms/auditOrApproveTaskManager.shtml?taskSmsId="+taskSmsId,
 			showMax: true,
 			showToggle: false,
 			showMin: false,
 			isResize: true,
 	 ...
 	 var listConfig={
	      ...
	      listGrid:{ columns: [
							...
							{display:'操作',name:'', align:'center', isSort:false, width:160, render:function(rowdata,rowindex){
								var html = [];
								html.push("<a href=\"javascript:auditOrApproveTask(" + rowdata.id + ");\">【审核】</a>");
								return html.join("");
							}}
			         ], 
					...
					<div class="search-td-combobox-condition" >
						<select id="taskStatus" name="taskStatus" style="width: 135px;" >
							<c:forEach items="${taskStatusList }" var="taskStatus">
								<c:if test="${taskStatus.statusValue == '1' }">

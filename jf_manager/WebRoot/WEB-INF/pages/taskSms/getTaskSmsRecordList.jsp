...
<head>
 ...
 <script type="text/javascript">
	...
	function viewTaskSms(taskSmsId) {
     ...
     //获取执行详细信息
     function getDetailedInformation(taskId, sendType, type) {
         $.ajax({
             type: 'post',
             url: '${pageContext.request.contextPath }/taskSms/getDetailedInformation.shtml',
             data: {taskId : taskId, sendType : sendType, type : type},
             dataType: 'json',
             success: function(data) {
                 if(data == null || data.code != 200){
                     commUtil.alertError(data.msg);
                 }else {
                     $("#"+taskId+"DetailedInformation"+type).html(data.reQuantityRate);
                 }
             },
             error: function(e) {
 	 ...
 	 var listConfig={
	      ...
	      btnItems:[],
	      listGrid:{ columns: [
							{display:'发送渠道',name:'taskSendChannelDesc', align:'center', isSort:false, width:100},
							{display:'任务名称',name:'', align:'center', isSort:false, width:100, render:function(rowdata,rowindex) {
								var html = [];
							...
							{display:'发送用户数',name:'sendMemberCount', align:'center', isSort:false, width:80},
							{display:'条数',name:'sendCount', align:'center', isSort:false, width:80},
							{display:'成本（元）',name:'totalSendAmount', align:'center', isSort:false, width:100},
							...
							{display:'审核人',name:'auditStaffName', align:'center', isSort:false, width:80},
							{display:'审批人',name:'approvalStaffName', align:'center', isSort:false, width:80},
							{display:'执行人',name:'executeStaffName', align:'center', isSort:false, width:80},
							{display:'7天内登录用户数',name:'memberLoginCount', align:'center', isSort:false, width:100, render:function(rowdata,rowindex) {
                                return '<span id='+ rowdata.taskId +'DetailedInformation1><a href="javascript:getDetailedInformation('+ rowdata.taskId +','+ rowdata.taskSendType +','+ 1 +');">查看</a></span>';
							}},
							{display:'7天内付款用户数',name:'memberOrderCount', align:'center', isSort:false, width:100, render:function(rowdata,rowindex) {
								return '<span id='+ rowdata.taskId +'DetailedInformation2><a href="javascript:getDetailedInformation('+ rowdata.taskId +','+ rowdata.taskSendType +','+ 2 +');">查看</a></span>';
							}},
							{display:'7天内付款订单数',name:'combineOrdeCount', align:'center', isSort:false, width:100, render:function(rowdata,rowindex) {
								return '<span id='+ rowdata.taskId +'DetailedInformation3><a href="javascript:getDetailedInformation('+ rowdata.taskId +','+ rowdata.taskSendType +','+ 3 +');">查看</a></span>';
							}},
							...
							{display:'任务状态',name:'taskStatusDesc', align:'center', isSort:false, width:80},

...
$(function ()  { 
  ...
  $("#form1").validate({
	...
	<form method="post" id="form1" name="form1"  action="${pageContext.request.contextPath}/work/addchangeWork.shtml" >
		...
		<table class="gridtable">
				<tr>
					...
					<td align="left" class="l-table-edit-td" >
						<select style="width: 135px;" id="orgId" name="orgId" onchange="staffIdinfo();" >
							...
							<c:forEach var="sysOrganizationlist" items="${sysOrganizationlist}">
								<option value="${sysOrganizationlist.orgId}" <c:if test="${sysOrganizationlist.orgId == worekrecordorgid}">selected</c:if> > 
	           	...
	           	<tr>
					...
					<td align="left" class="l-table-edit-td" >
						<select style="width: 135px;" id="workType" name="workType" >
							<option value="">请选择...</option>							
				            <option value='1' <c:if test="${woRk.workType eq '1' }">selected</c:if>>售后咨询</option>
				            <option value='2' <c:if test="${woRk.workType eq '2' }">selected</c:if>>售前咨询</option>
					        <option value='3' <c:if test="${woRk.workType eq '3' }">selected</c:if>>入驻咨询</option>
					        <option value='4' <c:if test="${woRk.workType eq '4' }">selected</c:if>>合同咨询</option>
					        <option value='5' <c:if test="${woRk.workType eq '5' }">selected</c:if>>工商投诉</option>
					        <option value='6' <c:if test="${woRk.workType eq '6' }">selected</c:if>>其他</option>
						</select>
					</td>	
	           	</tr>
	           	...
	           	<tr>	      
					...
					<td align="left" class="l-table-edit-td" >
						<select style="width: 135px;" id="urgentDegree" name="urgentDegree" >
							<option value="">请选择...</option>							
				            <option value='1' <c:if test="${woRk.urgentDegree eq '1' }">selected</c:if>>普通</option>
				            <option value='2' <c:if test="${woRk.urgentDegree eq '2' }">selected</c:if>>重要</option>
					        <option value='3' <c:if test="${woRk.urgentDegree eq '3' }">selected</c:if>>紧急</option>
					        <option value='4' <c:if test="${woRk.urgentDegree eq '4' }">selected</c:if>>重要紧急</option>					
						</select>
					</td>
	           	</tr>
	           	...
	           	<tr>
					...
					<td align="left" class="l-table-edit-td" >
						<select style="width: 135px;" id="relevantType" name="relevantType" >
							<option value=''>请选择...</option>
				            <option value='1' <c:if test="${woRk.relevantType eq '1' }">selected</c:if>>介入单</option>
				            <option value='2' <c:if test="${woRk.relevantType eq '2' }">selected</c:if>>投诉单</option>
					        <option value='3' <c:if test="${woRk.relevantType eq '3' }">selected</c:if>>意见反馈</option>
					        <option value='4' <c:if test="${woRk.relevantType eq '4' }">selected</c:if>>子订单</option>
					        <option value='5' <c:if test="${woRk.relevantType eq '5' }">selected</c:if>>售后单</option>
					        <option value='6' <c:if test="${woRk.relevantType eq '6' }">selected</c:if>>其他</option>					
						</select>
					</td>
	           	</tr>
	           	...
	           	<tr>	       
					...
					<td align="left" class="l-table-edit-td" >
						<input style="width:360px;" type="text" id="relevantCode" name="relevantCode" value="${woRk.relevantCode}" />
					</td>
	           	</tr>
	            ...
	            <tr>
					...
					<td align="left" class="l-table-edit-td" >
						<input style="width:360px;" type="text" id="titleContent" name="titleContent" value="${woRk.titleContent}"  />
					</td>
	           	</tr>
	         ...
	         <tr>
				...
				<td align="left" class="l-table-edit-td" >
					<textarea name="describeContent" id="describeContent" style="width:150px;height:300px;visibility:hidden;">${woRk.describeContent}</textarea>
              	</td>  
            </tr>

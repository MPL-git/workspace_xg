 ...
 * 合同生成PDF
 */
public class PDFCreaterMchtContract {
...
//        BaseFont bfChinese = BaseFont.createFont("STSong-Light", "UniGB-UCS2-H", BaseFont.NOT_EMBEDDED);
        BaseFont bfChinese = BaseFont.createFont(PDFCreaterMchtContract.class.getResource("/resources/fonts/simsun.ttf").getPath(),BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);
        //设置字体样式
...
//        PdfWriter writer = PdfWriter.getInstance(doc, new FileOutputStream(new File(filePath)));
        ...
        Paragraph paragraph = new Paragraph("《醒购商城合作协议》", new Font(bfChinese, 25, Font.BOLD));
...
//        paragraph = new Paragraph("   协议编号", firstTitleFont);
//        paragraph.setSpacingBefore(5);
...
//        doc.add(paragraph);
        ...
        cell = new PdfPCell(new Paragraph("协议编号：" + contract.getContractCode(), firstTitleFont));
        ...
        table.addCell(cell);
        cell = new PdfPCell(new Phrase("类目：" + productTypeStr, firstTitleFont));
        cell.setPaddingLeft(5);
        cell.setBorderWidth(0);
        cell.setHorizontalAlignment(Cell.ALIGN_LEFT); //设置水平居中
        cell.setVerticalAlignment(Cell.ALIGN_MIDDLE); //设置垂直居中
        ...
        cell.setVerticalAlignment(Cell.ALIGN_MIDDLE); //设置垂直居中
        table.addCell(cell);
        cell = new PdfPCell(new Phrase("联系方式：" + contactMobile, textFont));
        cell.setPaddingLeft(5);
        cell.setMinimumHeight(30); //设置单元格高度
        cell.setHorizontalAlignment(Cell.ALIGN_LEFT); //设置水平居中
        cell.setVerticalAlignment(Cell.ALIGN_MIDDLE); //设置垂直居中
        ...
        cell.setVerticalAlignment(Cell.ALIGN_MIDDLE); //设置垂直居中
        table.addCell(cell);
        cell = new PdfPCell(new Phrase("电子邮件：" + contactEmail, textFont));
        cell.setPaddingLeft(5);
        cell.setMinimumHeight(30); //设置单元格高度
        cell.setHorizontalAlignment(Cell.ALIGN_LEFT); //设置水平居中
        cell.setVerticalAlignment(Cell.ALIGN_MIDDLE); //设置垂直居中
        ...
        cell.setVerticalAlignment(Cell.ALIGN_MIDDLE); //设置垂直居中
        table.addCell(cell);
        cell = new PdfPCell(new Phrase("通讯地址：厦门市思明区塔埔东路171号1104-A单元", textFont));
        cell.setPaddingLeft(5);
        ...
        //当合作模式是SPOP时添加
        if(StringUtils.equals(mchtInfo.getMchtType(),"1")){
            List<String> mchtProductBrands = new ArrayList<>();
            for(MchtProductBrand mchtProductBrand : mchtProductBrandList){
                mchtProductBrands.add(mchtProductBrand.getProductBrandName());
            }
            phase.add(new Chunk("，经营品牌：", textFont));
            phase.add(new Chunk(StringUtils.join(mchtProductBrands.toArray(),","), underlineFont));
        }
        ...
        phase.add(Chunk.NEWLINE); //换行
        //POP、SPOP文本切换
        if(StringUtils.equals(mchtInfo.getMchtType(),"2")){
        ...
        }else if(StringUtils.equals(mchtInfo.getMchtType(),"1")){
            phase.add(new Chunk("3.1 乙方须向甲方支付技术服务费，技术服务费指乙方在醒购完成的每一笔订单交易额减去乙方与醒购约定的该笔订单结算额之后，醒购所收取的费用。若无特别约定，结算额应小于交易额。若乙方申请特卖，技术服务费费用由双方另行协商。", textFont));
            phase.add(Chunk.NEWLINE); //换行
            phase.add(new Chunk("3.2 乙方授权甲方从乙方销售货款中直接扣除乙方须支付给甲方的技术服务费。若：微信支付、支付宝支付等第三方支付机构调整支付服务费费用，则本条款约定的支付费用应当相应调整。", textFont));
            phase.add(Chunk.NEWLINE); //换行
            phase.add(new Chunk("3.3 乙方充分理解并确认，因后期实际运营需要，甲方可根据商城不同力度的优惠活动对商城技术服务费用进行调整，同时甲方保留变更收取商城费用标准的权利，具体收费标准将以商城规则形式发布。", textFont));
            phase.add(Chunk.NEWLINE); //换行
            phase.add(new Chunk("3.4 协议生效后，乙方须向甲方支付平台使用费 0 元/月。", textFont));
        }
        phase.add(Chunk.NEWLINE); //换行
        paragraph.add(phase);
        doc.add(paragraph);
        ...
        phase.add(new Chunk("上述“资质文件”包含但不限于乙方的营业执照、银行开户许可证、商标证书、品牌授权书、质检报告或商品质量合格证明、收款账户、通讯地址等。", boldFont));
        phase.add(Chunk.NEWLINE); //换行
        paragraph.add(phase);
        doc.add(paragraph);
        ...
        if(StringUtils.equals(mchtInfo.getMchtType(),"2")){
            ...
            phase.add(new Chunk("9.3.1 甲方系统在每周一自动对上星期（含周末）订单状态更新为完成的订单中属乙方的可结算订单明细商品进行生成结算汇总单，此结算汇总单简称为结算单。结算单内容包括但不限于：商品数量、订单实付金额、乙方直赔单金额、技术服务费金额、结算单金额。结算单金额为每个可结算订单明细商品的结算金额合计减乙方直赔单金额。", textFont));
        }
        phase.add(Chunk.NEWLINE); //换行
        ...
        phase.add(Chunk.NEWLINE); //换行
        phase.add(new Chunk("10.2.5	乙方应提高其服务质量尽量避免消费者发生退款，若发生退款频繁或因退款影响甲方正常运营等情形，甲方有权单方面解除合作并扣除保证金，乙方货款在消费者退款、优惠券及违约金等费用抵扣完毕后予以结算。", textFont));
        phase.add(Chunk.NEWLINE); //换行
        ...
        phase.add(Chunk.NEWLINE); //换行
        phase.add(new Chunk("11.4 乙方同意并保证，其任一级代表、代理、受托人或其董事、高级管理人员或其他职工及其自身，不得在中国之境内外注册、使用与甲方及其关联方拥有、使用、许可或控制的商标、标识、标志、微信公众号、域名、艺术作品、人物形象等相同或近似的商标，或侵犯甲方或其关联方享有的一切知识产权、工业产权和专有权利，或妨碍甲方或其关联方独自全部拥有或保留前述权利。", textFont));
        phase.add(Chunk.NEWLINE); //换行
        ...
        phase.add(Chunk.NEWLINE); //换行
        phase.add(new Chunk("11.6 乙方同意并保证其任一级代表、代理、受托人或其董事、高级管理人员或其他职工均遵守本合同的约定，受到本合同的拘束，如同该人被指定为“乙方”一样。", textFont));
        phase.add(Chunk.NEWLINE); //换行
        paragraph.add(phase);
        doc.add(paragraph);
        ...
        phase.add(Chunk.NEWLINE); //换行
        phase.add(new Chunk("13.1.4	甲方有权根据市场实际需求适度调整平台技术服务费,并将该费用调整后在醒购公示。", textFont));
        phase.add(Chunk.NEWLINE); //换行
        ...
        cell = new PdfPCell(new Phrase("企业邮箱：" + contactEmail, textFont));
        cell.setBorderWidth(0);
        cell.setMinimumHeight(30); //设置单元格高度
        cell.setHorizontalAlignment(Cell.ALIGN_LEFT); //设置水平居中
        cell.setVerticalAlignment(Cell.ALIGN_MIDDLE); //设置垂直居中
        table.addCell(cell);
        cell = new PdfPCell(new Phrase("企业邮箱：", textFont));
        cell.setBorderWidth(0);
        cell.setHorizontalAlignment(Cell.ALIGN_LEFT); //设置水平居中
        cell.setVerticalAlignment(Cell.ALIGN_MIDDLE); //设置垂直居中
        table.addCell(cell);

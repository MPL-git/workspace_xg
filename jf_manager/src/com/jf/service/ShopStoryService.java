...
public class ShopStoryService extends BaseService<ShopStory, ShopStoryExample> {
	...
	public void updateWordAndPic(Integer mchtId,String shopStoryId,String storyBrief,String picArrayJsonStr){
		...
		if(!StringUtils.isEmpty(storyBrief)){
		...
		if(!StringUtil.isEmpty(picArrayJsonStr)){//修改故事详情表
			...
			for (int i = 0; i < picArrayList.size(); i++) {
				...
				if(!StringUtil.isEmpty(word)){//div为文字模块
				shopStoryDetail4Update.setContent(word);
				shopStoryDetail4Update.setUpdateBy(mchtId);
				shopStoryDetail4Update.setUpdateDate(new Date());
				ShopStoryDetailExample  shopStoryDetailWordExample= new ShopStoryDetailExample();
				shopStoryDetailWordExample.createCriteria().andShopStoryIdEqualTo(Integer.parseInt(shopStoryId)).andContentEqualTo(word);
				wordCount = shopStoryDetailMapper.updateByExampleSelective(shopStoryDetail4Update, shopStoryDetailWordExample);
				}

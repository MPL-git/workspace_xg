 ...
 var listConfig={
      ...
      listGrid:{ columns: [  /* { display: 'ID', name: 'id'}, */ 
						...
						{display:'期望活动开始时间',name:'expectedStartTime',width: 150,render:function(rowdata,rowindex){
							if(rowdata.expectedStartTime==null||rowdata.expectActivityTime==""||rowdata.expectedStartTime==undefined){
								return "";
							}else{
						...
						{display:'审核人',name:'designOrLawName',render:function(rowdata,rowindex){
							if(type==3){
								if(rowdata.designName==null){
									return "<input type='button' style='cursor:pointer;background-color:rgba(0, 218, 236, 1);width: 50px;height: 30px;color: #FFFFFF;' value='领取' onclick='receive("+rowdata.id+");'>";
								}else{
									return rowdata.designName;
								}
							}else{
								if(rowdata.lawName==null){
									return "<input type='button' style='cursor:pointer;background-color:rgba(0, 218, 236, 1);width: 50px;height: 30px;color: #FFFFFF;' value='领取' onclick='receive("+rowdata.id+");'>";
								}else{

...
<%-- 		<c:if test="${type eq 2 }">
	...
	<c:if test="${type eq 1 }">
		...
		<table class="gridtable">
			<tr>
				<td style="border:none">
					...
					<span class="radioClass"><input border="none" class="radioItem" type="radio" value="2" id="adoptOperateStatus" name="adoptOperateStatus" onclick="adoptOperateStatus();" value="" <c:if test="${activityCustom.operateAuditStatus==2}">checked="checked"</c:if> >通过审核</span>
					<span class="radioClass"><input border="none" class="radioItem" type="radio" value="3" id="rejectOperateStatus" name="rejectOperateStatus" onclick="rejectOperateStatus();" value="" <c:if test="${activityCustom.operateAuditStatus==3}">checked="checked"</c:if> >已驳回</span>
				</td>
			</tr>
			<tr id="text">
				<td style="border:none">
					<textarea id="statusRemarks" class="textarea" rows="10" cols="100">${activityCustom.operateAuditRemarks}</textarea>
				</td>
			</tr>
		</table>	
		<br/><DIV style="BORDER-TOP: #00686b 1px dashed; OVERFLOW: hidden; HEIGHT: 1px"></DIV>
		...
		<table class="gridtable">
			<tr>
				<td style="border:none">
					<c:if test="${activityCustom.operateAuditStatus eq 1}">
	...
	if(type==1){
 	...
 	if(status==3){
		...
		if(activityEndTime.length==0){
 		...
 		if(preheatTime>=activityBeginTime){
		...
		var preheatDate=new Date(preheatTime);
		var beginDate=new Date(activityBeginTime);
		var endDate=new Date(activityEndTime);
		var nowDate=new Date();
		var dateDiff1=preheatDate.getTime()-nowDate.getTime();
		var dateDiff2=beginDate.getTime()-nowDate.getTime();
		var dateDiff3=endDate.getTime()-nowDate.getTime();
   		if (dateDiff3<0){
   			commUtil.alertError("活动结束时间不能小于当前时间。");
   			return;
   		}
   		var days1=Math.floor(dateDiff1/(24*3600*1000));
   		var days2=Math.floor(dateDiff2/(24*3600*1000));
   		var days3=Math.floor(dateDiff3/(24*3600*1000));
   		
   		if (days1>=30){
   			commUtil.alertError("预热时间不能大于当前时间30天。");
   			return;
   		}
   		if (days2>=30){
   			commUtil.alertError("活动开始时间不能大于当前时间30天。");
   			return;
   		}
   		if (days3>=30){
   			commUtil.alertError("活动结束时间不能大于当前时间30天。");
   			return;
   		}
	}else{

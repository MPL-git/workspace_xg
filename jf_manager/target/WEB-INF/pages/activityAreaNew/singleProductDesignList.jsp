...
$(function(){
	...
	$("#saveSort").bind('click',function(){
		...
		$.ajax({ //ajax提交
			type:'POST',
			url:"${pageContext.request.contextPath}/activityAreaNew/saveActivityProductSort.shtml",
			data:{
				sortIds:sortIds
			},
			dataType:'json',
						...
						<td class="l-grid-row-cell ">
							<div class="l-grid-row-cell-inner">
								<span style="display:block;text-align:left;margin-top:8px;">
									<img style="margin:10px;" src="${pageContext.request.contextPath}/file_servelt${activityProductCustom.productPic}" width="150" height="150" onclick="viewerPic(this.src)"><br>
									${activityProductCustom.productName}<br>
									价格：<span style="color:red;">${activityProductCustom.salePriceMin}-${activityProductCustom.salePriceMax}</span>元，库存总数：${activityProductCustom.productStock}件<br>
									30天本款销量：${activityProductCustom.dfgNum}件，销售额：${activityProductCustom.dfgPrice}元<br>
									排序值：<input type="text" name="sort"  style="width:80px;" activityProductId="${activityProductCustom.id}" value="${activityProductCustom.seqNo}"><br>
								</span>
							</div>
						</td>

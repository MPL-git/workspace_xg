...
$.ajax({
	...
	function saveMapInfo(_this) {
		...
		if(!testNumber(linkValue.value) && linkType.value!=9 && linkType.value!=16){//16.二级分类下拉多选
			commUtil.alertError("只能输入正整数！");
			return false;
		}
		if(linkType.value != 5 && linkType.value != 6 && linkType.value != 9 && linkType.value != 16){//排除5.栏目和6.一级分类和9.url链接和16.二级分类二级分类下拉多选
			$.ajax({
...
//			box_dbframe.title = pop_title.value;
	...
	function setLinkValueHtml(_this){
		...
		var html;
		if(linkType == 5){
			html = '<select id="linkValue" name="linkValue" style="position:absolute;margin-left: 65px;" onmousedown="if(this.options.length>5){this.size=6}" onblur="this.size=0" onchange="this.size=0"><option value="">请选择</option>';
			for(var i=0;i<linkValueList.length;i++){
				var id = linkValueList[i].linkValue;
				var name = linkValueList[i].linkValueName;			

...
$.ajax({
	...
	function openMapInfo(_this) {
		...
		if(!linkType){
			...
			if(linkType == "5"){	   
				...
				linkValueHtml+='</select>';
			 }else if (linkType == "16" || linkType == "18" || linkType == "20" || linkType == "22" || linkType == "24" 
	...
	function saveMapInfo(_this) {
		...
		if(linkType.value != 5 && linkType.value != 6 && linkType.value != 9  && linkType.value != 18 
			...
			/* box_dbframe.title = pop_title.value; */
			box_dbframe.setAttribute('data-linkType', linkType.value);
			if(linkType.value != 16 &&  linkType.value != 18 && linkType.value != "20" && linkType.value != "22" && linkType.value != "24" 
				 && linkType.value != "26" && linkType.value != "28" && linkType.value != "30" && linkType.value != "16"){
				box_dbframe.setAttribute('data-linkValue', linkValue.value);
			}else{
				var firstProductTypeId = $("#linkType").parent().next().find("select[name='firstProductType']").val();
				var productTypeIdsArray = [];
	...
	function setLinkValueHtml(_this){
		...
		if(linkType == 5){
			...
			var h = [];
			h.push('<select name="firstProductType" style="width:120px;">');

...
<form method="post" id="test_box" name="form" action="${pageContext.request.contextPath}/customPage/saveDecorateModule.shtml">
			...
			<input type="hidden" id="firstProductTypeId" name="firstProductTypeId" value="${firstProductTypeId}">
			<c:if test="${moduleType == 1 || moduleType == 9}">
			<div>
				...
				<div class="box-con">
					...
					<c:forEach items="${moduleMapJSONArray}" var="moduleMapJsonObject">	
						<div class="box-frame" data-x1="${moduleMapJsonObject.x1}" data-y1="${moduleMapJsonObject.y1}" data-x2="${moduleMapJsonObject.x2}" data-y2="${moduleMapJsonObject.y2}" data-linktype="${moduleMapJsonObject.linkType}" data-linkvalue="${moduleMapJsonObject.linkValue}"  <c:if test="${moduleMapJsonObject.linkType == 16 || moduleMapJsonObject.linkType == 18 || moduleMapJsonObject.linkType == 20 || moduleMapJsonObject.linkType == 22 || moduleMapJsonObject.linkType == 24 || moduleMapJsonObject.linkType == 26 || moduleMapJsonObject.linkType == 28 || moduleMapJsonObject.linkType == 30}">data-firstProductTypeId="${moduleMapJsonObject.firstProductTypeId}"</c:if>  style="left: ${moduleMapJsonObject.x1}%; top: ${moduleMapJsonObject.y1}%; width: ${moduleMapJsonObject.width}%; height: ${moduleMapJsonObject.height}%;">
...
var linkValueList = "";
$.ajax({
	url : "${pageContext.request.contextPath}/linkType/moduleMap/getLinkTypeList.shtml?showType=${showType}",
	type : 'POST',
	dataType : 'json',
	cache : false,
	async : false,
	...
	function openMapInfo(_this) {
		...
		var linkType = box_dbframe.getAttribute('data-linkType') || '',
		    ...
		    firstProductTypeId = box_dbframe.getAttribute('data-firstProductTypeId') || '';
	        ...
	        if(linkType == "5"){
	    	    ...
	    	    linkValueHtml=html1.join("");
   				...
   				<c:forEach items="${mallCategorys}" var="mallCategory">
   					var _selected="";
   					var mallCategoryTypeId = ${mallCategory.id};
   					if(linkValue == mallCategoryTypeId){
   						_selected = "selected";
   					}
    			...
    			linkValueHtml+='</select>';
    			linkTypeHtml=html.join("");
	    	}else if( linkType == "16" || linkType == "18" || linkType == "20" || linkType == "22" || linkType == "24" || linkType == "26" || linkType == "28" || linkType == "30"){
	    		...
	    		var html = [];
				html.push('<select name="firstProductType" style="width:250px;">');
				<c:forEach items="${productTypes}" var="productType">
					var selectedStr=""
	...
	function saveMapInfo(_this) {
		...
		if(!linkValue.value){
			if(linkType.value == 16 || linkType.value == 18 ||linkType.value == 20 || linkType.value == 22 || linkType.value == 24 || linkType.value == 26 || linkType.value == 28 || linkType.value == 30  ){
				commUtil.alertError("请选中详细里的二级类目，可以按住Ctrl键多选。");
			}else{
		...
		if(!testNumber(linkValue.value)&& linkType.value!=8 && linkType.value!=9 && linkType.value!=11 && linkType.value!=13 && linkType.value!=16 && linkType.value!=18 && linkType.value!=19 && linkType.value!=20 && linkType.value!=21 && linkType.value!=22 && linkType.value!=23 && linkType.value!=24 && linkType.value!=25 && linkType.value!=26 && linkType.value!=27 && linkType.value!=28 && linkType.value!=29 && linkType.value!=30 && linkType.value!=31){
			commUtil.alertError("只能输入正整数！");
			return false;
		}
		if(linkType.value != 5 && linkType.value != 6 && linkType.value != 9 && linkType.value != 16 && linkType.value != 18 
				&& linkType.value != 20 && linkType.value != 22 && linkType.value!=24 && linkType.value!=26 
				&& linkType.value!=28 && linkType.value!=30 ){//排除5.栏目和6.一级分类和9.url链接
			$.ajax({
...
//						box_dbframe.title = pop_title.value;
						...
						if(linkType.value != 16 && linkType.value != 18 && linkType.value != 20 && linkType.value != 22  && linkType.value!=24 && linkType.value!=26 && linkType.value!=28 && linkType.value!=30 ){
							...
							var firstProductTypeId = $("#linkType").parent().next().find("select[name='firstProductType']").val();
							var productTypeIdsArray = [];
							$("#linkValue").find("option:selected").each(function(){
								productTypeIdsArray.push(parseInt($(this).val()));
							});
							box_dbframe.setAttribute('data-linkValue', productTypeIdsArray.join(","));
							box_dbframe.setAttribute('data-firstProductTypeId', firstProductTypeId);
						}
//						box_dbframe.title = pop_title.value;
...
//			box_dbframe.title = pop_title.value;
	...
	function changeTyep2(h){
		h.push('<select name="firstProductType" id="firstProductType" style="width:250px;">');
		...
		if(linkType == 5){
			$.ajax({
				...
				success : function(data) {
						...
						html = '<select name="linkValue" id="linkValue" onmousedown="if(this.options.length>10){this.size=11}" onblur="this.size=0" onchange="this.size=0"><option value="">请选择</option>';
						for(var i=0;i<linkValueList.length;i++){
							var id = linkValueList[i].linkValue;
							var name = linkValueList[i].linkValueName;			

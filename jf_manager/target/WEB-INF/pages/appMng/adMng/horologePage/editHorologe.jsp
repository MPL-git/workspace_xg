...
function radioItem(linkType, linkId) {
	switch (linkType) {
		case '2':
			...
			$("#show1-2").html('<input type="text" id="linkId" name="linkId" value="' + linkId + '" validate="{digits:true,maxlength:9}"/>');
			break;
		case '3':
		...
		case '5':
			$("#show1-1").html('URL');
			$("#show1-2").html('<input type="text" id="linkUrl" name="linkUrl" value="' + linkId + '" />');
			break;
	}
}
...
<body>
	<form method="post" id="form1" name="form1" action="${pageContext.request.contextPath}/appMng/adMng/saveSubmit.shtml">
		...
		<input type="hidden" id="catalog" name="catalog" value="${adInfoCustom.catalog }"/>
			...
			<tr>
				...
				<td colspan="5" align="left" class="l-table-edit-td">
					<c:forEach var="typeItem" items="${linkTypeList }">
						<c:if test="${typeItem.statusValue != 6 and typeItem.statusValue != 1 and typeItem.statusValue != 4 }">
							<span class="radioClass"><input class="radioItem" onClick="radioItem(this.value, '')" type="radio" value="${typeItem.statusValue }" name="linkType" <c:if test="${typeItem.statusValue==adInfoCustom.linkType}">checked="checked"</c:if>>${typeItem.statusDesc}</span>
						</c:if>
					</c:forEach>
				</td>
			</tr>

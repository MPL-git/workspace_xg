...
function save(){
	...
	if(!productTypeId){
		commUtil.alertError("一级分类不能为空");
		return;
	}
	$.ajax({
		url : "${pageContext.request.contextPath}/brandteamType/save.shtml",
		type : 'POST',
		dataType : 'json',
		cache : false,
		async : false,
...
<html>
<body>
<form id="violateOrderForm" method="post" action="${pageContext.request.contextPath}/brandteamType/save.shtml">
	<input type="hidden" id="id" name="id" value="${brandteamType.id}">
	<table class="gridtable">
		<tr>
			<td class="title">名称<span class="required">*</span></td>
			<td align="left" class="l-table-edit-td">
				<input type="text" id="name" name="name" value="${brandteamType.name}" maxlength="8"/>
			</td>
		</tr>
		<tr>
			<td class="title">一级分类<span class="required">*</span></td>
			<td align="left" class="l-table-edit-td">
				<select id="productTypeId" name="productTypeId">
				    <option value="">请选择</option>
	               	<option value="0" <c:if test="${brandteamType.productTypeId eq '0'}">selected="selected"</c:if>>预告</option>
	               	<c:forEach items="${productTypeList}" var="productType">
		               	<option value="${productType.id}" <c:if test="${productType.id eq brandteamType.productTypeId}">selected="selected"</c:if>>${productType.name}</option>
	               	</c:forEach>
               	</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				<input type="button" class="l-button l-button-submit" value="取消" onclick="frameElement.dialog.close();"/>
			</td>
		</tr>
	</table>	
</form>		

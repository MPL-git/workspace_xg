...
<head>
    ...
    <script type="text/javascript">
        ...
        function fwResult(id) {
            $.ligerDialog.open({
                height: 300,
                width: 800,
                title: "法务处理",
                name: "INSERT_WINDOW",
                url: "${pageContext.request.contextPath}/businessCirclesAppealOrder/fwProcessingResults.shtml?id=" + id,
                showMax: true,
                showToggle: false,
                showMin: false,
                isResize: true,
        ...
        function businessCirclesResult(id) {
            $.ligerDialog.open({
                height: 300,
                width: 800,
                title: "工商处理结果",
                name: "INSERT_WINDOW",
                url: "${pageContext.request.contextPath}/businessCirclesAppealOrder/businessCirclesResult.shtml?id=" + id,
                showMax: true,
                showToggle: false,
                showMin: false,
                isResize: true,
        ...
        var listConfig = {
            url: "/businessCirclesAppealOrder/fwbusinessCirclesAppealOrderdata.shtml",
            listGrid: {
                columns: [
                    ...
                    {display: '工商处理情况', name: 'businessCirclesResult', align: 'center', isSort: false, width: 200},
                    {
                        display: '操作', width: 100, render: function (rowdata, rowindex) {
                            var html = [];
                            if (rowdata.appealStatus == '2') {
                                html.push("<a href=\"javascript:fwResult(" + rowdata.id + ");\">法务处理</a><br>");

                            } else {
                                html.push("<a href=\"javascript:businessCirclesResult(" + rowdata.id + ");\">工商处理结果</a><br>");
                            }
                            return html.join("");
                        }
                    }
            ...
            <div class="search-td">
                ...
                <div class="search-td-condition">
                    <select id="appealStatus" name="appealStatus" class="querysel">
                        <option value="2" selected="selected">待法务处理</option>

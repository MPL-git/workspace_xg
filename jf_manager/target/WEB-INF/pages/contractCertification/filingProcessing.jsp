...
$(function(){
	$("#confirm").bind('click',function(){
		...
		//驳回原因
		if(status == '2' && archiveDealRemarks == ''){
			commUtil.alertError("驳回原因不可为空");
			return;
		}
		$.ajax({
			url : "${pageContext.request.contextPath}/contractCertification/auditSubmit.shtml",
			type : 'POST',
			dataType : 'json',
			cache : false,
			async : false,
...
<body>
	...
	<div class="title-top">
	<input type="hidden" id="id" name="id" value="${mchtInfoChg.id}">
	<input type="hidden" id="mchtId" name="mchtId" value="${mchtInfoChg.mchtId}">
	<input type="hidden" id="realStatus" name="realStatus" value="${realStatus}">
		<table class="gridtable">
          	<tr>
          		<td class="title">审核状态<span style="color:red;">*</span></td>
          		<td><input type="radio" name="archiveDealStatus" value="1" <c:if test="${mchtInfoChg.archiveDealStatus=='1'}">checked="checked"</c:if>>通过
          		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          		<input type="radio" name="archiveDealStatus" value="2" <c:if test="${mchtInfoChg.archiveDealStatus=='2'}">checked="checked"</c:if>>驳回
          		</td>
          	</tr>
          	<tr>
          		<td class="title"><c:if test="${mchtInfoChg.archiveDealStatus == '0' || mchtInfoChg.archiveDealStatus == null}">备注/驳回原因</c:if><c:if test="${mchtInfoChg.archiveDealStatus == '2'}">驳回原因</c:if><span style="color:red;">*</span></td>
          		<td><textarea style="resize:none;" rows="7" cols="60" id="archiveDealRemarks" name="archiveDealRemarks" maxlength="256">${mchtInfoChg.archiveDealRemarks}</textarea></td>
          	</tr>
          	<c:if test="${mchtInfoChg.archiveDealStatus == '2'}">
          		<tr>
          		<td class="title">内部备注</td>
          		<td><textarea style="resize:none;" rows="7" cols="60" id="archiveDealInnerRemarks" name="archiveDealInnerRemarks" maxlength="256">${mchtInfoChg.archiveDealInnerRemarks}</textarea></td>
          	</tr>
          	</c:if>       	
          	<tr>

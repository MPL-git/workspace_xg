...
$(function(){
	$("input[name='editDecorateModule']").live('click',function(){
		...
		$.ligerDialog.open({
			...
			title: title,
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/customPage/editDecorateModule.shtml?decorateAreaId=" + decorateAreaId+"&moduleType="+moduleType+"&seqNo="+seqNo+"&decorateModuleId="+decorateModuleId+"&decorateInfoId="+decorateInfoId,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
...
function insertSlidePicModule(){
	...
	var moduleType = 9;
	$.ajax({
		url : "${pageContext.request.contextPath}/customPage/insertSlidePicModule.shtml",
		type : 'POST',
		dataType : 'json',
		cache : false,
		async : false,
		...
		success : function(data) {
			if ("0000" == data.returnCode) {
				var picModuleDiv = '<div style="position: relative;width: 750px;height:500px;border:1px solid #000" name="moduleDiv" decorateModuleId = "">'+
						'<div style="position: absolute; top:0; left: 0;width: 100%; height: 30px;">'+
						'<div style="float: left;padding-top:7px;padding-left:12px;width: 90px;height: 23px;background-color: blue;color: white;">滑动图片模块</div>'+
						'<input type="button" style="margin-left: 20px;float:right;width: 70px;height: 30px;cursor: pointer;" value="删除" name="delete" onclick="del(0,this);">'+
						...
						'<div style="margin-top: 50px;margin-left: 10px;">'+
						'自定义图片，可以加超链接。&nbsp;&nbsp;尺寸：宽大于1242像素，高度不限，大小：500Kb以内'+
						'</div>'+
						'</div>'+
						'<img src="" style="width: 100%;">'+
						'</div>'+
			...
			<div class="search-tr"  > 
				<div style="display: inline-flex;">
					<div style="padding-left: 10px;">
						<input type="button" style="width: 90px;height: 30px;cursor: pointer;" value="插入图片模块" onclick="insertPicModule();">
					</div>
					<div style="padding-left: 10px;">
						<input type="button" style="width: 100px;height: 30px;cursor: pointer;" value="插入滑动图片模块" onclick="insertSlidePicModule();">
					</div>
					<div style="padding-left: 10px;">
						<input type="button" style="width: 90px;height: 30px;cursor: pointer;" value="插入商品模块" onclick="insertProductModule();">
					</div>
					<div style="padding-left: 10px;">
						<input type="button" style="width: 90px;height: 30px;cursor: pointer;" value="插入特卖模块" onclick="insertActivityAreaModule();">
					</div>
					<div style="padding-left: 10px;">
						<input type="button" style="width: 90px;height: 30px;cursor: pointer;" value="插入商品列表" onclick="insertProductListModule();">
					</div>
					<div style="padding-left: 10px;">
						<input type="button" style="width: 90px;height: 30px;cursor: pointer;" value="插入特卖列表" onclick="insertActivityListModule();">
					</div>
					<div style="padding-left: 10px;">

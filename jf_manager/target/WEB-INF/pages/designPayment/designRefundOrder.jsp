...
$(function(){
	$("#export").bind('click',function(){
		...
		location.href="${pageContext.request.contextPath}/designRefundOrder/count/export.shtml?pay_date_begin="+pay_date_begin+"&pay_date_end="+pay_date_end;
	});
...
function designRefundOrderDtl(payDate,paymentId) {
	$.ligerDialog.open({
 		height: $(window).height()-100,
		width: $(window).width()-100,
		title: "设计退款明细",
		name: "INSERT_WINDOW",
		url: "${pageContext.request.contextPath}/designRefundOrderDtl/list.shtml?paymentId=" + paymentId+"&payDate="+payDate,
		showMax: true,
		showToggle: false,
		showMin: false,
		isResize: true,
...
function checkSearchCondition(){
  ...
  var listConfig={
	  ...
	  url:"/designRefundOrder/count/data.shtml",
					 	 ...
					 	 { display: '支付宝笔数',name:'', render: function (rowdata, rowindex) {
							  ...
							  if(zfbCount != ''){
								  if(Number(zfbCount) > Number(rowdata.zfbCount) ) {
									  html.push("<a href='javascript:;' style='color:green;' onclick='designRefundOrderDtl(\""+ rowdata.eachDay +"\", 2);'>"+ rowdata.zfbCount +"</span>");
								  }else if(Number(zfbCount) < Number(rowdata.zfbCount) ) {
									  html.push("<a href='javascript:;' style='color:red;' onclick='designRefundOrderDtl(\""+ rowdata.eachDay +"\", 2);'>"+ rowdata.zfbCount +"</span>");
								  }else {
									  html.push("<a href='javascript:;' onclick='designRefundOrderDtl(\""+ rowdata.eachDay +"\", 2);'>"+ rowdata.zfbCount +"</a>");
								  }
							  }else {
								  html.push("<a href='javascript:;' onclick='designRefundOrderDtl(\""+ rowdata.eachDay +"\", 2);'>"+ rowdata.zfbCount +"</a>");
							  }
		                ...
		                { display: '支付宝金额',render: function (rowdata, rowindex) {
		                	...
		                	if(zfbAmount != ''){
		                		if(Number(zfbAmount) > Number(formatMoney(rowdata.zfbAmount, 2)) ) {
		                			html.push("<a href='javascript:;' style='color:green;' onclick='designRefundOrderDtl(\""+ rowdata.eachDay +"\", 2);'>"+ formatMoney(rowdata.zfbAmount, 2) +"</a>");
			            		}else if(Number(zfbAmount) < Number(formatMoney(rowdata.zfbAmount, 2)) ) {
			            			html.push("<a href='javascript:;' style='color:red;' onclick='designRefundOrderDtl(\""+ rowdata.eachDay +"\", 2);'>"+ formatMoney(rowdata.zfbAmount, 2) +"</a>");
			            		}else {
			            			html.push("<a href='javascript:;' onclick='designRefundOrderDtl(\""+ rowdata.eachDay +"\", 2);'>"+ formatMoney(rowdata.zfbAmount, 2) +"</a>");
			            		}
		                	}else {
		                		html.push("<a href='javascript:;' onclick='designRefundOrderDtl(\""+ rowdata.eachDay +"\", 2);'>"+ formatMoney(rowdata.zfbAmount, 2) +"</a>");
		                	}

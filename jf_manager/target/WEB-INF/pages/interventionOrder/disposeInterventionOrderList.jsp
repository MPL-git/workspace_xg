...
<script type="text/javascript">
	...
	function updatePlatformProcessor(interventionOrderId) {
		$.ajax({
			type : 'post',
			url : '${pageContext.request.contextPath}/interventionOrder/updateInterventionOrder.shtml?statusFlag=7',
			data : {
				interventionOrderId : interventionOrderId
			},
			dataType : 'json',
			success : function(data) {
				if (data == null || data.code != 200) {
					...
					$("#updateDate-" + interventionOrderId).html(
							data.updateDate);
					$("#platformProcessor-" + interventionOrderId)
							.html(data.platformProcessor);
				}
			},
			error : function(e) {
	...
	var listConfig = {
		url : "/interventionOrder/getInterventionOrderList.shtml?statusFlag=7",
		btnItems : [],
		listGrid : {
			columns : [{display:'申请介入编号', name:'interventionCode', align:'center', isSort:false, width:180, render: function (rowdata, rowindex) {
					...
					},
					{
						...
						width : 100,
						render : function(rowdata, rowindex) {
							if (rowdata.platformProcessorName != null) {
								return "<span id='platformProcessor-"+rowdata.id+"'>"
										+ rowdata.platformProcessorName
										+ "</span>";
							} else {
								return "<span id='platformProcessor-"+rowdata.id+"'></span>";
							}
						}
					},
					{
						...
						width : 100,
						render : function(rowdata, rowindex) {
							return "<a href=\"javascript:updatePlatformProcessor("
									+ rowdata.id + ");\">受理</a>";
						}
					} ],

...
<script type="text/javascript">
	...
	$(function(){
		...
		$("[name='status']").bind("click",function() {
			 ...
			 if(status == '8') { //8 已结案
				 $("#recordOfCases-span").html("结案记录");
				 $("#recordOfCases").html("${interventionOrderCustom.recordOfCases }");
			 }else {
				 $("#recordOfCases-span").html("不同意理由");
				 $("#recordOfCases").html("");
			 }
		});
	...
	function submitFun(interventionOrderId) {
		var status = $("[name='status']:checked").val();
		var recordOfCases = $("#recordOfCases").val();
		$.ajax({
			type: 'post',
			url: '${pageContext.request.contextPath}/interventionOrder/updateInterventionOrder.shtml?statusFlag=5',
			data: {interventionOrderId:interventionOrderId, status:status, recordOfCases:recordOfCases},
			dataType: 'json',
			success: function(data) {
				if(data == null || data.code != 200) {
					commUtil.alertError(data.msg);
...
<body>
	...
	<div class="title-top">
		...
		<div class="marT10">
			<span class="marL20"><input type="radio" name="status" value="8" checked="checked" >同意结案</span>
			<span class="marL20"><input type="radio" name="status" value="6" >不同意结案</span>
		</div>
		<table class="gridtable marT10 status-table">
           	<tr>
               <td class="title title-width">
               ...
               <td>
               		<textarea rows="5" cols="100" id="recordOfCases" name="recordOfCases" >${interventionOrderCustom.recordOfCases }</textarea>
               </td>
			</tr>
        </table>
	</div>

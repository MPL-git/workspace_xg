...
<head>
 ...
 <script type="text/javascript">
	...
	//上架或下架
	function updateStatus(mallCategoryItemId, status, itemName, itemPic) {
		if(status == '1') {
			if(itemName == '' || itemPic == '' ) {
				commUtil.alertError("三级分类名称或三级分类图片不能为空，请先设置！");
				return;
			}
		}
		var titleStr = '是否上架？';
	...
	function addMallCategoryItem() {
		 $.ligerDialog.open({
				...
				title: "添加三级分类",
				name: "INSERT_WINDOW",
				url: "${pageContext.request.contextPath}/mallCategoryItem/productTypeManager.shtml?mallCategoryModuleId=${mallCategoryModuleId }",
				showMax: true,
				showToggle: false,
				showMin: false,
				isResize: true,
	...
	function editMallCategoryItem(mallCategoryItemId) {
 	...
 	var listConfig={
	      ...
	      btnItems:[
				{ text: '添加三级分类', icon:'add', id:'add', dtype:'win', click:addMallCategoryItem }          
	      ],
	      listGrid:{ columns: [
	                        ...
	                        {display:'三级分类名称', name:'itemName', align:'center', isSort:false, width:180},
	                        {display:'三级分类图片', name:'', align:'center', isSort:false, width:100, render:function(rowdata, rowindex) {
								var html = [];
	                        ...
	                        {display:'绑定类目ID', name:'itemLinkValue', align:'center', isSort:false, width:100},
	                        {display:'绑定类目', name:'productTypeName', align:'center', isSort:false, width:180},
	                        /* {display:'商品数', name:'productTypeCount', align:'center', isSort:false, width:100}, */
	                        ...
	                        {display:'操作', name:'', align:'center', isSort:false, width:260, render: function(rowdata, rowindex) {
								var html = [];
								var itemPic = rowdata.itemPic==null?'':rowdata.itemPic;
								var itemName = rowdata.itemName==null?'':rowdata.itemName;
								html.push("<a href=\"javascript:editMallCategoryItem(" + rowdata.id + ");\">【编辑】</a>");
								if(rowdata.status == '0' ) {
									html.push("<a href=\"javascript:updateStatus(" + rowdata.id + ", '1', '" + itemName + "', '" + itemPic + "');\">【上架】</a>");	
									html.push("<a href=\"javascript:delMallCategoryItem(" + rowdata.id + ");\">【删除】</a>");
								}else {
									html.push("<a href=\"javascript:updateStatus(" + rowdata.id + ", '0', '" + itemName + "', '" + itemPic + "');\">【下架】</a>");	
								}
							    return html.join("");
							}}
			         ], 
...
<body style="padding: 0px; overflow: hidden;">
	...
	<form id="dataForm" runat="server">
		<div class="search-pannel">
			<div class="search-tr"  > 
				<div class="search-td">
					<div class="search-td-label"  >绑定类目ID：</div>
					<div class="search-td-combobox-condition" >
						<input type="text" id="itemLinkValue" name="itemLinkValue" >
					</div>
				</div>
				<div class="search-td">
					<div class="search-td-label"  >三级分类名称：</div>
					<div class="search-td-combobox-condition" >

...
function save(){
	...
	if(innerRemarks.length>150){
		commUtil.alertError("字数请控制在150字以内");
		return;
	}
	$.ajax({
		url : "${pageContext.request.contextPath}/mcht/close/saveInnerRemarks.shtml",
		type : 'POST',
		dataType : 'json',
		cache : false,
		async : false,
		data : {mchtCloseId:mchtCloseId,innerRemarks:innerRemarks},
		timeout : 30000,
...
<form id="violateOrderForm" method="post" >
	<input type="hidden" id="mchtCloseId" name="mchtCloseId" value="${mca.id}">
		...
		<textarea  id="innerRemarks"  name ="innerRemarks"   rows="10" cols="65"  style="resize:none;">${mca.innerRemarks }</textarea>
		</div>

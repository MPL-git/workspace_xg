 ...
 <script type="text/javascript">
	 $(function() {
		...
		$("#export").bind('click', function(){
			$("#dataForm").attr("action","${pageContext.request.contextPath}/mcht/exportMchtListExcel.shtml");
			$("#dataForm").submit();
		});
	 ...
	 //导出网表
	 function exportFun(){
		$("#dataForm").attr("action","${pageContext.request.contextPath}/mcht/exportMchtNetlistExcel.shtml");
		$("#dataForm").submit();
	 }
 	 ...
 	 var listConfig={
	      ...
	      listGrid:{ columns: [
	                        ...
	                        {display:'品牌/资质/开通',name:'mchtProductBrandList', align:'center', isSort:false, width:160, render:function(rowdata, rowindex) {
		                		...
		                		for (var i = 0; i < mchtProductBrandList.length; i++) {
									...
									if(mchtProductBrandList[i].brandName!=null){
										html.push("<a href=\"javascript:editMchtProductBrand(" + mchtProductBrandList[i].id + ");\">"+mchtProductBrandList[i].brandName+"</a>");
									}else{
										html.push("<a href=\"javascript:editMchtProductBrand(" + mchtProductBrandList[i].id + ");\">"+mchtProductBrandList[i].productBrandName+"</a>");
									}
	                        ...
	                        {display:'查看详情',name:'OPER1', align:'center', isSort:false, width:100, render:function(rowdata, rowindex) {
	                        	var html = [];
							    html.push("<a href=\"javascript:editMchtBaseInfo(" + rowdata.mchtInfoCustom.id + ");\">店铺主信息</a></br>"); 
							    html.push("<a href=\"javascript:editMchtFinanceInfo(" + rowdata.mchtInfoCustom.id + ");\">财务信息</a></br>"); 
							    html.push("<a href=\"javascript:mchtContact(" + rowdata.mchtInfoCustom.id + ");\">商家对接人</a></br>"); 
							    html.push("<a href=\"javascript:mchtPlatformContact(" + rowdata.mchtInfoCustom.id + ");\">平台对接人</a></br>"); 
							    return html.join("");
	                        }},
	...
	<form id="dataForm" runat="server">
			...
			<div class="search-tr"  > 
				<div class="search-td">
					...
					<div class="l-panel-search-item" >
						<input type="text" class="dateEditor" id="startCreateDate" name="startCreateDate" value="${DateBegin}">
				 	</div>
				 	<div class="l-panel-search-item" >&nbsp;&nbsp;至：</div>
				</div>
				<div class="search-td">
					<div class="l-panel-search-item">
						<input type="text" class="dateEditor" id="endCreateDate" name="endCreateDate" value="${DateEnd}">
				 	</div>
				</div>
				<div class="search-td">
		...
		<div class="search-pannel">
			<div class="search-tr"  > 
		       ...
		       <div class="search-td">
					<div class="search-td-label"  >招商标记：</div>
					<div class="search-td-combobox-condition" >
						<select id="isSpeciallyApprovedDesc" name="isSpeciallyApprovedDesc" style="width: 135px;" >
							<option value="">请选择...</option>
							<c:forEach var="isSpeciallyApprovedDesc" items="${mchtproductbrandlist}">
							<option value="${isSpeciallyApprovedDesc.statusValue}">${isSpeciallyApprovedDesc.statusDesc}</option>
							</c:forEach>
						</select>
					</div>
				</div>
				...
				<div class="search-td-search-btn" style="display: inline-flex;" >
					...
					<div style="padding-left: 10px;">
						<input type="button" style="width: 60px;height: 25px;cursor: pointer;" value="导出网表" onclick="exportFun();">
					</div>
				</div>
			</div>
		</div>

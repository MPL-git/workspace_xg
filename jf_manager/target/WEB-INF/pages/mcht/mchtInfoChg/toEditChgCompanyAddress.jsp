...
$(function(){
	$("#confirm").bind('click',function(){
		...
		submitting = true;
		$.ajax({
			url : "${pageContext.request.contextPath}/mcht/mchtInfoChg/saveChgCompanyAddress.shtml",
			type : 'POST',
			dataType : 'json',
			cache : false,
			async : false,
			data : {"id":id,"companyAddress":companyAddress},
			timeout : 30000,
...
<body>
	...
	<input type="hidden" id="mchtChgId" value="${id}">
		<table class="gridtable">
          	<tr>
          		<td class="title"><c:if test="${settledType eq 1}">住所地址</c:if><c:if test="${settledType eq 2}">经营住所</c:if></td>
          		<td>
          			<textarea rows="5" cols="90" id="companyAddress" name="companyAddress">${companyAddress}</textarea>
          		</td>
          	</tr>
	        <tr>

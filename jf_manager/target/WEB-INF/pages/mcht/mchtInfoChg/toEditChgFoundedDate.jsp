...
$(function(){
	...
	$("#confirm").bind('click',function(){
		if($("#foundedDate").val() == ""){
			$.ligerDialog.warn('发照不可为空');
			return;
		}
		var id = $("#mchtChgId").val();
		var foundedDate = $("#foundedDate").val();
		$.ajax({
			url : "${pageContext.request.contextPath}/mcht/mchtInfoChg/saveChgFoundedDate.shtml",
			type : 'POST',
			dataType : 'json',
			cache : false,
			async : false,
	...
	<input type="hidden" id="mchtChgId" value="${id}">
		<table class="gridtable">
          	<tr>
          		...
          		<td>
          			<input type="text" id="foundedDate" name="foundedDate" class="dateEditor" placeholder="请选择" style="width: 135px;" value="<fmt:formatDate value="${foundedDate}" pattern="yyyy-MM-dd"/>"/>
          		</td>
          	</tr>
	        <tr>

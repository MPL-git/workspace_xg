...
<head>
 ...
 <script type="text/javascript">
	 $(function() {
 	 ...
 	 var listConfig={
	      url:"/mcht/singleProductActivityCloseList.shtml?mchtId=${mchtId }&productBrandId=${productBrandId }",
	      btnItems:[{ text:'全部驳回', icon:'modify', dtype:'win', click:singleProductActivityClose }],
	      listGrid:{ columns: [
							{display:'活动ID', name:'id', align:'center', isSort:false, width:80},   
							{display:'活动类型', name:'typeDesc', align:'center', isSort:false, width:100},   
 	 ...
 	 function singleProductActivityClose(){
 		var mchtId = ${mchtId };
 		var productBrandId = ${productBrandId };
 		$.ligerDialog.confirm('您确定将单品活动全部驳回吗？', function (yes) { 
			if(yes) {
				$.ajax({
		 			url : '${pageContext.request.contextPath}/mcht/singleProductActivityClose.shtml',
		 			type : 'POST',
		 			dataType : 'json',

...
$(function(){
	$("#confirm").bind('click',function(){
		...
		$.ajax({
			url : "${pageContext.request.contextPath}/mchtSettleOrder/batchPay.shtml",
			type : 'POST',
			dataType : 'json',
			cache : false,
			async : false,
			data : {"payCode":payCode,"ids":ids},
			timeout : 30000,
			success : function(data) {
				if ("0000" == data.returnCode) {
					commUtil.alertSuccess("排款成功");
					setTimeout(function(){
	...
	<input type="hidden" id="ids" name="ids" value="${ids}">
		<table class="gridtable">
          	<tr>
          		<td>你选择共：</td>
          		<td>
	          		${totalCount}家，需支付：${totalNeedPayAmount}元。
				</td>
          	</tr>
          	<tr>
          		<td>付款编号：</td>
          		<td>
	          		<input type="text" name="payCode" id="payCode">
				</td>
          	</tr>
          	<tr>
          		<td colspan="5">
					<div style="margin-left: 30%">
						<input id="confirm" type="button" style="float:left;width: 85px;" class="l-button l-button-test" value="确认批量付款"/>
					</div>

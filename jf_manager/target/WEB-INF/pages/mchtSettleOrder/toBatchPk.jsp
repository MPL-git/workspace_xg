...
$(function(){
	$("#confirm").bind('click',function(){
		var pay_ready_date = $("#pay_ready_date").val();
		var ids = $("#ids").val();
		$.ajax({
			url : "${pageContext.request.contextPath}/mchtSettleOrder/batchPk.shtml",
			type : 'POST',
			dataType : 'json',
			cache : false,
			async : false,
			data : {"pay_ready_date":pay_ready_date,"ids":ids},
			timeout : 30000,
	...
	<input type="hidden" id="ids" name="ids" value="${ids}">
		<table class="gridtable">
          	<tr>
          		<td>批量排款日期：</td>
          		<td>
	          		<input type="text" id="pay_ready_date" name="pay_ready_date" value="${payReadyDate}"/>
					<script type="text/javascript">
						$(function() {
							$("#pay_ready_date").ligerDateEditor( {
								showTime : false,
								labelWidth : 150,
								width:150,
								labelAlign : 'left'

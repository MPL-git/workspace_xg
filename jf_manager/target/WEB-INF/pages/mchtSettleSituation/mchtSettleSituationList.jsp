...
function exportDetail(mchtId) {
	...
	location.href="${pageContext.request.contextPath}/mchtSettleSituation/export.shtml?mchtId="+mchtId+"&createDateBegin="+createDateBegin+"&createDateEnd="+createDateEnd;	
}
 ...
 function popDealOrderDtl(mchtCode,date_begin,date_end){
	 $.ligerDialog.open({
			height: $(window).height()-10,
			width: $(window).width()-10,
			title: "《POP交易商品明细》",
			name: "INSERT_WINDOW",
			url: "${pageContext.request.contextPath}/popDealOrderDtl/list.shtml?mchtCode="+mchtCode+"&date_begin="+date_begin+"&date_end="+date_end,
			showMax: true,
			showToggle: false,
			showMin: false,
			isResize: true,
 ...
 var listConfig={
     url:"/mchtSettleSituation/data.shtml",
     listGrid:{ columns: [
		                ...
		                { display: '商品交易完成数量',name:'productNum'},
		                { display: '订单金额（元）',render: function (rowdata, rowindex) {
		                	if(rowdata.orderAmount){
								...
								return '<a href="javascript:;" onclick="popDealOrderDtl('+"'"+rowdata.mchtCode+"'"+','+"'"+date_begin+"'"+','+"'"+date_end+"'"+');">'+formatMoney(rowdata.orderAmount,2)+'</a>';	                		
		                	}else{
		                		return "0.00";
		                	}
		                }},
		                { display: '技术服务费（元）', render: function (rowdata, rowindex) {
		                ...
		                { display: '商品结算金额（元）',render: function (rowdata, rowindex) {
		                ...
		                { display: '应付款金额(元)',render: function (rowdata, rowindex) {
		                ...
		                { display: '付款金额(元)',render: function (rowdata, rowindex) {

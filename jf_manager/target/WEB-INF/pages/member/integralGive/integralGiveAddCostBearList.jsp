...
<script type="text/javascript">
	...
	function updateIsDepositDeduct(id, isDepositDeduct) {
		$.ligerDialog.confirm("保证金扣款，是否修改为已扣？", function(yes) {
			if(yes) {
				$.ajax({
					url : "${pageContext.request.contextPath }/member/integralGive/updateIsDepositDeduct.shtml",
					data : {id : id, isDepositDeduct : isDepositDeduct},
					dataType : 'json',
	...
	var listConfig = {
		url:"/member/integralGive/integralGiveList.shtml?auditStatus=1,3",
		btnItems:[],
		listGrid:{ columns: [  
                  ...
                  {display:'保证金扣款',name:'isDepositDeductDesc', align:'center', isSort:false, render: function(rowdata, rowindex) {
                  		if(rowdata.costBear == 2 && rowdata.isDepositDeduct == 0) {
                  			return "<a href=\"javascript:updateIsDepositDeduct(" + rowdata.id + ", 1);\">" + rowdata.isDepositDeductDesc + "</a>"
                  		}else {
                  			return rowdata.isDepositDeductDesc;
                  		}
                  }},
                  ...
                  {display:'操作',name:'auditOrRetrial', align:'center', isSort:false, width:180, render: function(rowdata, rowindex) {
                	  	if(rowdata.auditStatus == 0) {
                	  		return "<a href=\"javascript:addOrShowOrAuditIntegralGive(" + rowdata.id + ", 3);\">审核</a>"
                	  	}else {
                	  		return "<a href=\"javascript:addOrShowOrAuditIntegralGive(" + rowdata.id + ", 1);\">查看</a>"
                	  	}
				  }}
               ],   
...
<!-- 								<option value="${auditStatus.statusValue}" <c:if test="${auditStatus.statusValue == 0 }">selected</c:if> >${auditStatus.statusDesc}</option> -->
<!-- 							</c:forEach> -->
<!-- 						</select> -->
<!-- 					</div> -->
<!-- 				</div> -->
					...
					<div class="search-td-combobox-condition" >
						<select  id="isDepositDeduct" name="isDepositDeduct" style="width: 135px;">
							...
							<c:forEach var="isDepositDeduct" items="${isDepositDeductList}">
								<option value="${isDepositDeduct.statusValue}" <c:if test="${isDepositDeduct.statusValue == 1 }">selected</c:if> >${isDepositDeduct.statusDesc}</option>
							</c:forEach>
						</select>
					</div>
				</div>

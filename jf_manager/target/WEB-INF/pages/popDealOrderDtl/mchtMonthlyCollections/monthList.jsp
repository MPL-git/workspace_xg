...
$(function(){
	$("#export").bind('click',function(){
		...
		var year = $("#year").val();
		var month = $("#month").val();
		location.href="${pageContext.request.contextPath}/popDealOrderDtl/mchtMonthlyCollections/monthExport.shtml?mchtCode="+mchtCode+"&name="+name+"&year="+year+"&month="+month;
	});
});
...
function toEdit(id){
	$.ligerDialog.open({
		...
		title: "修改折让",
		name: "INSERT_WINDOW",
		url: "${pageContext.request.contextPath}/popDealOrderDtl/mchtMonthlyCollections/toEdit.shtml?id=" + id,
		showMax: true,
		showToggle: false,
		showMin: false,
		isResize: true,
...
function mchtInfo(mchtId){
 ...
 var listConfig={
     url:"/popDealOrderDtl/mchtMonthlyCollections/monthData.shtml",
     listGrid:{ columns: [
		                ...
		                { display: '数量',render: function (rowdata, rowindex) {
		                	var productCount = rowdata.collectionProductCount-rowdata.refundProductCount;
							return productCount+"";	                		
		                }},
		                { display: '商品金额',render: function (rowdata, rowindex) {
		                	if(rowdata.collectionProductAmount){
		                		if(rowdata.refundProductAmount){
			                		return formatMoney(rowdata.collectionProductAmount-rowdata.refundProductAmount,2);
		                		}else{
			                		return formatMoney(rowdata.collectionProductAmount,2);
		                		}
		                	}else{
		                		return "0.00";
		                	}
		                ...
		                { display: '商家优惠', render: function (rowdata, rowindex) {
		                	if(rowdata.collectionMchtPreferential){
		                		if(rowdata.refundMchtPreferential){
									return formatMoney(rowdata.collectionMchtPreferential-rowdata.refundMchtPreferential,2);	                		
		                		}else{
									return formatMoney(rowdata.collectionMchtPreferential,2);	                		
		                		}
		                	}else{
		                		return "0.00";
		                	}
		                ...
		                { display: '平台优惠',render: function (rowdata, rowindex) {
		                	if(rowdata.collectionPlatformPreferential){
		                		if(rowdata.refundPlatformPreferential){
			                		return formatMoney(rowdata.collectionPlatformPreferential-rowdata.refundPlatformPreferential,2);
		                		}else{
			                		return formatMoney(rowdata.collectionPlatformPreferential,2);
		                		}
		                	}else{
		                		return "0.00";
		                	}
		                ...
		                { display: '积分优惠',render: function (rowdata, rowindex) {
		                	if(rowdata.collectionIntegralPreferential){
		                		if(rowdata.refundIntegralPreferential){
			                		return formatMoney(rowdata.collectionIntegralPreferential-rowdata.refundIntegralPreferential,2);
		                		}else{
			                		return formatMoney(rowdata.collectionIntegralPreferential,2);
		                		}
		                	}else{
		                		return "0.00";
		                	}
		                ...
		                { display: '实收金额',render: function (rowdata, rowindex) {
		                	if(rowdata.orderAmount){
		                		if(rowdata.returnAmount){
			                		return formatMoney(rowdata.orderAmount-rowdata.returnAmount,2);
		                		}else{
		                ...
		                { display: 'POP技术服务费',render: function (rowdata, rowindex) {
		                	if(rowdata.collectionCommissionAmount){
		                		if(rowdata.refundCommissionAmount){
		                			return formatMoney(rowdata.collectionCommissionAmount-rowdata.refundCommissionAmount,2);
		                		}else{
		                			return formatMoney(rowdata.collectionCommissionAmount,2);
		                		}
		                	}else{
		                		return "0.00";
		                	}
		                ...
		                { display: 'POP结算金额',render: function (rowdata, rowindex) {
		                	if(rowdata.settleAmount){
		                		if(rowdata.returnOrderAmount){
									return formatMoney(rowdata.settleAmount-rowdata.returnOrderAmount,2);	                		
		                		}else{
		                ...
		                { display: '折让',render: function (rowdata, rowindex) {
		                	if(rowdata.discount){
		                		return formatMoney(rowdata.discount,2)+'<a href="javascript:;" onclick="toEdit('+rowdata.id+');">【修改】</a>';
		                	}else{
		                		return '0.00'+'<a href="javascript:;" onclick="toEdit('+rowdata.id+');">【修改】</a>';
		                	}
		                }},

...
<script type="text/javascript">
	...
	<form name="form1" class="form1" method="post" id="form1" action="${pageContext.request.contextPath}/memberSignIn/addblackList.shtml" >
			...
			<tr>
				...
				<td align="left" class="l-table-edit-td" >
					<select id="blackType" name="blackType" style="width: 160px;" validate="{required:true}" >
						<c:if test="${empty supplementaryCard}">
							<c:if test="${empty newSignIn}">
								<option value="1" >现金签到</option>
							</c:if>
							<c:if test="${not empty newSignIn}">
								<option value="4" >新版签到</option>
							</c:if>
						</c:if>
						<c:if test="${not empty supplementaryCard}">
							<option value="3" >补签卡邀请</option>
						</c:if>
					</select>
				</td>
           	</tr>

 ...
 <script type="text/javascript">
	 ...
	 function staffreply(id) {
		 $.ligerDialog.open({
				...
				title: "详情",
				name: "INSERT_WINDOW",
				url: "${pageContext.request.contextPath}/staffOpinionFeedback/editstaffreply.shtml?id="+id,
				showMax: true,
				showToggle: false,
				showMin: false,
				isResize: true,
 	 ...
 	 var listConfig={ 
	      url:"/staffOpinionFeedback/staffOpinionFeedbackdata.shtml",
	      btnItems:[],
	      listGrid:{ columns: [
						   ...
						   {display:'发送给谁',name:'organizationName', align:'center', isSort:false, width:150,render:function(rowdata, rowinde){
						   ...
						   {display:'反馈人',name:'', align:'center', isSort:false, width:150,render:function(rowdata, rowindex){
						    if (rowdata.isanonymous=='1') {
								return "匿名用户";
							}else if (rowdata.isanonymous=='2') {
			               ...
			               {display:'创建时间',name:'', align:'center', isSort:false, width:180, render:function(rowdata, rowindex) {
							...
							{display:'操作',name:'', align:'center', isSort:false, width:120, render: function(rowdata, rowindex) {
								var html = [];
								html.push("<a href='javascript:;' onclick='staffreply(" + rowdata.id + ");'>详情</a></br>");												
							    return html.join("");
							}}
			         ], 

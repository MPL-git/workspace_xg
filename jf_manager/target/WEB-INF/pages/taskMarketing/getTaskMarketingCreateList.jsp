...
<head>
 ...
 <script type="text/javascript">
	...
	//添加任务
 	function addTaskMarketing() {
 		$.ligerDialog.open({
 			height: $(window).height() - 100,
 			width: $(window).width() - 100,
 			title: "添加任务",
 			name: "INSERT_WINDOW",
 			url: "${pageContext.request.contextPath}/taskMarketing/eidtTaskMarketingManager.shtml",
 			showMax: true,
 			showToggle: false,
 			showMin: false,
 			isResize: true,
	 ...
	 //编辑任务
	 function updateTaskMarketing(taskMarketingId) {
 		$.ligerDialog.open({
 			height: $(window).height() - 100,
 			width: $(window).width() - 100,
 			title: "编辑任务",
 			name: "INSERT_WINDOW",
 			url: "${pageContext.request.contextPath}/taskMarketing/eidtTaskMarketingManager.shtml?taskMarketingId="+taskMarketingId,
 			showMax: true,
 			showToggle: false,
 			showMin: false,
 			isResize: true,
	 ...
	 function auditTask(taskId, taskStatus) {
 	 ...
 	 var listConfig={
	      ...
	      btnItems:[
	  		      { text: '添加任务', icon:'add', id:'add', dtype:'win', click:addTaskMarketing }
	  	      ],
	      listGrid:{ columns: [
							...
							{display:'执行人',name:'executeStaffName', align:'center', isSort:false, width:100},
							...
							{display:'操作',name:'', align:'center', isSort:false, width:180, render:function(rowdata,rowindex){
								var html = [];
								if(rowdata.taskStatus == '0' || rowdata.taskStatus == '5' || rowdata.taskStatus == '6' ) {
									html.push("<a href=\"javascript:updateTaskMarketing(" + rowdata.id + ");\">【编辑】</a>");
								}
								if(rowdata.taskStatus == '0' ) {
									html.push("<a href=\"javascript:auditTask(" + rowdata.taskId + ", 1);\">【提交审核】</a>");
								}
								if(rowdata.taskStatus != '4' ) {
					...
					<div class="search-td-combobox-condition" >
						<select id="taskStatus" name="taskStatus" style="width: 135px;" >
							...
							<c:forEach items="${taskStatusList }" var="taskStatus">
								<c:if test="${taskStatus.statusValue != '7' }">
									<option value="${taskStatus.statusValue }" <c:if test="${taskStatus.statusValue == '0' }">selected="selected"</c:if> >${taskStatus.statusDesc }</option>
								</c:if>
							</c:forEach>
						</select>
					</div>

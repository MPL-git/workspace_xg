	...
	//添加任务
 	function addTaskSms() {
 		$.ligerDialog.open({
 			...
 			title: "添加任务",
 			name: "INSERT_WINDOW",
 			url: "${pageContext.request.contextPath}/taskSms/eidtTaskSmsManager.shtml",
 			showMax: true,
 			showToggle: false,
 			showMin: false,
 			isResize: true,
	 ...
	 //编辑任务
	 function updateTaskSms(taskSmsId) {
 		$.ligerDialog.open({
 			height: $(window).height() - 100,
 			width: $(window).width() - 100,
 			title: "编辑任务",
 			name: "INSERT_WINDOW",
 			url: "${pageContext.request.contextPath}/taskSms/eidtTaskSmsManager.shtml?taskSmsId="+taskSmsId,
 			showMax: true,
 			showToggle: false,
 			showMin: false,
 			isResize: true,
	 ...
	 function auditTask(taskId, taskStatus) {
 	 ...
 	 var listConfig={
	      ...
	      btnItems:[
	  		      { text: '添加任务', icon:'add', id:'add', dtype:'win', click:addTaskSms }
	  	      ],
	      listGrid:{ columns: [
							...
							{display:'操作',name:'', align:'center', isSort:false, width:180, render:function(rowdata,rowindex){
								...
								if(rowdata.taskStatus == '0' || rowdata.taskStatus == '5' || rowdata.taskStatus == '6' ) {
									html.push("<a href=\"javascript:updateTaskSms(" + rowdata.id + ");\">【编辑】</a>");
								}

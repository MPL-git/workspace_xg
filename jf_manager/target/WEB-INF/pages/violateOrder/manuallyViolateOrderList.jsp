...
function editManuallyViolateOrder(id) {
	var auditStatus = "0";
	$.ligerDialog.open({
		...
		title: "编辑人工发起违规",
		name: "INSERT_WINDOW",
		url: "${pageContext.request.contextPath}/violateOrder/editManuallyViolateOrder.shtml?id=" + id+"&auditStatus="+auditStatus,
		showMax: true,
		showToggle: false,
		showMin: false,
		isResize: true,
...
//子框口调用方法
function childFun() {
	$.ligerDialog.success('添加人工发起违规成功！', function() {
		$("#searchbtn").click();
	});	
}
 ...
 var listConfig={
     ...
     url:"/violateOrder/manuallyData.shtml",
     btnItems:[{ text: '添加人工发起违规', icon: 'add', dtype:'win',  click: itemclick, url: "/violateOrder/editManuallyViolateOrder.shtml", seqId:"", width: 1200, height: 800}],
     listGrid:{ columns: [
		                ...
		                { display: '发布审核状态', render: function (rowdata, rowindex) {
		                	if(rowdata.auditStatus == "0"){
		                		...
		                		return '<span style="color:#d43f3a;">'+rowdata.auditStatusDesc+'<a href="javascript:;" onclick="editManuallyViolateOrder('+rowdata.id+');">【修改】</a></span>';
		                	}
		                }}, 
 ...
 function excel(){  
	$("#dataForm").attr("action","${pageContext.request.contextPath}/violateOrder/exportViolateOrder.shtml?pageStatus=3");
	$("#dataForm").submit();
 }

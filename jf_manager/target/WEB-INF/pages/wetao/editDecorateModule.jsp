...
$.ajax({
        ...
        reader.onload = function(e) { 
        	...
        	image.onload = function() {
        		...
        		if(moduleType == 1){
        			...
        			}else if(this.height != 283){
        				commUtil.alertError("图片高度只能是283像素！");
        			}else{
        				ajaxFileUpload();
        			}
        		}
	...
	function saveMapInfo(_this) {
		...
		if(!testNumber(linkValue.value) && linkType.value!=9 && linkType.value!=14 && linkType.value!=13){
			commUtil.alertError("只能输入正整数！");
			return false;
		}
		if(linkType.value != 5 && linkType.value != 6 && linkType.value != 9 && linkType.value != 14){//排除5.栏目和6.一级分类和9.url链接 和14.关键字
			$.ajax({

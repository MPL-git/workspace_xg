...
<link href="${pageContext.request.contextPath}/css/work_search_form.css" rel="stylesheet" type="text/css" />
 ...
 <script type="text/javascript">
	 ...
	 $(function() {
		...
		/*$(".l-dialog-close").live("click", function(){//关闭弹出窗口时刷新父级页面
		 ...
		 $("#export").bind('click', function(){
			 ...
			 window.location.href="${pageContext.request.contextPath}/work/exportAllWorkOrders.shtml?startCreateDate="+startCreateDate
					 +"&endCreateDate="+endCreateDate+"&workType="+workType+"&status="+status+"&titleContent="+titleContent+"&id="+id+"&createBy="+createBy+"&staffId="+staffId
		 });
	 ...
	 function viewWork(id) {
		 $.ligerDialog.open({
				...
				title: "工单详情",
				name: "INSERT_WINDOW",
				url: "${pageContext.request.contextPath}/work/wholeviewWork.shtml?id="+id,
				showMax: true ,
				showToggle: false,
				showMin: false,
				isResize: true,
 	 ...
 	 var listConfig={ 
	      ...
	      listGrid:{ columns: [
				           ...
				           {display:'当前创建者',name:'createbyName', align:'center', isSort:false, width:100},
	...
	<form id="dataForm" runat="server" >
		<div class="search-pannel">
			  ...
			  <div class="search-td">
				...
				<div class="search-td">
					...
					<div class="search-td-combobox-condition" >
						<input type="text" id="relevantCode" name="relevantCode" style="width: 162px">
					</div>
				</div>
			   <div class="search-td">
			...
			<div class="search-tr"  >
			  ...
			  <div class="search-td">
			 	<div class="search-td-label" >工单创建者：</div>
			 	<div class="search-td-condition" >
				<select id="createBy" name="createBy" class="querysel">
					<option value="">请选择...</option>
					<c:forEach var="staffidListc" items="${staffidListc}">
					<c:if test="${staffidListc.create_by==staffId}">
					<option value="${staffId}">我创建的</option>
					</c:if>	
					<c:if test="${staffidListc.create_by!=staffId}">
					   <option value="${staffidListc.create_by}">${staffidListc.staff_name}</option>
				   </c:if>				
				   </c:forEach>	
				</select>
				</div>
				...
				<select id="staffId" name="staffId" class="querysel">
					...
					<c:forEach var="staffidList" items="${staffidList}">
				      <c:if test="${staffidList.staff_id==staffId}">
					   <option value="${staffId}">指派给我的</option>
				      </c:if>			      					
					  <c:if test="${staffidList.staff_id!=staffId}">
					   <option value="${staffidList.staff_id}">${staffidList.staff_name}</option>
				      </c:if>
				   </c:forEach>
				</select>
				</div>

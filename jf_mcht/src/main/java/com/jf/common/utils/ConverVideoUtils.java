...
/**
 * @ClassName ConverVideoUtils
 * @Description TODO
 * @author chengh
 * @date 2019年8月27日 下午6:24:58
 */
public class ConverVideoUtils {
	...
	private String videofolder = Const.videofolder; 		// 需要被转换格式的视频目录
	private String targetfolder = Const.targetfolder; 	// 转码后视频保存的目录
	private String ffmpegpath = Const.ffmpegpath;		 // ffmpeg.exe的目录
	private String mencoderpath = Const.mencoderpath; 	// mencoder的目录
	private String imageRealPath = Const.imageRealPath;   // 截图的存放目录
	...
	/**
	 * 转换视频格式
	 * @param String mchtId 商家ID
	 * @param String targetExtension 目标视频后缀名 .xxx
	 * @param boolean isDelSourseFile 转换完成后是否删除源文件
	 * @param String filename_extension 原视频文件后缀
	 * @return
	 */
	public boolean beginConver(String mchtId,String targetExtension, boolean isDelSourseFile,String filename_extension) {
		...
		//执行转码机制
		if (process(mchtId,targetExtension,isDelSourseFile)) {
			...
			String temppath=videofolder.concat(File.separator) + filerealname + "."+filename_extension;
	...
	public boolean processImg(String sourceVideoPath) {
		...
		commend.add("image2");
		commend.add("-y");
		commend.add(imageRealPath.concat(File.separator) + filerealname + ".jpg");		//生成截图xxx.jpg
	 ...
	 * @return
	 */
	private boolean process(String mchtId,String targetExtension, boolean isDelSourseFile) {
		...
		if (type == 0) {
			...
			status = processVideoFormat(mchtId,sourceVideoPath,targetExtension,isDelSourseFile);//可以指定转换为什么格式的视频
			...
			if (avifilepath == null){
				...
				status = processVideoFormat(mchtId,avifilepath,targetExtension,isDelSourseFile);
			}
		}
	 ...
	 * @param oldfilepath 绝对路径
	 * @param targetExtension 目标格式后缀名 .xxx
	 * @param isDelSourseFile 转换完成后是否删除源文件
	 * @return
	 */
	private boolean processVideoFormat(String mchtId,String oldfilepath, String targetExtension, boolean isDelSourceFile) {
		 
		System.out.println("调用了ffmpeg.exe工具");
		
		//先确保保存转码后的视频的文件夹存在
		String path = "";
		StringBuffer pathBuffer = new StringBuffer(targetfolder);
		pathBuffer.append("/"+mchtId+"/");
		...
		commend.add("-strict");
		commend.add("-2");
		commend.add(path + filerealname+targetExtension);  // //转码后的路径+名称，是指定后缀的视频
	 ...
	 * @return
	 */
	public static boolean processVideoFormat(String oldfilepath) {
		
		System.out.println("调用了ffmpeg.exe工具");
		
		//先确保保存转码后的视频的文件夹存在
		String path = oldfilepath.substring(0, oldfilepath.lastIndexOf("."));
		List<String> commend = new ArrayList<String>();
		commend.add("ffmpeg");		 //ffmpeg.exe工具地址D\:/ffmpeg-20190826-0821bc4-win64-static/bin/ffmpeg.exe
		commend.add("-i");
		commend.add(oldfilepath);			//源视频路径
		...
		commend.add("-strict");
		commend.add("-2");
		commend.add(path +".mp4");  // //转码后的路径+名称，是指定后缀的视频
		...
		try {
			...
			p.waitFor();		//进程等待机制，必须要有，否则不生成mp4！！！
			System.out.println("生成mp4视频为:"+path +".mp4");
			return true;
		} 
		catch (Exception e) {
			e.printStackTrace();

...
$(document).ready(function () {
	...
	$("#btn-out").on('click',function(){
		...
		location.href="${pageContext.request.contextPath}/dataCenter/exportProductSale.shtml?search_productName="+search_productName+"&search_productCode="+search_productCode+"&search_brandId="+search_brandId+"&timeBegin="+timeBegin+"&timeEnd="+timeEnd+"&search_activityType="+search_activityType+"&orderByType="+orderByType;
	});
    ...
    table = $('#datatable').dataTable({
        "ajax": function (data, callback, settings) {
            ...
            $.ajax({
                method: 'POST',
                url: '${ctx}/dataCenter/getProductSaleData',
                data: param,
                dataType: 'json'
        ...
        "columns": [
            ...
            {"data": "id","title":"商品","render": function (data, type, row, meta) {
            	var html = "";
                if(row.sku_pic!=null&&(row.sku_pic.indexOf("http") >= 0)){
                 	html = '<div><div class="width-60"><img src="'+row.sku_pic+'"></div><div class="width-226" style="width:390px;"><p class="ellipsis" title="'+row.brand_name+"&nbsp;&nbsp;"+row.product_name+"&nbsp;&nbsp;"+row.art_no+'">'+row.brand_name+"&nbsp;&nbsp;"+row.product_name+"&nbsp;&nbsp;"+row.art_no+'</p><p>ID:'+row.code+'</p></div></div>';
                }else{
                	html = '<div><div class="width-60"><img src="${ctx}/file_servelt'+row.sku_pic+'"></div><div class="width-226" style="width:390px;"><p class="ellipsis" title="'+row.brand_name+"&nbsp;&nbsp;"+row.product_name+"&nbsp;&nbsp;"+row.art_no+'">'+row.brand_name+"&nbsp;&nbsp;"+row.product_name+"&nbsp;&nbsp;"+row.art_no+'</p><p>ID:'+row.code+'</p></div></div>';
                }
            	return html;
            }},
            {"data": "id","title":"数量","render": function (data, type, row, meta) {
                if(row.product_count == '0.00'){
                    return '0';
                }else{
            ...
            {"data": "id","title":"平台优惠","render": function (data, type, row, meta) {
                if(row.platform_preferential == '0.00'){
                    return '0';
                }else{

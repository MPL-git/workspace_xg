...
<div class="x_panel container-fluid gf-hd">
 ...
 <c:if test="${activityType == 8 || activityType == 9 }">
  	...
  	<!--报名过  -->
   	<c:if test="${isExistence==0 }">
		...
		<c:if test="${TAG eq 1 && activityRuleConfiguration.shopComment ne null}">&nbsp;&nbsp;<img alt="" src="${ctx}/static/images/warn.png" style="margin-bottom:3px;">店铺DSR评分三项指标平均值不得低于${activityRuleConfiguration.shopComment}</c:if>
   		...
   		<button class="buttonShopText" id="sevenDay" style="background: rgba(153,153,153,1);" disabled>立即报名</button>
   		<span class="shenhe">审核状态:</span>
   		<span class="fuhe"><img alt="" src="${ctx}/static/images/fullWarn.png" style="margin-bottom:3px;">驳回</span>
   		<span class="shijian" > 距重新报名还剩: </span>  <span class="shijian" id="spanTime"></span>
   	...
   	<ul class="nav nav-tabs q" role="tablist" style="margin-top:20px;">
        <li> <a href="#" role="tab" data-toggle="tab" class="weiqiehuanText" >活动介绍</a></li>
        <li <c:if test="${tabType eq 1}">class="change"</c:if>> <a href="#" role="tab" data-toggle="tab" <c:if test="${tabType eq 1}">class="qiehuanText"</c:if>>活动要求</a></li>
    </ul>
    <div class="lines"></div>
	<div class="introduceContent">对店铺的要求</div>
	<div class="tables">
	 <table border="1" bordercolor=#ddd style="width:100%">
	 	...
	 	<tr>
	 		<td>店铺DSR评分三项指标平均值</td>
	 		<td>
	 			<c:if test="${activityRuleConfiguration.shopComment ne null}">
	 				店铺DSR评分三项指标平均值不得低于${activityRuleConfiguration.shopComment}
	 			</c:if>
	 			<c:if test="${activityRuleConfiguration.shopComment eq null}">
	 				无
	 			</c:if>
	 		</td>
	 	</tr>
	...
	<div class="introduceContent">对商品的要求</div>
	<div class="tables">
	 <table border="1" bordercolor=#ddd style="width:100%">
	 	<tr>
	 		<td width="60%">活动要求</td>
	 	...
	 	<c:if test="${activityRuleConfiguration.priceRulesDesc != null || activityRuleConfiguration.priceRulesDesc != '' || activityRuleConfiguration.priceRulesDesc != 'null'}">
	 	<tr>
	 		<td>价格规则</td>
	 		<td>
	 			<c:if test="${activityRuleConfiguration.priceRulesDesc ne null}">
	 				${activityRuleConfiguration.priceRulesDesc}
	 			</c:if>
	 			<c:if test="${activityRuleConfiguration.priceRulesDesc eq null}">
	 				无
	 			</c:if>
	 		</td>
	 	</tr>
	 	...
	 	<tr>
	 		<td>销量规则</td>
	 		<td>
	 			<c:if test="${activityRuleConfiguration.salesRulesDesc ne null}">
	 				${activityRuleConfiguration.salesRulesDesc}
	 			</c:if>
	 			<c:if test="${activityRuleConfiguration.salesRulesDesc eq null}">
	 				无
	 			</c:if>
	 		</td>
	 	</tr>
	 	<tr>
	 		<td>库存规则</td>
	 		<td>
	 			<c:if test="${activityRuleConfiguration.stockRulesDesc ne null}">
	 				${activityRuleConfiguration.stockRulesDesc}
	 			</c:if>
	 			<c:if test="${activityRuleConfiguration.stockRulesDesc eq null}">
	 				无
	 			</c:if>
	 		</td>
	 	</tr>
	 	<tr>
	 		<td>好评率</td>
	 		<td>
	 			<c:if test="${activityRuleConfiguration.favorableRate ne null}">
	 				${activityRuleConfiguration.favorableRate *100}%
	 			</c:if>
	 			<c:if test="${activityRuleConfiguration.favorableRate eq null}">
	 				无
	 			</c:if>
	 		</td>
	 	</tr>
...
<div class="video_box" style="position:fixed; width:500px; height:350px; top:35%; left:35%; display: none;" id="reasonsDiv">
    <a href="javascript:;" class="video_close"><img src="${ctx}/static/images/close.png"></a> 
    	<div class="panel panel-default" style="text-align:left" >
		   ...
		   <p style="text-align:-webkit-auto; margin:15px ;  color:#123456;font-size:15px;height:80px"  >${sourceNiche.auditRemarks }</p>

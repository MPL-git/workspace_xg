...
<div class="x_panel container-fluid py-tm">
    ...
    <div class="search-container container-fluid vm-famate">
        <form id="dataFromView">
            ...
            <input type="hidden" id="preSellAuditStatus" value="${activity.preSellAuditStatus}" name="preSellAuditStatus">
            <input type="hidden" id="preferentialType" value="${activityArea.preferentialType}" name="preferentialType">
            <div class="table-responsive">
                ...
                <table class="table table-bordered ">
                    ...
                    <tr id="tr_preferentialType">
                        <td class="editfield-label-td">申请预售</td>
                        <td colspan="2" class="text-left">
                            <c:if test="${activity.preSellAuditStatus == 0}">未申请</c:if>
                            <c:if test="${activity.preSellAuditStatus == 1}">已申请(待审核)</c:if>
                            <c:if test="${activity.preSellAuditStatus == 2}">已申请(通过)</c:if>
                            <c:if test="${activity.preSellAuditStatus == 3}">已申请(驳回)</c:if>
                        </td>
                    </tr>
                            ...
                            <div class="container-fluid">
                                <div class="row" id="fullGive_type1" style="margin-bottom: 10px;">
                                    <c:if test="${fullGive.type==1}">
                                        满${fullGive.minimum}元赠${fullGive.productNum}件<c:if test="${fullGive.sumFlag==1}">【可累加】</c:if>(商品ID：${fullGive.get('productCode')})
                                    </c:if>
                                    <c:if test="${fullGive.type==2}">
                                        买即赠(商品ID：${fullGive.get('productCode')})
                                    </c:if>
                                </div>
                            </div>
                        </td>
                                    ...
                                    <c:if test="${not empty fullDiscount && fullDiscount.get('ruleList').size()>0}">
                                        <c:forEach var="rule" items="${fullDiscount.get('ruleList')}" varStatus="vs" >
                                            <c:if test="${fullDiscount.type==1}">
                                                <span class="fullDiscount_text1">满</span>${rule.minimum}<span class="fullDiscount_text2">件</span>
                                                <span class="fullDiscount_text3">减</span>${rule.money}<span class="fullDiscount_text4">件</span>
                                            </c:if>
                                            <c:if test="${fullDiscount.type==2}">
                                                ${rule.minimum}<span class="fullDiscount_text2">件</span>
                                                ${rule.money}<span class="fullDiscount_text4">元</span>
                                            </c:if>
                                            <c:if test="${fullDiscount.type==3}">
                                                ${rule.minimum}<span class="fullDiscount_text2">件</span>
                                                ${rule.money}<span class="fullDiscount_text4">折</span>
                                            </c:if>
                                            <c:if test="${vs.index+1 != fullDiscount.get('ruleList').size()}">，</c:if>
                                        </c:forEach>
                                    </c:if>
                                </div>
                            </div>
                            ...
                            <c:if test="${not empty activity && activity.designAuditStatus == '3'}">
	                            	<span id="seeDesign-span">
	                            		...
	                            		<span style="color: red;">设计状态：驳回</span>
	                           			...
	                           			<c:if test="${not empty activity && activity.status != '4'}">
                                            <a href="javascript:updateDesignImg(${activity.id }, '2');">【修改图片】</a>
                                        </c:if>
	                            	</span>
                            </c:if>
                        </td>
                    ...
                    <tr>
                        ...
                        <td colspan="2" class="text-left">
                            <c:if test="${activity.status eq '1' || activity.status eq '4' }">
                                <a class="btn btn-info" href="javascript:;" onclick="commitActivity(${activity.id});">【提报审核】</a>
                                <a class="table-opr-btn" href="javascript:;" onclick="edit(${activityArea.id});">【修改活动】</a>
                                <a class="table-opr-btn" href="javascript:;" onclick="del(${activityArea.id});">【删除活动】</a>
                            </c:if>
                            <c:if test="${empty isHide}">
    ...
    <div class="x_content container-fluid">
        <div class="row">
            <h4>商品信息  <c:if test="${activity.status == 1 || activity.status == 4}"><a href="javascript:;" id="toBatchEdit">【批量改价/库存】</a></c:if></h4>
            <div class="col-md-12 datatable-container">
                <table id="datatable"
                    ...
                    <tr role="row">
                        ...
                        <c:if test="${activity.preSellAuditStatus != 0}">
                            <th width="88">定金金额</th>
                            <th width="88">可抵扣金额</th>
                        </c:if>
...
<!--弹出批量发货Div-->
<div class="video_box" style="position:fixed; width:400px; height:265px; top:35%; left:0;right:0;margin:0 auto; display: none;border-radius: 2px;" id="batchEditDiv">
    ...
    <div class="panel panel-default" style="height: 200px;">
        ...
        <form class="form-horizontal" role="form" id="uploadForm" style="margin-top: 25px;" >
            <div style="padding-left: 20px;padding-top: 20px;">
                <div>商品信息：<a href="javascript:;" id="downloadExcel">【下载】</a></div>
                <br>
                <div>
                    <span style="float: left;margin-top: 5px;">批量改价/库存：</span>
                    <input style="float: left;width: 180px;" id="file" name="file" type="file" accept="application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet">
                    <input type="hidden"  id="filePath" name="filePath" value="">
                    <a href="javascript:;" id="import">【确认】</a>
                </div>
            </div>
        </form>
    </div>
...
<!--弹出div End-->
<div class="black_box" style="display: none;" id="batchEditBlackBox"></div>
...
<script id="tpl_data" type="text/html">
    <input type="hidden" id="oldSortValue" name="oldSortValue" value="">
    <ul>
        ...
        <li data-id="{%= o.list[i].id %}" data-activityproductid="{%= o.list[i].activityProductId %}">
            <div class="con-preview-top">
                ...
                <input class="sort" type="text" value="{%= i+1 %}"  id="onlySort"  >
...
<script type="text/javascript">
    ...
    var viewerPictures;
    $(function () {
        $("#toBatchEdit").on('click',function(){
            $("#batchEditDiv").show();
            $("#batchEditBlackBox").show();
        });

        $(".video_close").on('click',function(){
            $("#batchEditDiv").hide();
            $("#batchEditBlackBox").hide();
        });

        $("#downloadExcel").on('click',function(){
//    		swal("正在下载excel...");
            location.href="${ctx}/mcht/activity/exportExcel?activityId=${activity.id}";
        });
        ...
        dataFromValidate = $("#dataFromView").validate({
        ...
        table = $('#datatable').dataTable({
            ...
            "columns": [
                ...
                {"data": "stock"},
                {"data": "activityProductSaleQuantity"},
                {
                    "data": "id", "render": function (data, type, row, meta) {
                        var html = [];
                        if(!row.activityProductOperateAuditStatus || row.activityProductCooAuditStatus == "1" || row.activityProductOperateAuditStatus == "1" || row.activityProductCooAuditStatus == "3" || row.activityProductOperateAuditStatus == "3"){
                            var preSellAuditStatus = $("#preSellAuditStatus").val();
                            console.log(preSellAuditStatus)
                            if(preSellAuditStatus == 1 || preSellAuditStatus == 3){
                                html.push('<a class="table-opr-btn" href="javascript:;" onclick="toActivityProductDeposit(${activity.id},'+row.id+');" >预售设置</a>');
                            }
                        }
        ...
        $("#import").on('click',function(){
            ...
            $.ajax({
                method: 'POST',
                url: '${ctx}/mcht/activity/importExcel?activityId='+$("#activityId").val(),
                data: formData,
                dataType: 'json',
                cache: false,
                processData: false,
                ...
                if (result.returnCode =='0000') {
                    $("#batchEditDiv").hide();
                    $("#batchEditBlackBox").hide();
                    if(result.returnData.errorCount>0){
                        swal("您的"+result.returnData.successCount+"条商品记录成功导入，有"+result.returnData.errorCount+"条导入失败！");
                    }else{
                        swal("您的"+result.returnData.successCount+"条商品记录成功导入。");
                    }
                    table.ajax.reload();
                }else{
                    if(result.returnMsg){
                        swal(result.returnMsg);
                        if(result.returnMsg != "请先上传文件"){
                            $("#batchEditDiv").hide();
                            $("#batchEditBlackBox").hide();
                        }
                    }else{
    ...
    function setSortValue(that) {
        that.closest('li').find("input").val(that.closest('li').index() + 1);
        let lis = that.closest('li').siblings();
        for (let i = 0; i < lis.length; i++) {
            let li = lis[i];
            let l = $(li).index() + 1;
            let find = $(li).find("input");
            find.val(l);
        }
    }
    ...
    function viewAndSortSucsess(data) {
        ...
        $('.sort').focus(function () {
            ...
            set_text_value_position(this,length)
        });
        ...
        $(".sort").blur(function () {
            var that = $(this);
            let value = this.value;
            let index = that.closest('li').index() + 1;

            if (value < 1 || value > that.closest('ul').find('li').length) {
                swal({
                    title: "输入的排序号不在范围内！",
                    type: "error",
            ...
            if (value < index) {
                if (value == 1) {
                    ...
                    let s1 = '#con_preview li:nth-child(' + index + ') .by-top';
                    let s2 = '#con_preview li:nth-child(' + index + ') .to-top';
                    let s3 = '#con_preview li:nth-child(' + index + ') span';
                    $(s3).removeClass('ban');
                    $(s1).addClass('ban');
                    $(s2).addClass('ban');
                }
                if (index == that.closest('ul').find('li').length) {
                    if (value != 1) {
                        $('#con_preview li:last-child span').removeClass('ban');
                    }

                    let n = index - 1;
                    let s1 = '#con_preview li:nth-child(' + n + ') .by-bot';
                    let s2 = '#con_preview li:nth-child(' + n + ') .to-bot';
                    let s3 = '#con_preview li:nth-child(' + n + ') span';
                    $(s3).removeClass('ban');
                    $(s1).addClass('ban');
                    $(s2).addClass('ban');
                }
                //将某个商品排序值降低（减少排序值），【调整值后及原排序值】区间的商品排序值都+1
                // 比如：原商品值为25 调整为10， 则 原值10-24区间的商品排序值都+1
                $('#con_preview li').eq(that.closest('li').index() + number).before(that.closest('li'));
            }
            if (value > index) {
                if (that.closest('li').index() == 0) {
                    // that.removeClass('ban').siblings().removeClass('ban')
                    $('#con_preview li:nth-child(1) span').removeClass('ban');
                ...
                if (value == that.closest('ul').find('li').length) {
                    // $('#con_preview li:nth-child(2) span').addClass('ban');
                    let s1 = '#con_preview li:nth-child(' + index + ') .by-bot';
                    let s2 = '#con_preview li:nth-child(' + index + ') .to-bot';
                    let s3 = '#con_preview li:nth-child(' + index + ') span';
                    $(s3).removeClass('ban');
                    $(s1).addClass('ban');
                    $(s2).addClass('ban');
                    // that.siblings('.by-bot').addClass('ban');
                    if (that.closest('ul').find('li').length != 2) {
                        $('#con_preview li:last-child span').removeClass('ban');
                    }
                }
                //将某个商品排序值提高（提高排序值），【调整值后及原排序值】区间的商品排序值都-1，
                // 比如：原商品值为25 调整值为35， 则 原26-35区间的商品排序值都 - 1
                $('#con_preview li').eq(that.closest('li').index() + number).after(that.closest('li'));
            }
            // 不管增减,只要把this换到合适的位置之后,将索引+1改成排序值就好了,索引获取到其他的li,修改value
            // this.value = value;
            setSortValue(that);
        });
    }
    ...
    function toActivityProductDeposit(activityId,productId){
        $.ajax({
            url: "${ctx}/mcht/activity/toActivityProductDeposit?activityId="+activityId+"&productId="+productId,
            type : 'GET',
            success: function(data){
                $("#viewModal").html(data);
                $("#viewModal").modal();
    ...
    function commitSave() {
        ...
        if (dataFromValidate.form()) {
            ...
            var dataJson = $("#dataFromView").serializeArray()
            dataJson.push({"name":"json", "value":JSON.stringify($("#dataFromView").find(':input').filter(function () {
...
//                        });
    ...
    function toBack(){
        ...
        getContent("${ctx}/mcht/activity?pageNumber=${pageNumber}&name="+name+"&productBrandId=${productBrandId}&status=${status}")
    }
</script>
</body>
</html>

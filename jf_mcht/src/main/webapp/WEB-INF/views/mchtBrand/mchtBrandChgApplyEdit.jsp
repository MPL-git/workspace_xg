  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="width: 910px;">
      <div class="modal-header">
		...
		<form id="dataFrom">
		<input type="hidden" value="${mchtProductBrand.id}" name="mchtProductBrandId" id="mchtProductBrandId">
			...
			<table class="table table-bordered ">
				<tbody>
					...
					<tr>
						...
						<td colspan="2" class="text-left">
							<input validate="{required:true}" type="text" value="${mchtBrandChg.productBrandName}" <c:if test="${not empty mchtProductBrand.id || not empty mchtBrandChg.id}">readonly="readonly"</c:if> name="productBrandName" id="productBrandName">
					...
					<tr>
						...
						<td colspan="2" class="text-left">
						  <select  name="aptitudeType" id="aptitudeType" validate="{required:true}">
						   ...
						   <option value="${aptitudeTypeStatus.statusValue}"   <c:if test="${mchtBrandChg.aptitudeType==aptitudeTypeStatus.statusValue}">selected</c:if>>${aptitudeTypeStatus.statusDesc}</option>
						   </c:forEach>
                          </select>
						</td>
					</tr>
					...
					<tr>
						...
						<td colspan="2" class="text-left">
							...
							<c:if test='${mchtBrandChg.logo!=null&&mchtProductBrand.logo!=""}'>
							<div class="single_pic_picker"><input id="logoFile" onchange="loadImageFile(this)" type="file"><img  src="${ctx}/file_servelt${mchtBrandChg.logo}"><input type="hidden"  id="logo" name="logo" value="${mchtBrandChg.logo}"></div>
							</c:if>
							<c:if test='${mchtBrandChg.logo==null||mchtProductBrand.logo==""}'>
							<div class="single_pic_picker"><input id="logoFile" onchange="loadImageFile(this)" type="file"><div>+</div><input type="hidden"  id="logo" name="logo" value=""></div>
				    </c:if>
						</td>
					</tr>
					...
					<tr>
						...
						<td colspan="2" class="text-left">
										...
										<td colspan="2" class="text-left">
											<div class="pic-container">
												<ul class="docs-pictures clearfix td-pictures pictures-list" name="aptitude-pictures-list" ondrop="drop(event)" ondragover="allowDrop(event)">
													<c:if test="${empty mchtBrandChg.id}">
													...
													<c:if test="${not empty mchtBrandChg.id}">
							...
							<table class="table table-bordered" name="mchtBrandAptitudeTable" mchtBrandAptitudeId="">
								<tbody>
									<tr>
										<td class="editfield-label-td">商标注册证号<span class="required">*</span></td>
										<td colspan="2" class="text-left">
											<input validate="{required:true}" type="text" value="" name="certificateNo">
											<div style="display: inline-block;color: #999999;">商标注册证号必填</div>
										</td>
									</tr>
									<tr>
					...
					<tr>
						...
						<div class="pic-container">
								...
								<c:forEach var="mchtPlatformAuthPic" items="${mchtPlatformAuthPicChgs}" varStatus="varStatus">
					...
					<tr>
						...
						<td colspan="2" class="text-left">
						<input type="text" validate="{required:true}" value="<fmt:formatDate value='${mchtBrandChg.platformAuthExpDate}' pattern='yyyy-MM-dd'/>" id="platformAuthExpDate" name="platformAuthExpDate" data-date-format="yyyy-mm-dd">
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td colspan="2" class="text-left">
						<div style="display: inline-block;color: #999999;">质检报告出具人必须是商标权人或入驻商家，质检报告上要体现的品牌名须与商标证书上的一致，且质检报告是最新一年的质检报告</div>
						<div class="pic-container">
					...
					<tr>
						<td class="editfield-label-td">报告有效期<span class="required">*</span></td>
						<td colspan="2" class="text-left">
						<input type="text" validate="{required:true}" value="<fmt:formatDate value='${mchtBrandChg.inspectionExpDate}' pattern='yyyy-MM-dd'/>" id="inspectionExpDate" name="inspectionExpDate" data-date-format="yyyy-mm-dd">
						</td>
					</tr>
					...
					<tr>
						...
						<td colspan="2" class="text-left">
						<input type="text" value="<fmt:formatDate value='${mchtBrandChg.otherExpDate}' pattern='yyyy-MM-dd'/>" id="otherExpDate" name="otherExpDate" data-date-format="yyyy-mm-dd">
						</td>
					</tr>
...
<!--弹出div End-->
<div class="black_box" style="display: none;" id="mchtBrandChgBlackBox"></div>
...
$(function(){
	...
	$("input[name='aptitudeExpDate']").each(function(){
		$(this).datetimepicker(
	    		{
    ...
    $("#addMchtBrandAptitude").on('click',function(){
    	...
    	html.push('<td colspan="2" class="text-left">');
    	html.push('<input type="text" validate="{required:true}" value="" id="'+aptitudeExpDateId+'" name="aptitudeExpDate" >');
...
//			dataJson.push({"name":"mchtBrandChangeAgreementPics","value":JSON.stringify(mchtBrandChangeAgreementPics)});
			...
			$.ajax({
				url : "${ctx}/mcht/mchtBrandChgCommitAudit",
				type : 'POST',
				dataType : 'json',
				cache : false,
				async : false,

  ...
  <div class="modal-dialog" role="document" style="width:700px;">
    <div class="modal-content">
      <div class="modal-header">
		...
		<form id="dataFrom">
		<input type="hidden" value="${mchtTaxInvoiceInfo.id}" name="id">
			...
			<table class="table table-bordered ">
				<tbody>
					<tr>
						...
						<td colspan="2" class="text-left">
						  <select  name="taxType" id="taxType" validate="{required:true}">
						   ...
						   <c:forEach var="taxTypeDescItem" items="${taxTypeDesc}">
						   <option value="${taxTypeDescItem.statusValue}"   <c:if test="${mchtTaxInvoiceInfo.taxType==taxTypeDescItem.statusValue}">selected</c:if>>${taxTypeDescItem.statusDesc}</option>
						   </c:forEach>
                          </select>
						</td>
					</tr>
					...
					<tr>
						...
						<td colspan="2" class="text-left">
						<input style="width:80%;" type="text" id="taxNumber" name="taxNumber" value="${mchtTaxInvoiceInfo.taxNumber }" validate="{required:true}">
						</td>
					</tr>
										<tr>
						<td class="editfield-label-td">地址</td>
						<td colspan="2" class="text-left">
						<input style="width:80%;" type="text" id="address" name="address" value="${mchtTaxInvoiceInfo.address }"  validate="{required:true}">
						</td>
					</tr>
										<tr>
						<td class="editfield-label-td">电话</td>
						<td colspan="2" class="text-left">
						<input style="width:80%;" type="text" id="tel" name="tel" value="${mchtTaxInvoiceInfo.tel }" validate="{required:true}">
						</td>
					</tr>
					<tr>
						<td class="editfield-label-td">开户行</td>
						<td colspan="2" class="text-left">
						<input style="width:80%;" type="text" id="depositBank" name="depositBank" value="${mchtTaxInvoiceInfo.depositBank }" validate="{required:true}">
						</td>
					</tr>
					<tr>
						<td class="editfield-label-td">帐号</td>
						<td colspan="2" class="text-left">
						<input style="width:80%;" type="text" id="accountNumber" name="accountNumber" value="${mchtTaxInvoiceInfo.accountNumber }" validate="{required:true}">
						</td>
					</tr>
					...
					<tr>
						...
						<td colspan="2" class="text-left">
							<c:if test='${mchtTaxInvoiceInfo.confirmFile!=null&&mchtTaxInvoiceInfo.confirmFile!=""}'>
							<div class="single_pic_picker"><input id="confirmFileFile" onchange="loadImageFile(this)" type="file"><img  src="${ctx}/file_servelt${mchtTaxInvoiceInfo.confirmFile}"><input type="hidden"  id="confirmFile" name="confirmFile" value="${mchtTaxInvoiceInfo.confirmFile}"></div>
							</c:if>
							<c:if test='${mchtTaxInvoiceInfo.confirmFile==null||mchtTaxInvoiceInfo.confirmFile==""}'>
					...
					<c:if test="${mchtTaxInvoiceInfo.id!=null}">
					<tr>
						<td class="editfield-label-td">状态</td>
						<td colspan="2" class="text-left">
						<span>${mchtTaxInvoiceInfo.auditStatusDesc }</span>
						</td>
					</tr>
					</c:if>
					<c:if test="${mchtTaxInvoiceInfo.id!=null&&mchtTaxInvoiceInfo.auditStatus=='4' }">
					<tr>
						<td class="editfield-label-td">驳回原因</td>
						<td colspan="3" class="text-left">
						<span>${mchtTaxInvoiceInfo.auditRemarks }</span>
						</td>
					</tr>
					</c:if>
				</tbody>
...
function commitSave(){
	...
	if(dataFromValidate.form()){
		...
		$.ajax({
			url : "${ctx}/mchtTaxInvoice/mchtTaxInvoiceCommitSave",
			type : 'POST',
			dataType : 'json',
			cache : false,
			async : false,
...
function commitAudit(){
	if(dataFromValidate.form()){
		...
		var dataJson = $("#dataFrom").serializeArray();
		$.ajax({
			url : "${ctx}/mchtTaxInvoice/mchtTaxInvoiceCommitAudit",
			type : 'POST',
			dataType : 'json',
			cache : false,
			async : false,

...
<div class="x_panel container-fluid">
    ...
    <div class="row content-title">
        <div class="t-title">
       		 	活动商品管理
       		 <c:if test="${type eq '1'}">
	       		 <a href="javascript:void(0);" onclick="getContent('${ctx}/plat/activity/viewActivity?id=${activityAreaId}&activityId=${activity.id}')">返回</a>
       		 </c:if>	
       		 <c:if test="${type eq '2'}">
        ...
        <form class="form-horizontal" id="searchForm">
            ...
            <div class="form-group">
                ...
                <div class="col-md-3 search-condition" >
                    <input class="form-control" type="text"  id="searchKeywrod" name="searchKeywrod" >
                </div>
            ...
            <div class="col-md-12 datatable-container">
                ...
                <c:if test="${status==1}">
                    <div class="datatable-caption">
                        ...
                        <c:if test="${activityArea.isPreSell eq '1' && activityArea.type eq '2'}">
                            <input type="checkbox" id="advanceSaleTag">报名即表示同意<a href="javascript:;" onclick="toAgreement();">《醒购预售商家协议》</a>
                        </c:if>
                        <c:if test="${preferentialWay != null}">
                            <span style="color: red">${preferentialWay}</span>
                        </c:if>
                        <c:if test="${activityArea.preferentialType eq '5'}">
                ...
                <table id="datatable"
                    ...
                    <tr role="row">
                        ...
                        <c:if test="${status==2 &&activityArea.isPreSell ne '1' && activityArea.preferentialType eq '5'}">
                        <th width="68">购物津贴</th>
                        </c:if>
                        <th width="68">库存</th>
                        <th width="88">销量</th>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#btn-batch-add").click(function(){
            var productIds=[];
            if(${activityArea.isPreSell ne '1'}){
                ...
                var tag = true;
                var productIds=[];
                var productIdsAdvanceSale = [];
                $("input[name='productId']:checked").each(function(){
                    if(!$('#advanceSaleTag').is(':checked') && ${activityArea.type eq '2'}){
                        tag = false;
                    ...
                    if(${sessionScope.mchtInfo.mchtType eq '1'}){
                        payInAdvance = $(this).parent().parent().parent().parent().children().eq(3).children().val().replace(/\s+/g, "");
                        deduction = $(this).parent().parent().parent().parent().children().eq(4).children().val().replace(/\s+/g, "");
                    }else{
                        payInAdvance = $(this).parent().parent().parent().parent().children().eq(2).children().val().replace(/\s+/g, "");
                        deduction = $(this).parent().parent().parent().parent().children().eq(3).children().val().replace(/\s+/g, "");
                    }
                    //预付金额、抵扣金额不可为空
                    if(payInAdvance == "" || deduction == ""){
                        tag = false;
                        swal('预付金额与抵扣金额不可为空');
                        return;
                    }
                    ...
                    //预付金额、抵扣金额不可小于0
                    if(Number(payInAdvance) <= 0  || Number(deduction) <= 0){
                        tag = false;
                        swal('预付金额与抵扣金额不可小于0');
                        return;
                    }
                    ...
                    if(!reg.test(payInAdvance) || !reg.test(deduction)){
                        tag = false;
                        swal('预付金额与抵扣金额只能是数字');
                        return;
                    }
                    //预付金额、抵扣金额不能大于上限
                    if(Number(payInAdvance) >99999999.99  || Number(deduction) >99999999.99){
                        tag = false;
                        swal('预付金额和抵扣金额不能超过上限');
                        return;
                    }
                    var product = {"productId":$(this).val(),"payInAdvance":payInAdvance,"deduction":deduction};
                    productIds.push($(this).val());
                    productIdsAdvanceSale.push(product);
                });
                if(!tag){
                    return;
                }
                ...
                addProducts(productIds,productIdsAdvanceSale);
            }
        });
        ...
        table = $('#datatable').dataTable({
            ...
            "columns": [
                ...
                // }},
                // </c:if>
                {"data": "id","width":"230","render": function (data, type, row, meta) {
                    var html = [];
                    
                    <c:if test="${status==1}">
                        html.push('<div class="is-check">');
                        if(${activityArea.isPreSell eq '1'}){
                            html.push('<div class="width-33"><input type="checkbox" onclick="tests();" name="productId" value="' + data + '" /></div>');
                        }else{
                            if(row.svipDiscount){
                                html.push('<div class="width-33"><input type="checkbox" onclick="tests();" name="productId" value="' + data + '" /></div>');
                            }
                    ...
                    html.push('<div class="width-226"><a href="https://m.xgbuy.cc/share_buy.html?id='+row.code+'" target="_blank">'+row.name+'</a><br><p style="color: #999;margin: 5px 0 0;">货号：<div class="lineFeed">'+row.artNo+'</div></p><p style="color: #999;margin: 0;">ID：'+row.code+'</p></div>');
                    html.push("</div>");
               ...
               /* 购物津贴*/
                <c:if test="${status==2 &&activityArea.isPreSell ne '1' && activityArea.preferentialType eq '5'}">
                    {"data": "id","render": function (data, type, row, meta) {
                                return "满"+row.minimum+"减"+row.money;
                ...
                <c:if test="${activityArea.isPreSell eq '1'}">
                    {"data": "id","render": function (data, type, row, meta) {
                        ...
                        <c:if test="${status eq '1'}">
                            return '<input id="payInAdvance"  placeholder="请输入数字">';
                        </c:if>
                    }},
                    {"data": "id","render": function (data, type, row, meta) {
                        <c:if test="${status eq '2' || status eq '3'}">
                        ...
                        <c:if test="${status eq '1'}">
                            return '<input width="150" id="deduction" placeholder="请输入数字">';
                        </c:if>
                    }},
                </c:if>
                {"data": "stock"},
                {"data": "saleQuantity"},
                {"data": "id", "width":"60","render": function (data, type, row, meta) {
                        var html = [];
                        <c:if test="${status==2}">
                        ...
                        html.push('<br/>');
                        html.push('<a class="table-opr-btn" href="javascript:void(0);" id="test" onclick="addProduct('+row.id+',this);" >立即报名</a>');
                        html.push('<br/>');
                        </c:if>
                        <c:if test="${status==3}">
                        html.push('<a class="table-opr-btn" href="javascript:void(0);" onclick="editProduct(', "'"+row.id+"'", ');" >修改商品</a>');
    ...
    function addProduct(productId,_this) {
        if(${activityArea.isPreSell eq '1'}) {
            if(${status ne '3'}){
                //判断是否勾选同意协议
                if(!$('#advanceSaleTag').is(':checked') && ${activityArea.type eq '2'}){
                ...
                if(${sessionScope.mchtInfo.mchtType eq '1'}){
                    debugger;
                    payInAdvance = $(_this).parent().parent().children().eq(3).children().val().replace(/\s+/g, "");
                    deduction = $(_this).parent().parent().children().eq(4).children().val().replace(/\s+/g, "");
                }else{
                    payInAdvance = $(_this).parent().parent().children().eq(2).children().val().replace(/\s+/g, "");
                    deduction = $(_this).parent().parent().children().eq(3).children().val().replace(/\s+/g, "");
                }
                //预付金额、抵扣金额不可为空
                if(payInAdvance == "" || deduction == ""){
                    swal('预付金额与抵扣金额不可为空');
            ...
            var productIdsAdvanceSale = [];
            var product = {"productId":productId,"payInAdvance":payInAdvance,"deduction":deduction};
            productIdsAdvanceSale.push(product);
            addProducts(productIds,productIdsAdvanceSale);
        }else {
            var productIds = [];
    ...
    function addProducts(productIds,productIdsAdvanceSale) {
        var postData= {};
        postData.activityId = ${activityId};
        postData.productIds = JSON.stringify(productIds);
        postData.productIdsAdvanceSale = JSON.stringify(productIdsAdvanceSale);
        postData.isPreSell = ${activityArea.isPreSell};
        postData.productStatus = ${status};
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    //table.ajax.reload();
                    if(${activityArea.isPreSell eq '1' && status eq '1'}){
                        if(data.data.returnType == "0"){

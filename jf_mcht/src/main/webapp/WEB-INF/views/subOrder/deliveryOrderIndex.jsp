...
$(document).ready(function () {
	...
	table =	$('#datatable').dataTable({
        "ajax": function (data, callback, settings) {
            ...
            $.ajax({
                method: 'POST',
                url: '${ctx}/subOrder/getDeliveryOrderList',
                data: param,
                dataType: 'json'
        ...
        "rowCallback": function( row, data, index ) {
			...
			if(subOrder.memberStatus=='P'){
				blackMemberRemark = '<span style="color:red;">（订单异常，请勿发货）</span>';
			}
        	...
        	if(customerServiceType == "直赔单"){
           	...
           	rowtext.push('</table>');
          	rowtext.push('</td>');
          	rowtext.push('<td width="118" class="br"><div>快递未打<br>发货单未打</div></td>');
          	rowtext.push('<td width="118" name="operation'+subOrder.status+'" data-suborderid="'+subOrder.id+'" data-statusdate="'+subOrder.deliveryLastDate+'"></td>');
          	rowtext.push('</tr>');
          	rowtext.push('</table>');
	...
	$("button[name='exportSelected']").on('click',function(){
		...
		var subOrderIds = subOrderIdArray.join(",");
		location.href="${ctx}/subOrder/exportSelectedDeliverySubOrder?subOrderIds="+subOrderIds;
	});
	...
	$("#batchSet").on('click',function(){
		...
		$.ajax({
	        method: 'POST',
	        url: '${ctx}/subOrder/batchExpress',
	        data: {"subOrderIds":subOrderIds,"expressId":expressId},
	        dataType: 'json'
	    }).done(function (result) {
	        if (result.returnCode =='0000') {
	        	$("#batchExpressDiv").hide();
	...
	$("#confirmSelected").on('click',function(){
		...
		$.ajax({
	        method: 'POST',
	        url: '${ctx}/subOrder/confirmSelected',
	...
	$("#import").on('click',function(){
		...
		$.ajax({
	        method: 'POST',
	        url: '${ctx}/subOrder/importExpressNo',
	        data: formData,
	        dataType: 'json',
	        cache: false,  
            processData: false,  
	...
	$("#toUpdate").on('click',function(){
		...
		$.ajax({
	        method: 'POST',
	        url: '${ctx}/subOrder/batchUpdateExpress',
	        data: formData,
	        dataType: 'json',
	        cache: false,  
            processData: false,  
...
function list(searchStatus) {
    var url = "${ctx}/subOrder/deliveryOrderIndex?searchStatus=" + searchStatus;
    getContent(url);
}

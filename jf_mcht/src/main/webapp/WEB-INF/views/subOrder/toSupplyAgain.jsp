	 ...
	 <div>
		 ...
		 <div class="col-md-12 datatable-container" id="deliveryDiv">
		 	<table name="table" class="table table-striped table-bordered dataTable no-footer"
				...
				<tr>
					<td><span style="white-space: nowrap;">发货商品信息*</span></td>
					<td>
						<ul class="list-group">
							<c:forEach items="${orderDtlList}" var="orderDtl">
								<li class="list-group-item">
									<div style="display: inline-flex;">
										<input type="checkbox" name="orderDtlId" value="${orderDtl.id}" <c:if test="${orderDtl.deliveryStatus eq 0}">checked="checked"</c:if>>
	  ...
	  $("#addNewDelivery").on('click',function(){
	  	...
	  	$("#commit").on('click',function(){
	  		...
	  		$("table[name='table']").each(function(){
	  			...
	  			var merchantCode = $(this).find("input[name='merchantCode']").first().val().trim();
	  		...
	  		$.ajax({
	            method: 'POST',
	            url: '${ctx}/subOrder/supplyAgain',
	            data: {subOrderId:$("#subOrderId").val(),jsonStr:JSON.stringify(array)},
	            dataType: 'json'

    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
     	<span class="modal-title" >修改物流</span>
      </div>
	 ...
	 <div>
		 ...
		 <div class="col-md-12 datatable-container" id="deliveryDiv">
		 	<table name="table" class="table table-striped table-bordered dataTable no-footer"
					...
					<td>
						<select class="form-control" name="expressId" >
							...
							<c:forEach var="express" items="${expressList}">
								<c:if test="${express.id eq orderDtl.dtlExpressId}">
								<option value="${express.id}" selected = "selected">${express.name}</option>
								</c:if>
								<c:if test="${express.id ne orderDtl.dtlExpressId}">
				...
				<tr>
					...
					<td>
						<input type="text" class="form-control" name="expressNo" style="width: 200px;" value="${orderDtl.dtlExpressNo}">
					</td>
				</tr>	
				<tr>
					<td class="shopnum">
					...
					<td>
						<input placeholder="若您选择则的快递公司为京东,请务必填写快递商户编号" type="text" class="form-control" name="merchantCode" style="width:100%" value="${orderDtl.merchantCode}">
					</td>
				</tr>			
				<tr>
					<td>发货商品信息*</td>
		...
		<span style="margin-left: 10px;"><a href="javascript:;" id="addNewDelivery">新增修改信息</a></span>
  ...
  $(function(){	  
	  ...
	  $("#addNewDelivery").on('click',function(){
	  	...
	  	$("#commit").on('click',function(){
	  		...
	  		$.ajax({
	            method: 'POST',
	            url: '${ctx}/subOrderNew/editDeliveryNew',

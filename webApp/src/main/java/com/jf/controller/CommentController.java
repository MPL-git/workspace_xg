...
public class CommentController extends BaseController{
	...
	public ResponseMsg addMemberComment(HttpServletRequest request){
		try {
			...
			Map<String,Object> map = commentService.addMemberComment(reqDataJson,memberId);
	...
	public ResponseMsg addMemberCommentDraw(HttpServletRequest request){
		try {
			...
			this.requiredStr(obj,request);
			Integer memberId = getMemberId(request);
			commentDrawService.addMemberCommentDraw(reqDataJson,memberId);
	...
	public ResponseMsg getLuckDrawRule(HttpServletRequest request){
		try {
			...
			dataMap.put("content", "醒购用户于活动期间对已购买商品进行评价，即有机会至醒购抽奖活动界面刮奖一次，有机会获得最高8888积分的奖励。");
			dataList.add(dataMap);
	...
	public ResponseMsg updateOrderComment(HttpServletRequest request){
		try {
			...
			this.requiredStr(obj,request);
			Integer memberId = getMemberId(request);
			commentService.updateOrderComment(reqDataJson,memberId);
			return new ResponseMsg(ResponseMsg.SUCCESS,ResponseMsg.SUCCESS_MSG);
		} catch (ArgException args){
			return new ResponseMsg(ResponseMsg.ERROR,args.getMessage());
		}catch (Exception e) {

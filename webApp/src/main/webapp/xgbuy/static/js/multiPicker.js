...
function MultiPicker(config) {
	...
	this.int = config.int;
	this.checkRangepost = config.checkRangepost
	...
	this.idxArr = [];
	this.jsonArr = [];
	this.liHeight = win.lib ? Number(doc.getElementsByTagName('HTML')[0].style.fontSize.replace('px', '')) * 1 : 40;
	this.maxHeight = [];
	this.distance = [];
	this.start = {
		Y: 0,
	...
	this.resultArr = [];
	this.initDomFuc();
	this.initReady(0, this.jsonData[this.int[0]]);
	this.initBinding();
}
...
MultiPicker.prototype = {
	...
	generateArrData: function (targetArr) {
		...
		loop(0, targetArr.length, function (i) {
			tempArr.push({
				"id": targetArr[i].areaId,
				"value": targetArr[i].areaName
			})
		});
		return tempArr;
	},
	...
	initBinding: function () {
		...
		_this.resultArr.length = 0;
		loop(0, _this.idxArr.length, function (i) {
			_this.distance[i] = _this.int[i] ? _this.int[i]*_this.liHeight : _this.distance[i] 
			var dh =  _this.distance[i] / _this.liHeight
			_this.resultArr.push({
				"id": _this.jsonArr[i][ dh + 2].id,
				"value": _this.jsonArr[i][dh + 2].value,
				"index": dh
			});
		});
	...
	checkRange: function (idx) {
		...
		switch (idx) {
			case 0:
				tb0 = _this.distance[0] / _this.liHeight
				_this.checkRangepost(tempObj[tb0].areaId, function (res) {
					console.log(res)
					tempObj[tb0].child = res
				})
				// _this.checkRangepost(tempObj[tb0].child[0].areaId, function (res) {
				// 	console.log(res)
				// 	tempObj[tb0].child[0].child = res
				// })
				readydata = tempObj[tb0]
				break;
			case 1:
				tb0 = _this.distance[0] / _this.liHeight
				tb1 = _this.distance[1] / _this.liHeight
				// _this.checkRangepost(tempObj[tb0].child[tb1].areaId, function (res) {
				// 	console.log(res)
					tempObj[tb0].child[tb1].child = []
				// })
				readydata = tempObj[tb0].child[tb1]
				break;
		}

		_this.initReady(idx, readydata);
	},
	...
	touch: function (event, that, $picker, array, idx) {
		...
		switch (event.type) {
			...
			case "touchend":
				...
				var targetIdx = this.distance[idx] / this.liHeight;
				if (temp != that.distance[idx]) this.checkRange(idx);
				setTimeout(function () {

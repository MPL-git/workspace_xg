var checkin_draw = $('.page[data-id="activity_newsign_draw"]:visible'),
	checkin_draw_scroll = checkin_draw.find('#notScroll'),
	...
	_role = checkin_draw.find('.role-text');
...
var checkinDrawApi = {
	...
	record: function () {
		...
		document.getElementById("notScroll").classList.add("scroll-hide");
	},

	closeSelf: function(){
		closeSelf();
		document.getElementById("notScroll").classList.remove("scroll-hide");
	},
	...
	getCnf: function (_amount, _id, _withdrawText) {
		...
		if (!member.app) {
			...
			if (_td.memberBalance < _amount) {
				...
				this.confirmCnf(_amount, _withdrawText, function () {
						...
						success: function (e) {
							...
							if (_type == '0') {
								...
								if (member.sys == 'android') {
									checkAppVer(52, function () {
										appBindDrawCash(1);
									});
								} else if (member.sys == 'ios') {
									checkAppVer(54, function () {
										callIntentTrailer('intentTrailerListNative', 'appBindDrawCash', {'type': 1});
									});
								}
							} else if (_type == '2') {
								// 微信未绑定
								if (member.sys == 'android') {
									...
									checkAppVer(54, function () {
										callIntentTrailer('intentTrailerListNative', 'appBindDrawCash', {'type': 2});
									});
								}
							}
						},
						error: function (e) {
							if (_td.isWithdrawFail == false) {
								delayTip(e.returnMsg);
							}
							if (_td.memberBalance >= _amount) {
								if (_td.isWithdrawFail == true) {
									getUrl('activity/newsign/draw/yichang/index.html?withdrawCashId=' + _id, true);
								}
							} else {
								delayTip("余额不足");
							}
	...
	confirmCnf: function (_money, _type, _fun) {
		...
		$('.page:visible').append('<div class="popup popup-style-confirm popup-style-sign show">'
			+ '<div class="popup-checkin-con" onclick="stopSelf()">'
				...
				+ '<h3 class="flex ac jc"> '+ (_type == "现金券" ? "确定兑换" : "确定领取") +'' + _money + '元' + _type + '</h3>'
				+ '<div class="flex jc">'
	...
	confirmSuss: function (_money, _type) {
		...
		$('.page:visible').append('<div class="popup popup-style-confirm popup-style-sign show">'
			+ '<div class="popup-checkin-con" onclick="stopSelf()">'
				...
				+ '<div class="flex jc">'
					+ '<a class="btn-checkin flex ac jc fn" style="width: 2.8rem;" onclick="closeSelf(this)">知道了</a>'
				+ '</div>'
			+ '</div>'
		+ '</div>');
...
function onShow() {
	traffic.setPv(checkinDrawApi.data);
}